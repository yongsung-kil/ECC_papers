...
- **Status**: ❌
- **Reason**: DRAM 메모리 UMP 대수 부호(Hamming 변형) — LDPC 아닌 블록코드, NAND LDPC 이식 기법 없음
- **ID**: ieee:8720213
- **Type**: journal
- **Published**: Oct. 2019
- **Authors**: Clayton Schoeny, Frederic Sala, Mark Gottscho +3
- **PDF**: https://ieeexplore.ieee.org/document/8720213
...
- **Status**: ❌
- **Reason**: IoT HRLL 튜토리얼 — 유한블록 채널코드 언급뿐, 신규 LDPC 디코더·구성 기여 없음 서베이
- **ID**: ieee:8673568
- **Type**: journal
- **Published**: Oct. 2019
- **Authors**: Zheng Ma, Ming Xiao, Yue Xiao +3
- **PDF**: https://ieeexplore.ieee.org/document/8673568
...
- **Status**: ❌
- **Reason**: 음향 OFDM Doppler 주파수오프셋 추정 — 동기화 기법, ECC/LDPC 무관
- **ID**: ieee:8488526
- **Type**: journal
- **Published**: Oct. 2019
- **Authors**: Amir Tadayon, Milica Stojanovic
- **PDF**: https://ieeexplore.ieee.org/document/8488526
...
- **Status**: ❌
- **Reason**: Polar 부호 OFDM-IDMA 결합 검출/복호 — polar 전용 JDD, 바이너리 LDPC BP로 이식되는 부호 비의존 기법 아님
- **ID**: ieee:8718402
- **Type**: journal
- **Published**: Oct. 2019
- **Authors**: Xiangyun Deng, Jin Sha, Xiaotian Zhou +4
- **PDF**: https://ieeexplore.ieee.org/document/8718402
...
- **Status**: ❌
- **Reason**: 코드 변조 채널 용량/유한길이 이론 분석(Gallager mapping), 떼어낼 디코더·HW·코드설계 기법 없음 — 순수 이론 bound
- **ID**: ieee:8927997
- **Type**: conference
- **Published**: 23-25 Oct.
- **Authors**: Chong Zhang, Xijin Mu, Jinhong Yuan +2
- **PDF**: https://ieeexplore.ieee.org/document/8927997
- **Abstract**: This paper investigates the capacity of coded modulation (CM) schemes with Gallager mapping based constellations in the finite length regime. We first present the channel capacity for M-ary modulations with a finite block-length. Then, we discuss the performance of Gallager mapping based constellation with probabilistic shaping. We show that at an asymptotic high signal-to-noise-ratio (SNR), the channel capacity of equiprobable constellations with a finite block-length can reach the capacity with infinite block-length while the channel capacity of nonequiprobable constellations can not approach the capacity in the infinite length regime. Finally, we discuss the channel capacity loss of non-equiprobable constellations in the finite length regime. At an asymptotic high SNR, the channel dispersion of nonequiprobable constellations equals to a positive constant, which causes the channel capacity loss for non-equiprobable constellations. We also derive an upper bound and a tight approximation of the asymptotic capacity loss (ACL) of the constellations in the finite length regime.

## Performance Evaluation of Autoencoder for Coding and Modulation in Wireless Communications

- **Status**: ❌
- **Reason**: 무선 통신용 end-to-end autoencoder vs Hamming code 비교 — LDPC 기법 아님, 떼어낼 ECC 기법 없음
- **ID**: ieee:8928071
- **Type**: conference
- **Published**: 23-25 Oct.
- **Authors**: Jialong Xu, Wei Chen, Bo Ai +5
- **PDF**: https://ieeexplore.ieee.org/document/8928071
- **Abstract**: The end-to-end autoencoder is a novel and attracting concept to innovate communication system architecture. In its training stage, the end-to-end autoencoder needs differentiable channel models to execute back-propagation algorithm. This shortage impedes the further development of the end-to-end autoencoder. Estimating the channel model by reinforcement learning is a good way to alleviate this problem. However, these training methods increase the difficulty in the training stage. In this paper, we set up a general channel model and employ the measured channel data as input data to alleviate this problem. We compare the performance of the end-to-end autoencoder and Hamming code modulated by Binary Phase Shift Keying under additive white Gaussian noise channel and Rayleigh channel and explore the performance of the end-to-end encoder under the real scenarios. Furthermore, we investigate the performance of the end-to-end autoencoder under mismatched channels. The results demonstrate that the performance of the end-to-end autoencoder is similar with Hamming code and has a significant performance improvement under mismatched channels.

## A BP-NN Decoding Algorithm for Polar Codes

- **Status**: ❌
- **Reason**: polar 부호 BP-NN 분할 디코딩 — polar 서브블록 NN, LDPC BP 비의존적 이식 불명
- **ID**: ieee:8927910
- **Type**: conference
- **Published**: 23-25 Oct.
- **Authors**: Chen Wen, Jian Xiong, Lin Gui +1
- **PDF**: https://ieeexplore.ieee.org/document/8927910
- **Abstract**: Polar code is selected for the 5-th generation of wireless communication standard. Meanwhile, with the demand of low latency in 5G and great success in deep learning area, neural network decoder (NND) becomes a promising candidate for future decoder. However, NND is currently only feasible for very short code lengths. In this paper, we proposed a belief propagation-neural network (BP-NN) algorithm for polar codes based on partitioning. The core of our algorithm is replacing subblock of belief propagation (BP) decoder with NNDs, and using BP decoding framework to connect them. In addition, we also introduce a weight function to complete the message propagation process. Simulation results show the proposed algorithm reduces the decoding delay compared to the traditional BP algorithm and have a well bit error rate (BER) performance. At the same time, it also suitable for long polar codes.

## Deep Learning-Based Decoding of Block Markov Superposition Transmission

- **Status**: ❌
- **Reason**: BMST 부호용 DNN 디코더+sliding window — 비-LDPC, 부호 특화 신경망 디코더로 바이너리 LDPC BP로 이식할 부호 비의존 기법 없음
- **ID**: ieee:8928042
- **Type**: conference
- **Published**: 23-25 Oct.
- **Authors**: Sheng Bi, Qianfan Wang, Zengzhe Chen +2
- **PDF**: https://ieeexplore.ieee.org/document/8928042
- **Abstract**: In this paper, we present deep learning-based decoding of block Markov superposition transmission (BMST). For the basic code, a deep neural network (DNN) is trained, providing with good decoding performance. The complexity of the trained DNN can be reduced by removing those negligibly small coefficients. For the BMST code, we introduce a sliding window decoding algorithm, which integrates the DNN decoder of the basic code in an iterative manner. Numerical results show that the performance of our proposed scheme is close to that of the BMST system with conventional decoders and matches well with the corresponding lower bound of BMST. The above results verify that the extra gain and lower bound of BMST are effective for neural network decoder and thus demonstrate the universality of BMST.

## BP List Decoding of Polar Codes with Adaptive Bit Splitting over Critical Set

- **Status**: ❌
- **Reason**: polar 부호 BP list 디코딩(BS-BPL) — bit-splitting/critical-set이 polar 전용, 바이너리 LDPC BP에 비의존적 이식 어려움
- **ID**: ieee:8927848
- **Type**: conference
- **Published**: 23-25 Oct.
- **Authors**: Deng Zheng, Kangjian Qin, Zhaoyang Zhang
- **PDF**: https://ieeexplore.ieee.org/document/8927848
- **Abstract**: Low decoding delay and low block error rate (BLER) are two most challenging design goals for 5G channel decoders. The state of the art cyclic redundancy check (CRC) aided successive cancellation list (CA-SCL) decoders suffer from the high decoding delay due to its bit-by-bit decoding ordering albeit its advantage of relatively low complexity. In this paper, we resort to a modified version of belief propagation (BP) decoding, i.e., the so-called BS-BPL, and take advantage of its parallel structure to address the issue. In particular, we first modify the BP list decoder with a special processing element which turns the unconditional messages achieved in a conventional BP decoder into the conditional ones which are required in the bit splitting process in list decoding. Further, for each iteration, we only conduct the bit splitting in positions within the so-called Critical Set which includes only the most vulnerable bits, thus resulting a greatly reduced number of splits. On the one hand, the proposed algorithm can determine multiple bits per iteration and effectively terminate earlier than the traditional CA-SCL algorithm. On the other hand, the entire codeword can be directly output due to the parallel property of BP, avoiding that some correct paths are wrongly deleted halfway as in SCL. Numerical results show that our proposed BS-BPL decoders Significantly outperform their CA-SCL counterparts in terms of decoding delay and BLER.

## A PAPR-Reduction SCMA Codebook Design for Satellite Communication Systems

- **Status**: ❌
- **Reason**: SCMA 위성 코드북 PAPR 저감 — LDPC는 부수 베이스라인, 떼어낼 ECC 기법 없음
- **ID**: ieee:8927869
- **Type**: conference
- **Published**: 23-25 Oct.
- **Authors**: Xin Ma, Jiayi Zhang, Zhongyang Yu +2
- **PDF**: https://ieeexplore.ieee.org/document/8927869
- **Abstract**: Sparse code multiple access (SCMA), as a promising multiple access scheme suggested for the fifth generation (5G)wireless networks, can achieve higher user capacity. In this paper, we consider the SCMA technique for low earth orbit (LEO)satellite communication systems to improve user capacity and support massive connection for satellite Internet of Things (IoT). Note that, high peak-to-average power ratio (PAPR)is a tough challenge for SCMA-based LEO satellite downlink systems due to its power-limited property. Thus, an optimized codebook design based on the traditional codebook is proposed to reduce the PAPR. Firstly, the signal points with large power are reassigned to construct a low-PAPR mother constellation. Then, the constellation rotations of users colliding in a resource are jointly designed to further reduce the PAPR. Numerical results show that the designed codebook in an LDPC coded SCMA system can achieve lower PAPR while maintaining better performance and lower complexity than the traditional codebook.

## Efficiency of Satellite Transponder Using Paired Carrier Multiple Access with Low Density Parity Check

- **Status**: ❌
- **Reason**: 위성 트랜스폰더 대역폭 효율 연구에서 표준 LDPC modcod 사용 — 신규 LDPC 기여 없음
- **ID**: ieee:8980423
- **Type**: conference
- **Published**: 23-24 Oct.
- **Authors**: Rahmat Nursiaga, Mudrik Alaydrus
- **PDF**: https://ieeexplore.ieee.org/document/8980423
- **Abstract**: The increasing use of satellite communication systems for internet purposes requires much more bandwidth than the available. This work discusses the results of research on efficient use of bandwidth and transponder power using Paired Carrier Multiple Access. In this research, a satellite communication link point to point with a speed 2048 kbps was used for measuring the bandwidth and calculating the transponder power. This research is looking for the most optimal value of using transponder capacity with modulation, FEC and coding variations. The 8PSK LDPC 2/3 modcod is the most optimal ones because it uses the smallest transponder capacity 3.48% with 50% efficiency. The changing of transponder services from power limited to bandwidth limited will increase the obtained efficiency but reduce the link quality.
...
- **Status**: ❌
- **Reason**: interleaved LRPC(rank-metric) 부호 디코딩, 비-LDPC rank 부호로 이식 불가
- **ID**: ieee:9003356
- **Type**: conference
- **Published**: 21-25 Oct.
- **Authors**: Julian Renner, Thomas Jerkovits, Hannes Bartz
- **PDF**: https://ieeexplore.ieee.org/document/9003356
...
- **Status**: ❌
- **Reason**: polar code permutation 디코딩, polar SCL 전용 기법으로 바이너리 LDPC BP 이식성 없음
- **ID**: ieee:9003333
- **Type**: conference
- **Published**: 21-25 Oct.
- **Authors**: Mikhail Kamenev, Yulia Kameneva, Oleg Kurmaev +1
- **PDF**: https://ieeexplore.ieee.org/document/9003333
- **Abstract**: A new permutation decoding approach for polar codes is presented. The proposed decoder can be used to achieve the error correction performance of a successive cancellation list (SCL) decoder, while it can either improve latency of the SCL decoder or decrease its hardware complexity. As opposed to the SCL algorithm, the proposed permutation algorithm can be implemented without sorting operation. It is shown that the error correction performance of the algorithm is similar to that of the SCL decoder for polar codes. Moreover, a new construction aiming to improve the error correction performance of polar codes under the proposed algorithm is presented.

## On the Lightweight McEliece Cryptosystem for Low-Power Devices

- **Status**: ❌
- **Reason**: 경량 McEliece 암호(보안) 복잡도-보안 trade-off, 디코더/구성 이식 기여 없음
- **ID**: ieee:9003324
- **Type**: conference
- **Published**: 21-25 Oct.
- **Authors**: Fedor Ivanov, Evgenii Krouk, Alexey Kreshchuk
- **PDF**: https://ieeexplore.ieee.org/document/9003324
...
- **Status**: ❌
- **Reason**: polar 부호용 PCC 패리티검사 구성, polar 의존적 BP — 비-LDPC, 바이너리 LDPC BP로 떼어낼 기법 불명확
- **ID**: ieee:8966111
- **Type**: conference
- **Published**: 20-24 Oct.
- **Authors**: Inseon Kim, Hong-Yeop Song
- **PDF**: https://ieeexplore.ieee.org/document/8966111
- **Abstract**: In this paper, we propose a construction of parity-check equations for Low Density Parity-Check (LDPC) based Parity-Check Concatenated (PCC) polar codes in order to (i) increase the minimum Hamming weight and (ii) correct some errors using belief propagation decoding. We simulate some experiment to show that the proposed LDPC base PCC polar codes have coding gain in the sense of block error rate compared with existing PCC polar codes.
...
- **Status**: ❌
- **Reason**: mmWave WLAN FPGA OFDM 송수신 프로토타입; LDPC/디코더 기여 없음, 떼어낼 ECC 기법 없음
- **ID**: ieee:9020634
- **Type**: conference
- **Published**: 20-23 Oct.
- **Authors**: Dongming Ren, Kang Chen, Shengheng Liu +1
- **PDF**: https://ieeexplore.ieee.org/document/9020634
...
- **Status**: ❌
- **Reason**: AWGN 컨스텔레이션 AE 설계 + Q-ary(비이진) LDPC 소프트 디코딩 — 비이진 LDPC, 떼어낼 바이너리 기법 없음
- **ID**: ieee:9020501
- **Type**: conference
- **Published**: 20-23 Oct.
- **Authors**: Qisheng Huang, Ming Jiang, Chunming Zhao
- **PDF**: https://ieeexplore.ieee.org/document/9020501
...
- **Status**: ❌
- **Reason**: 비이진 스태빌라이저 양자부호, non-binary+양자 전용 - 제외
- **ID**: ieee:8965879
- **Type**: conference
- **Published**: 19-21 Oct.
- **Authors**: Jingjie Lv, Xi Zhang, Ruihu Li
- **PDF**: https://ieeexplore.ieee.org/document/8965879
- **Abstract**: Constructions of quantum error-correcting codes with good properties utilizing classical codes is an important task for quantum computing and quantum information. In this paper, we employ a class of one-generator quasi-negacyclic codes and provide a sufficient condition for symplectic self-orthogonality. As the computational results, some good non-binary stabilizer quantum codes are constructed and their parameters are determined.
...
- **Status**: ❌
- **Reason**: 제목·초록 모두 'Proceeding'으로 내용 없음, 판정 불가
- **ID**: ieee:8924526
- **Type**: conference
- **Published**: 17-19 Oct.
- **Authors**: 
- **PDF**: https://ieeexplore.ieee.org/document/8924526
- **Abstract**: Proceeding

## A Soft-Decision Demodulator for WBAN Systems Using Stochastic Computing

- **Status**: ❌
- **Reason**: WBAN π/4-DQPSK 소프트복조의 stochastic computing 구현 — 복조기이지 LDPC 디코더/ECC 기법 아님
- **ID**: ieee:8918990
- **Type**: conference
- **Published**: 17-19 Oct.
- **Authors**: Kaining Han, Junchao Wang, Qiang Fang
- **PDF**: https://ieeexplore.ieee.org/document/8918990
- **Abstract**: Wireless body area network (WBAN) is an efficient fundamental technology for medical and healthcare applications. Soft decision receiver is attractive for the reason that the performance gains result in significant transmitting energy reduction. However, the soft decision demodulation (SDD) for π/4-DQPSK modulation specified in IEEE 802.15.6 standard suffers from high complexity and power consumption. Stochastic computing is a promising low hardware cost and low power consumption implementation candidate. In this paper, a novel stochastic computing based soft decision demodulation method is proposed to achieve low complexity and low power demodulation. According to the evaluation results, the proposed design utilizes advantages on hardware cost and power consumption with respect to the existing SDD schemes. In addition, the proposed design supports prior probability input, demonstrating a potential for turbo-like iterative receiver for WBAN systems to further improve the performance.
...
- **Status**: ❌
- **Reason**: DVB-S2 멀티컴포넌트 신호·펄스성형 최적화, LDPC는 반복횟수만 부수 언급 — 떼어낼 ECC 기법 없음
- **ID**: ieee:8906865
- **Type**: conference
- **Published**: 17-18 Oct.
- **Authors**: Phuoc Nguyen T. H., Aleksandr Gelgor
- **PDF**: https://ieeexplore.ieee.org/document/8906865
- **Abstract**: This paper considers the improvement of DVB-S2 system by using bandwidth-efficient multicomponent signals (MCS) with optimal shaping pulses. Finite optimal pulses are obtained by solving the optimization problem of minimizing partial correlation of pulses taking into account spectral mask. Optimal pulses with different ISI levels are considered to choose the best pulses for various signal-to-noise conditions. It is showed that the pulse with high ISI level performs better as signal-to-noise ratio increases. Moreover, increasing the number of iterations in LDPC decoding leads to more bandwidth efficiency gain for the system with optimal pulses. A practical realization based on USRP device of the DVB-S2 system with optimal pulses is supposed to confirm system performance in radio frequency channel.

## A Systematic Multi-Packet HARQ Scheme Using Polar Codes for IoT

- **Status**: ❌
- **Reason**: polar code 기반 HARQ 처리량 최적화로 LDPC ECC에 떼어낼 디코더/HW/코드설계 기법 없음
- **ID**: ieee:8947016
- **Type**: conference
- **Published**: 16-19 Oct.
- **Authors**: Hao Liang, Aijun Liu, Chao Gong +2
- **PDF**: https://ieeexplore.ieee.org/document/8947016
- **Abstract**: The Internet of Things (IoT) in the 5G communication is expected to support energy-constrained low complexity applications with satisfactory throughput efficiency. Hybrid automatic repeat request (HARQ) schemes are required to enable wireless reliability with high transmission efficiency in IoT. To improve the throughput performance between the low-power devices, in this paper, a systematic multi-packet HARQ (SMP-HARQ) scheme using polar codes is first proposed. Then, in order to obtain maximal throughput, parameter configuration of proposed HARQ scheme is optimized with low complexity. The optimization strategy can be applied into fading channels. Finally, numerical simulation results show that the proposed SMP-HARQ could improve throughput performance compared to existing HARQ schemes for IoT applications over both the AWGN and Rayleigh fading channels, particularly in the low SNR regions.

## Information Coupled Irregular Repetition Slotted ALOHA

- **Status**: ❌
- **Reason**: IRSA 슬롯 ALOHA 접속 프로토콜·밀도진화 — 채널 LDPC ECC 디코더/구성 기법 아님
- **ID**: ieee:8946999
- **Type**: conference
- **Published**: 16-19 Oct.
- **Authors**: Dai Jia, Junli Jia
- **PDF**: https://ieeexplore.ieee.org/document/8946999
...
- **Status**: ❌
- **Reason**: HEVC 비디오용 UEP/ROI 스킴, 채널 LDPC ECC의 떼어낼 기법 없음
- **ID**: ieee:9015248
- **Type**: conference
- **Published**: 15-18 Oct.
- **Authors**: Bijaya Paudel, Sina Vafi, Prakash Bhattarai
- **PDF**: https://ieeexplore.ieee.org/document/9015248
- **Abstract**: This paper presents an unequal error protection (UEP) scheme for high efficiency video coding (HEVC) compressed video bitstreams based on region-of-interest (ROI). The concept of motion density (MD) is applied to identify important regions of the video frame. Simulation results confirm that the proposed scheme achieves better video quality, compared to conventional UEP approaches.
