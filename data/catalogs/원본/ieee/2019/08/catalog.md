...
- **Status**: ❌
- **Reason**: 인지간섭채널(CIC) 전용 joint decoder·LDPC 설계로 무선응용 특이적, NAND로 떼어낼 기법 없음
- **ID**: ieee:8692396
- **Type**: journal
- **Published**: Aug. 2019
- **Authors**: Mahdi Shakiba-Herfeh, A. Korhan Tanc, Tolga M. Duman
- **PDF**: https://ieeexplore.ieee.org/document/8692396
...
- **Status**: ❌
- **Reason**: non-binary LDPC 디코더(GF(q)) — 비이진 LDPC 명시 제외
- **ID**: ieee:8550764
- **Type**: journal
- **Published**: Aug. 2019
- **Authors**: Wenjie Huang, Weiguo Tang, Junlin Chen +1
- **PDF**: https://ieeexplore.ieee.org/document/8550764
- **Abstract**: This brief presents a low-power non-binary low density parity-check decoder design exploiting domain specific information for high-throughput wireless video communication applications. The proposed technique can dynamically scale the refresh rate of DRAM according to channel conditions as well as decoding states, hence to greatly reduce the DRAM power as well as the decoder power with lower refresh overhead. Evaluation results show that the proposed technique can achieve significant decoder power saving than conventional techniques.

## Edge Connection Optimization for JSCC System Based on DP-LDPC Codes

- **Status**: ❌
- **Reason**: DP-LDPC 기반 JSCC edge connection 최적화 — 소스-채널 결합(JSCC), 떼어낼 채널 ECC 기법 없음
- **ID**: ieee:8663419
- **Type**: journal
- **Published**: Aug. 2019
- **Authors**: Sanya Liu, Chen Chen, Lin Wang +1
- **PDF**: https://ieeexplore.ieee.org/document/8663419
- **Abstract**: Recently, it is found that the bit error rate performance can be improved by optimizing the edge connections between the source code and the channel code (BL1 or BL2 in the joint base matrix BJ) for joint source-channel coding system based on double protograph low density parity check codes. In this letter, the connection relationship in BL1 from the angle of both the source code and the channel code is analyzed, where the protograph LDPC codes with various row weights used as the source code are taken into account. And a search algorithm for BL1 under the constraints is proposed based on the joint protograph extrinsic information transfer (JPEXIT) and the source PEXIT to improve the performance. Both the JPEXIT analysis and the simulation results indicate that the performance of the searched optimal scheme has considerable gain compared with the conventional solutions.
...
- **Status**: ❌
- **Reason**: 레이더-통신 통합파형에 LDPC가 cascaded 베이스라인, 떼어낼 ECC 기법 없음
- **ID**: ieee:8693859
- **Type**: journal
- **Published**: Aug. 2019
- **Authors**: Qingyu Li, Keren Dai, Yu Zhang +1
- **PDF**: https://ieeexplore.ieee.org/document/8693859
...
- **Status**: ❌
- **Reason**: 광통신 위상잡음 보상(FG/SPA·칼만), LDPC 부호·디코더 기여 아님
- **ID**: ieee:8695027
- **Type**: journal
- **Published**: Aug. 2019
- **Authors**: Arni F. Alfredsson, Erik Agrell, Henk Wymeersch
- **PDF**: https://ieeexplore.ieee.org/document/8695027
...
- **Status**: ❌
- **Reason**: UV 통신에 표준 BP/BF LDPC 적용 비교만, 새 디코더/구성 없는 표준 기법 단순 재사용
- **ID**: ieee:8934709
- **Type**: conference
- **Published**: 5-8 Aug. 2
- **Authors**: Weiwei Wang, Ping Su, Jianshe Ma
- **PDF**: https://ieeexplore.ieee.org/document/8934709
- **Abstract**: Low-Density Parity-Check (LDPC) codes are applied into ultraviolet (UV) communication system. In this paper, LU decomposition algorithm is adopted as the channel coding method. Simulations are conducted to compare the performance of different LDPC decoding algorithms in UV communication system. The results show that the belief propagation (BP) based UV system has a better bit error ratio (BER) performance than the one with the bit flipping (BF) decoding algorithm. Moreover, communication system performance trade-offs among transmission distance, path loss, frame length and BER are studied.

## Superposition Construction of NB-QC-LDPC Codes for optical communication systems

- **Status**: ❌
- **Reason**: NB-QC-LDPC(비이진 GF(q)) 구성으로 비이진 LDPC 제외
- **ID**: ieee:8934825
- **Type**: conference
- **Published**: 5-8 Aug. 2
- **Authors**: Zhihui Wang, Liqian Wang, Dongdong Wang +1
- **PDF**: https://ieeexplore.ieee.org/document/8934825
- **Abstract**: In this paper, we consider the construction of NB-QC-LDPC code in the framework of superposition for optical communication systems. By maximizing the girth and reducing the number of g-cycles of the base matrix, NB-QC-LDPC codes with good cycle distributions could be constructed. Simulation results show that, with the same implementation cost, the constructed codes could achieve 0.1dB~0.4dB NCG in iterative decoding at the BER of 10 when compared with the recent proposed codes.

## Research on Adaptive Optimal Shaping Gain Selection MethodBased on Huffman Coding

- **Status**: ❌
- **Reason**: Huffman 기반 shaping gain 선택(소스 코딩/확률 성형)으로 채널 ECC가 아님
- **ID**: ieee:8933955
- **Type**: conference
- **Published**: 5-8 Aug. 2
- **Authors**: Ziqi Ren, Qi Zhang, Xiangjun Xin +7
- **PDF**: https://ieeexplore.ieee.org/document/8933955
...
- **Status**: ❌
- **Reason**: Polar 부호 CA-SCL 협력 통신 스킴; 비-LDPC, LDPC BP에 이식 가능한 부호 비의존 기법 없음
- **ID**: ieee:8862150
- **Type**: conference
- **Published**: 28-31 Aug.
- **Authors**: Rahim Umar, Fengfan Yang, Shoaib Mughal
- **PDF**: https://ieeexplore.ieee.org/document/8862150
- **Abstract**: This article proposes a network polar coded-cooperative single carrier frequency division multiple access (NPCC-SC-FDMA) scheme. The NPCC-SC-FDMA scheme is based on Plotkin construction having joint cyclic redundancy check (CRC) aided successive cancellation list (CA-SCL) polar decoding over the frequency-selective Rayleigh fading channels. The strong synergy between two users and their joint CA-SCL decoding significantly reduces the bit-error rate (BER) of proposed scheme. Monte Carlo simulated result confirms that proposed scheme outperforms the direct Plotkin based polar coded SC-FDMA scheme. Furthermore, the proposed scheme for a single user i.e., NPCC-SC-FDE (single carrier frequency domain equalization) scheme convincingly beats the existing network coded SC-FDE (NC-SC-FDE) scheme under an identical conditions at a higher SNR.
...
- **Status**: ❌
- **Reason**: 240GHz RF/트랜시버 설계, LDPC 기법 없음
- **ID**: ieee:8877170
- **Type**: conference
- **Published**: 27-30 Aug.
- **Authors**: Nebojsa Maletic, Vladica Sark, Mohamed H. Eissa +3
- **PDF**: https://ieeexplore.ieee.org/document/8877170
- **Abstract**: The increasing demand for high data rates and the emergence of new data-hungry applications motivate the exploration of higher frequency bands for the beyond fifth generation (5G) wireless systems. The frequencies above 100 GHz, so-called terahertz (THz) spectrum, offer abundant communication bandwidth, hence enabling, in theory, terabit throughputs using small transceivers and, potentially, high energy efficiency. In this paper, we describe prospective applications and discuss challenges related to short-to-mid-range communication at these frequencies. Moreover, we present some initial results on the 240 GHz system and radio-frequency (RF) transceiver design, envisioned within the WORTECS project. We show that, for applications requiring short-range high-throughput data links, highly integrated THz wireless transceivers with low efficiency on-chip beam steering antennas can be used. These fully integrated transceivers set considerable limitations in terms of achievable data rate and communication distance. To overcome these limitations, in this paper, we propose different approaches, especially for short-range applications.

## A Distributed Digital Codec for Jointly Sparse Correlated Signals

- **Status**: ❌
- **Reason**: 유한체 CS 행렬+joint BP의 압축센싱 소스코딩, 채널 ECC 아님 + 비이진(finite field)
- **ID**: ieee:8877301
- **Type**: conference
- **Published**: 27-30 Aug.
- **Authors**: Xuechen Chen, Fan Li, Xingcheng Liu
- **PDF**: https://ieeexplore.ieee.org/document/8877301
- **Abstract**: In this paper, targeting jointly sparse correlated signals, we propose a distributed digital codec framework. First, the non-zero coefficients of each signal are quantized and then mapped onto a finite field. Subsequently, measurements obtained by compressed sensing (CS) over finite fields, where Low-density Parity-check (LDPC) matrices are adopted as CS encoding matrices, are modulated and conveyed through separate additive white Gaussian noise (AWGN) channel. Such an order for the encoder is named as "first quantization, then compressed sensing". For joint recovery, a novel joint belief propagation (JBP) algorithm is applied at the sink node, where a new type of constraint node, i.e., correlation constraint nodes is introduced, which connect two factor graphs that separately represent the CS encoding matrix of each signal. Experimental results prove that the proposed scheme outperforms the scheme that ignores the correlated information between the signals, especially when the correlation is high. Moreover, we compare the proposed framework vis-a-vis "first compressed sensing, then quantization" framework and validate the superiority in terms of recovery quality.
...
- **Status**: ❌
- **Reason**: BEC상 SC-LDPC error floor의 covariance evolution 이론 분석만, 디코더/HW/구성 신규 기여 없는 순수 이론 bound
- **ID**: ieee:8989366
- **Type**: conference
- **Published**: 25-28 Aug.
- **Authors**: Yuta Nakahara, Toshiyasu Matsushima
- **PDF**: https://ieeexplore.ieee.org/document/8989366
...
- **Status**: ❌
- **Reason**: 레이더 간섭 공존 채널용 코드 설계(conv+LDPC), 채널 특이적 디코딩 메트릭, 바이너리 LDPC ECC에 이식할 신규 기법 없음
- **ID**: ieee:8989014
- **Type**: conference
- **Published**: 25-28 Aug.
- **Authors**: Federico Brunero, Daniela Tuninetti, Natasha Devroye
- **PDF**: https://ieeexplore.ieee.org/document/8989014
...
- **Status**: ❌
- **Reason**: GSM+Gallager mapping 무선 응용, 5G 표준 LDPC 및 비이진 LDPC 사용. 표준 부호 단순 적용+비이진
- **ID**: ieee:8855838
- **Type**: conference
- **Published**: 11-13 Aug.
- **Authors**: Dan Feng, Mengmeng Liu, Baoming Bai
- **PDF**: https://ieeexplore.ieee.org/document/8855838
- **Abstract**: In this paper, we combine the generalised spatial modulation (GSM) system with Gallager mapping (GM-GSM), a method of many-to-one mapping, to achieve the same spectral efficiency as GSM with less number of transmit antennas. In GM-GSM, all possible antenna combinations will be used. Furthermore, we apply 5G standard LDPC codes and nonbinary LDPC codes to GM-GSM systems to improve the bit error rate (BER) performance. Simulation results show that the proposed scheme can achieve nearly comparable performance of GSM system with reduced complexity.

## A RNN Decoder for Channel Decoding under Correlated Noise

- **Status**: ❌
- **Reason**: 컨볼루션 부호용 RNN 디코더(Viterbi 대체). 비-LDPC이고 BP에 이식할 메시지패싱 기법 아님
- **ID**: ieee:8849949
- **Type**: conference
- **Published**: 11-13 Aug.
- **Authors**: Xiangxiang Zhang, Tao Luo
- **PDF**: https://ieeexplore.ieee.org/document/8849949
- **Abstract**: We present and use a recurrent neural network as RNN decoder for the convolutional code under correlated noise. Using the bidirectional GRU(Gated Recurrent Unit) network and the fully connected neural network(DNN), the sequence feature information of the convolutional code is extracted, and the decoding result is calculated by the fully connected neural network. The RNN neural network decoder achieves better BER(Bit Error Rate) performance than the traditional Viterbi decoding algorithm for convolutional codes with shorter memory lengths, such as (2, 1, 3) convolutional codes under correlated noise. The greater noise correlation, the greater performance improvement of the decoder relative to the traditional Viterbi decoding algorithm. Besides, the RNN decoder is close to the MAP performance under the AWGN channel. In addition, the decoder is robust under different noise correlation models. Due to the limitation of the structure and complexity of the RNN decoder, as the memory length of the convolutional code increases, its decoding performance is gradually reduced, which is not suitable for convolutional codes with long memory lengths.

## A novel spinal codes based on chaotic Kent mapping

- **Status**: ❌
- **Reason**: Spinal codes(비-LDPC rateless) 복잡도 감소, 카오스 맵 기반 bubble decoder. LDPC BP에 이식 불가
- **ID**: ieee:8849929
- **Type**: conference
- **Published**: 11-13 Aug.
- **Authors**: Pengcheng Shi, Zhenyong Wang, Ci He +2
- **PDF**: https://ieeexplore.ieee.org/document/8849929
