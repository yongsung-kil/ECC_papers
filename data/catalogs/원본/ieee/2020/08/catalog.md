...
- **Status**: ❌
- **Reason**: 태양 신틸레이션 우주링크 end-to-end 시뮬레이션 — 수신기 파라미터, LDPC ECC 기법 없음
- **ID**: ieee:8977360
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Marco Baldi, Franco Chiaraluce, Nicola Maturo +8
- **PDF**: https://ieeexplore.ieee.org/document/8977360
...
- **Status**: ❌
- **Reason**: IoT 신뢰성 서베이 — LDPC/ECC 기법 없음, 도메인 일반 리뷰.
- **ID**: ieee:9089244
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Liudong Xing
- **PDF**: https://ieeexplore.ieee.org/document/9089244
...
- **Status**: ❌
- **Reason**: 분산컴퓨팅 가속용 resolvable design — 조합설계 기법, LDPC ECC 무관.
- **ID**: ieee:9103948
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Konstantinos Konstantinidis, Aditya Ramamoorthy
- **PDF**: https://ieeexplore.ieee.org/document/9103948
...
- **Status**: ❌
- **Reason**: 속도다양 무선 네트워크 코딩 전력할당 — LDPC 무관, 통신 응용 특이.
- **ID**: ieee:9097253
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Taotao Wang, Soung Chang Liew, Shakeel Salamat Ullah
- **PDF**: https://ieeexplore.ieee.org/document/9097253
- **Abstract**: This paper derives the maximum achievable sum-rate and presents the optimal encoding/decoding framework for rate-diverse wireless network coding (RD-WNC) over broadband channels consisting of multiple parallel subchannels. RD-WNC applies to a communication scenario in which a base station wants to deliver two different messages with different rates to two users. The base station combines the two separate messages into one network-coded message and broadcasts the network-coded message to both users. Each user then extracts its desired message from the network-coded message by subtracting from it the other message, which we assume to be side information available to the user. Deriving the maximum achievable sum-rate for RD-WNC is challenging when the channel consists of multiple parallel subchannels with different channel coefficients (e.g., the subcarrier channels of OFDM systems), since apart from the rate allocation between the two users, optimal power allocation among multiple subchannels needs to be identified. The first contribution of this paper is a new “mountain-leveling” power allocation algorithm to achieve the maximum sum-rate. With the resulting power allocation, we can then achieve the corresponding optimal sum-rate by having a separate encoding/decoding mechanism for each subchannels, but doing so is cumbersome and complex when the number of subchannels is large. The second contribution of this paper is a practical encoding/decoding framework using only one encoder-decoder mechanism for all subchannels without sacrificing sum-rate optimality. We provide numerical results to corroborate our theoretical findings and to demonstrate the benefits of our encoding/decoding framework.

## Construction and Optimization for Adaptive Polar Coded Cooperation

- **Status**: ❌
- **Reason**: polar coded cooperation — LDPC 아님, NAND 이식성 없음
- **ID**: ieee:9052681
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Hao Liang, Aijun Liu, Xian Liu +1
- **PDF**: https://ieeexplore.ieee.org/document/9052681
- **Abstract**: In this letter, an adaptive polar coded cooperation (APCC) is explored in the fading channels. In terms of the imperfect inter-user channels, the cooperation way of APCC is carefully designed based on the multi-level structure of polar codes. Meanwhile code construction with dynamic-optimized information sets ensures the coding gains at the receivers. Moreover, the adaptive code rates of two cooperative users are optimized by maximizing the throughput efficiency. Simulation results demonstrate the bit error rate and throughput advantages of APCC over the existing coded cooperation schemes.

## Bidirectional Soft-Decision Feedback Equalization for OFDM Systems

- **Status**: ❌
- **Reason**: OFDM용 양방향 소프트결정 피드백 이퀄라이저 — LDPC 무관, 떼어낼 ECC 기법 없음.
- **ID**: ieee:9072108
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Shiduo Zhao, Shefeng Yan, Junyi Xi
- **PDF**: https://ieeexplore.ieee.org/document/9072108
- **Abstract**: A bidirectional soft-decision feedback equalizer (BiSDFE) for orthogonal frequency-division multiplexing (OFDM) systems over doubly selective channels is proposed. Soft, rather than hard, decision feedback equalizer (SDFE) is adopted to suppress intercarrier interference (ICI) and the band structure of the frequency-domain channel matrix is exploited to trade off between performance and computational complexity. By combining a conventional SDFE with a frequency-reversed SDFE, the proposed scheme can eliminate ICI induced by neighboring subcarriers of both sides, thereby obtaining frequency-reversal diversity and mitigating error propagation. Simulation results have demonstrated the advantage of this approach in bit error rate (BER), especially in the channels with large Doppler spreads.

## Distributed Error Correction Coding Scheme for Low Storage Blockchain Systems

- **Status**: ❌
- **Reason**: 블록체인 메모리 저감용 LDPC 분산 코딩 — 표준 LDPC 응용, 새 ECC 기법 없음
- **ID**: ieee:9042862
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Huihui Wu, Alexei Ashikhmin, Xiaodong Wang +3
- **PDF**: https://ieeexplore.ieee.org/document/9042862
...
- **Status**: ❌
- **Reason**: LTP 프로토콜 변형(우주 링크 재전송/FEC 운용) — 떼어낼 LDPC ECC 기법 없음
- **ID**: ieee:8954819
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Nicola Alessi, Carlo Caini, Azzurra Ciliberti +1
- **PDF**: https://ieeexplore.ieee.org/document/8954819
...
- **Status**: ❌
- **Reason**: PAM-4 버스트모드 광수신기 아날로그 회로 — LDPC는 BER 기준점으로만 부수 언급, ECC 기법 없음.
- **ID**: ieee:9080551
- **Type**: journal
- **Published**: Aug. 2020
- **Authors**: Gertjan Coudyzer, Peter Ossieur, Johan Bauwelinck +1
- **PDF**: https://ieeexplore.ieee.org/document/9080551
...
- **Status**: ❌
- **Reason**: screen-to-camera 통신 OOK LLR 계산 — 채널 특이적 LLR, NAND LDPC로 이식할 ECC 기법 없음
- **ID**: ieee:9184521
- **Type**: conference
- **Published**: 9-12 Aug. 
- **Authors**: Norisato Suga, Ryohei Sasaki, Yuho Tanaka +2
- **PDF**: https://ieeexplore.ieee.org/document/9184521
- **Abstract**: This paper studies a log likelihood ratio (LLR) calculation method for an on-off keying symbol in screen to camera communication (SCC) systems. To enhance the reliability of SCC, forward error correction is an essential technique. For soft decision decoding of capacity approaching codes such as turbo code and low density parity check code, it is necessary to calculate the LLR as an input of the decoder. However, it has not been examined in any detail in the conventional studies on the SCC systems. This motivates us to focus on the LLR calculation method to realize a more reliable SCC system. For the calculation of LLR, we estimate the conditional probability of a received signal intensity assuming both Gaussian and Rayleigh distributions. To evaluate the coding scheme coupled with the proposed LLR calculation, we conduct an experiment using actual commercial devices. The results show that the proposed LLR calculation appropriately works and reduces bit errors.
...
- **Status**: ❌
- **Reason**: binary CEO 소스코딩 rate-distortion 비교 — 채널코딩 ECC 아님, 순수 이론 bound
- **ID**: ieee:9260575
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Mahdi Nangir, Jafar Pourrostam, Javad Musevi Niya +1
- **PDF**: https://ieeexplore.ieee.org/document/9260575
- **Abstract**: A comparison between the joint and the successive decoding schemes for a two-link case binary Chief Executive Officer (CEO) problem is presented. We utilize the logarithmic loss as the criterion for evaluating the total distortion of the decoding techniques. The binary symmetric channel (BSC) is considered as the test-channel model, which provides a theoretical rate-distortion bound. Simulation results show that the joint decoding scheme provides smaller compression rate and distortion values compared to the successive decoding scheme. On the other hand the successive decoding method has lower complexity; therefore, it is efficient for the cases with higher number of links.

## Implementation and Performance Analysis of Multiuser Detector for Massive MIMO OFDM System over Rayleigh Channel

- **Status**: ❌
- **Reason**: Massive MIMO OFDM 다중사용자 검출기, LDPC 무관
- **ID**: ieee:9198538
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Shefin Shoukath, P.A. Haris
- **PDF**: https://ieeexplore.ieee.org/document/9198538
- **Abstract**: The Massive Multiple Input Multiple Output system using Orthogonal Frequency Division Multiplexing is emerging as a promising technique for future high data rate wireless communication. As the number of antennas at transmitter and receiver increases, major advantage is that spectral efficiency can be improved. The fading may be effectively reduced because of the increased achievable diversity by Massive MIMO. Multiuser Detector for Massive MIMO OFDM model is implemented and its performance is analyzed. Symbol Error Rate, Bit Error Rate and outage probability of the multiuser detector are simulated and might be inferred that the performance will be enhanced as compared to MIMO OFDM. The radiation patterns of both the transmitter and receiver antennas are plotted and it can be substantiated that the output power radiated by the multiuser detector at the base station is radiated as desired in cellular wireless communication.

## A Simplified Multiset-Partition Distribution Matching for PS-16QAM Optical Fiber Systems

- **Status**: ❌
- **Reason**: 광섬유 PS-16QAM용 분포정합기(distribution matcher), 채널 ECC가 아닌 확률성형이라 무관
- **ID**: ieee:9255945
- **Type**: conference
- **Published**: 2-6 Aug. 2
- **Authors**: Xinlin Jing, Jing Zhang, Taowei Jin +2
- **PDF**: https://ieeexplore.ieee.org/document/9255945
- **Abstract**: We propose a simplified multiset-partition distribution matcher (MPDM) with pairwise probability partitioning without distribution changing. The simplified MPDM has a shaping gain and lower rate loss in fiber-optic systems with AWGN and fiber nonlinearities.
