...
- **Status**: ❌
- **Reason**: 광기록 채널용 parity-check 코드+post-processing 검출; 비-LDPC, 떼어낼 바이너리 LDPC BP 기법 없음
- **ID**: ieee:1532346
- **Type**: journal
- **Published**: Nov. 2005
...
- **Status**: ❌
- **Reason**: 블록오류확률 점근 분석(N,I→무한), 순수 이론 bound로 디코더/HW/구성으로 안 이어짐
- **ID**: ieee:1522644
- **Type**: journal
- **Published**: Nov. 2005
...
- **Status**: ❌
- **Reason**: MIMO용 IRA 부호 최적화(EXIT/density evolution); IRA→LDPC 변환 언급뿐, 표준 LDPC 설계기법 재사용으로 신규 이식 기여 약함
- **ID**: ieee:1545861
- **Type**: journal
- **Published**: Nov. 2005
...
- **Status**: ❌
- **Reason**: MIMO 공간상관 점근 mutual information 분석; LDPC/디코더 무관
- **ID**: ieee:1532498
- **Type**: journal
- **Published**: Nov. 2005
...
- **Status**: ❌
- **Reason**: 다중반송파 DS/SSMA 페이딩 성능 분석; LDPC/디코더 기법 없음
- **ID**: ieee:1532495
- **Type**: journal
- **Published**: Nov. 2005
- **Authors**: 
...
- **Status**: ❌
- **Reason**: 이중선택 페이딩 채널 block-differential 변조; ECC/LDPC 무관
- **ID**: ieee:1532494
- **Type**: journal
- **Published**: Nov. 2005
- **Authors**: 
...
- **Status**: ❌
- **Reason**: turbo 코드 UEP+CRC 기반 error-prone 비트 보호; 부호 의존적 후처리로 LDPC BP 이식성 없음
- **ID**: ieee:1532470
- **Type**: journal
- **Published**: Nov. 2005
...
- **Status**: ❌
- **Reason**: 무선 block-fading 채널 오류확률 union bound (순수 이론) — 떼어낼 디코더/HW/코드설계 기법 없음, LDPC 부수 언급도 없음
- **ID**: ieee:1573875
- **Type**: journal
- **Published**: Nov. 2005
...
- **Status**: ❌
- **Reason**: linear space-time code outage capacity 이론; ECC 디코더/구성 기법 없음
- **ID**: ieee:1545839
- **Type**: journal
- **Published**: Nov. 2005
...
- **Status**: ❌
- **Reason**: abstract null, 분산 소스 코딩(distributed source coding)용 LDPC - 소스 코딩이며 채널 ECC 아님
- **ID**: ieee:1599952
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: MIMO-OFDM 채널 특이적 LDPC 코드 설계, 떼어낼 NAND 이식 기법 불명(초록 없음, 무선 특화)
- **ID**: ieee:1600004
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: 분산 space-time 협력중계 응용, LDPC는 베이스라인이며 이식 가능한 디코더/코드설계 기법 없음
- **ID**: ieee:1599999
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: 비이진 분산 소스코딩+turbo, 비이진·비-LDPC·소스코딩 모두 제외 대상
- **ID**: ieee:4014079
- **Type**: conference
- **Published**: 30 Oct.-2 
- **Authors**: Wei Liu, Wenjun Zeng
- **PDF**: https://ieeexplore.ieee.org/document/4014079
- **Abstract**: In this paper we propose a bit-plane based, scalable distributed source coding (DSC) scheme for non-binary correlated sources. In the source coding part of DSC, the symbols are quantized and the quantization indices are converted into binary forms using Gray codes. The channel coding part encodes each bit-plane independently using turbo codes. The turbo decoder that works in binary domain is facilitated by an outer loop that captures the symbol-domain correlation. Simulation results show that, in asymmetric DSC of joint Gaussian sources, the overall system performance is greatly improved by using Gray codes. It is also demonstrated that progressive transmission is well supported.
...
- **Status**: ❌
- **Reason**: Rateless Slepian-Wolf(분산 소스코딩/rateless), 채널 ECC 기법 아님
- **ID**: ieee:1600071
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: 중계용 distributed/layered 코드, LDPC ECC 기법 아님
- **ID**: ieee:1600070
- **Type**: conference
- **Published**: 30 Oct.-2 
- **Authors**: G. Kramer
...
- **Status**: ❌
- **Reason**: abstract null, 제목상 EXIT vs VT 반복디코더 분석 도구 비교 - 순수 분석 도구, 떼어낼 디코더/HW/구성 기법 없음
- **ID**: ieee:1599898
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: 헤더압축용 convolutional HARQ, LDPC 아님 비-LDPC 부호
- **ID**: ieee:1599696
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: Wyner-Ziv 비디오 소스코딩, 채널 ECC 기법 없음
- **ID**: ieee:4013968
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: duobinary turbo code, block fading - 비-LDPC 부호이며 떼어낼 LDPC BP 기법 없음
- **ID**: ieee:1599823
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: 분산 wavelet 비디오 코딩, LDPC syndrome encoding=소스코딩(DSC), 채널 ECC 아님
- **ID**: ieee:4013954
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: sphere decoder 병렬구현(MIMO LLR 계산), LDPC 디코더 아님 검출기
- **ID**: ieee:1599816
- **Type**: conference
- **Published**: 30 Oct.-2 
...
- **Status**: ❌
- **Reason**: VLC+LDPC 결합 소스-채널 반복디코딩; JSCC라 떼어낼 순수 ECC 기법 없음
- **ID**: ieee:1629970
- **Type**: conference
- **Published**: 21-24 Nov.
...
- **Status**: ❌
- **Reason**: IEEE 802.16 터보코드 Chase-Pyndiah 성능평가; 비-LDPC 부호, 이식 디코더 기여 없음
- **ID**: ieee:1629960
- **Type**: conference
- **Published**: 21-24 Nov.
...
- **Status**: ❌
- **Reason**: software radio용 송수신 전력관리 스킴 중심, NAND 이식할 디코더/HW 신규 기여 부족
- **ID**: ieee:1579837
- **Type**: conference
- **Published**: 2-4 Nov. 2
