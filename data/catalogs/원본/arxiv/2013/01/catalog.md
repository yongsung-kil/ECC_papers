...
- **Status**: ❌
- **Reason**: Gaussian 간섭채널 무선 응용에 표준 LDPC 적용 — Han-Kobayashi 기반 무선 특이적, 떼어낼 신규 ECC 기법 없음
...
- **Status**: ❌
- **Reason**: SC-LDPC threshold saturation 순수 이론 증명 — 디코더·HW·구성으로 이어지지 않는 점근적 bound 증명
...
- **Status**: ❌
- **Reason**: spatial coupling을 증명 기법으로 사용하는 순수 이론(MAP threshold, replica formula) — 디코더/HW/구성 기여 없음
...
- **Status**: ❌
- **Reason**: 비이진(NB) LDPC 대상 비트 인터리빙 — 비이진 LDPC는 제외 대상
