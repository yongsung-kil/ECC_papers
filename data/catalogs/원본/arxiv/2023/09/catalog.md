...
- **Status**: ❌
- **Reason**: 3D 위상 양자코드(Layer Codes) 구성 — good qLDPC를 입력으로 쓰는 양자 전용 구성, 바이너리 NAND 이식 불가
...
- **Status**: ❌
- **Reason**: 양자 good LDPC code surgery(논리 큐비트 수술) — 양자 전용 개념, 고전 NAND ECC와 무관
...
- **Status**: ❌
- **Reason**: GKP-스태빌라이저 연접 코드 fault-tolerance 임계값 이론(양자 최적수송) — 양자 CV 전용, 이식 기법 없음
...
- **Status**: ❌
- **Reason**: 기하적 국소 양자/고전 코드 subdivision 구성 — balanced product qLDPC 기반 차원/거리 최적화 이론, 디코더/HW 기여 없음
...
- **Status**: ❌
- **Reason**: 양자 surface/Floquet 코드용 하이퍼그래프 fault 분할 디코더 — 양자 스태빌라이저 체크 구조 의존, 바이너리 채널 LDPC로 이식 불가
...
- **Status**: ❌
- **Reason**: 양자 surface/hypergraph product 부호, 스태빌라이저·논리 큐빗 등 양자 전용 개념 의존 — 원칙 제외
...
- **Status**: ✅
- **Reason**: 바이너리 LDPC 디코딩을 조합최적화/Ising 머신 물리연산으로 새 정식화 — 이식 가능 디코더 관점(C), 애매하여 살림
...
- **Status**: ❌
- **Reason**: QLDPC(양자) 부호를 Euclidean geometry로 구성, 양자 EC 전용 — 원칙 제외
