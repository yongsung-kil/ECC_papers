...
- **Status**: ❌
- **Reason**: 내고장성 양자계산 코드 concatenation — 양자 전용, 이식할 고전 LDPC 디코더 기법 없음
...
- **Status**: ❌
- **Reason**: surface code용 MWPM 매칭 디코더 — 스태빌라이저·위상 코드 의존, 양자 전용 제외
...
- **Status**: ❌
- **Reason**: 양자 메시지 BP(BPQM), classical-quantum 채널·paired measurement 의존 — 고전 바이너리 LDPC로 이식 불가
...
- **Status**: ❌
- **Reason**: LDPC 패리티행렬을 grant-free 접속 시퀀스로 활용한 IoT 검출기 — 채널 ECC 디코더 아님, 떼어낼 LDPC ECC 기법 없음
