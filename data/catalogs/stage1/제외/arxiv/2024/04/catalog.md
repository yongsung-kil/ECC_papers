...
- **Status**: ❌
- **Reason**: CSS/subsystem 양자 코드 stabilizer를 gauge로 분할 — 양자 전용 stabilizer 개념 의존, 고전 바이너리 LDPC 이식 불가
...
- **Status**: ❌
- **Reason**: qLDPC의 2D 국소 구현(bivariate bicycle, syndrome 측정) — 양자 전용 아키텍처, 고전 NAND LDPC 이식 불가
...
- **Status**: ❌
- **Reason**: 양자 LDPC(qLDPC) 코드 — 중성원자 레지스터/스태빌라이저 의존, 양자 전용으로 원칙 제외
...
- **Status**: ❌
- **Reason**: 양자 오류정정(parity code, biased-noise qubit, 스태빌라이저) — 양자 전용 개념 의존, 제외
...
- **Status**: ❌
- **Reason**: 광섬유 MIMO 유한블록길이 용량 bound 이론 — LDPC는 부수 비교 대상, 떼어낼 디코더/HW/구성 기법 없음
