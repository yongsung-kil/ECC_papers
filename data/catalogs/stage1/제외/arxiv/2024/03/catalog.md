...
- **Status**: ❌
- **Reason**: BP 디코딩 복잡도 제약하 BER lower bound 순수 이론 — 디코더/HW/구성 신규 기법 없는 bound 분석
...
- **Status**: ❌
- **Reason**: 양자 LDPC(QLDPC) 저지연 디코딩 — 신드롬 추출/circuit-level noise 등 양자 전용, 제외
...
- **Status**: ❌
- **Reason**: 양자 EC(many-hypercube concatenated codes), 양자 전용 인코더/디코더로 고전 바이너리 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: 양자/고전 qLDPC의 열역학·Gibbs sampling 패시브 QEC 이론, NAND 이식 디코더/HW/구성 없음
...
- **Status**: ❌
- **Reason**: 고전 LDPC로 stabiliser 양자회로 표현하는 양자 전용 도구, fault-tolerance 검증용일 뿐 NAND ECC 이식 기법 아님
