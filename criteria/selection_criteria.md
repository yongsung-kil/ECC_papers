...
- 양자 LDPC(qLDPC)·양자 EC, QKD·보안 — **단, 기반이 되는 고전 QC-LDPC 구성(girth·사이클 제거 등)을 독립적으로 떼어낼 수 있으면 포함(E)**
...
> 양자·무선 논문이라도 **디코더 알고리즘·HW·코드설계가 명확히 이식 가능**하면 포함(C/D/E).
> 양자 논문은 **"기반 고전 LDPC 부호설계가 독립적으로 떼어내지는가"**를 하위기준으로 판단:
> 떼어낼 수 있으면 포함(E), 순수 양자 EC 회로·직교성 전용이면 제외.
