...
- **블라인드 부호 인식/식별·블라인드 LLR 추정 (blind recognition/identification/estimation)** — 미지 파라미터의 부호를 외부에서 추정·식별하거나(인터셉트·인지통신 등) 채널 LLR을 블라인드 추정하는 문제는 NAND처럼 부호·채널이 이미 알려진 ECC와 무관하므로 제외. 부수적으로 min-sum/BP 디코더를 언급해도 기여의 본질이 블라인드 추정이면 제외
