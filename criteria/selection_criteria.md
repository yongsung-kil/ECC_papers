...
- 양자 LDPC(qLDPC)·양자 EC, QKD·보안 — **원칙 제외.** 단, 디코더·코드설계 기법이 **고전 바이너리 LDPC에서 유래**하고 양자 전용 개념(스태빌라이저·직교성·축퇴 등)에 **의존하지 않고 그대로 이식**되면 예외적 포함(C/E)
...
0-1. **양자(qLDPC)·양자 EC·QKD는 원칙 제외.** 단, 디코더·코드설계가 고전 바이너리 LDPC에서 유래하고 양자 전용 개념(스태빌라이저·직교성·축퇴)에 의존하지 않으면 예외 포함(C/E).
