...
0-1. **양자(qLDPC)·양자 EC·보안은 원칙 제외.** 단, 디코더·코드설계가 고전 바이너리 LDPC에서 유래하고 양자 전용 개념(스태빌라이저·직교성·축퇴)에 의존하지 않으면 예외 포함(C/E).
0-2. **QKD 정보조정(reconciliation)은 원칙 제외**(채널 ECC 아님). 단, 떼어낼 디코더 양자화·HW 기법이 있으면 예외 포함(C/D).
0-3. **비-LDPC 부호(polar/turbo/RS/BCH)·서베이는 원칙 제외.** 단, 비-LDPC는 디코더 기법이 부호 비의존적으로 LDPC BP에 이식되면, 서베이는 구체적 신규 기여·정량 비교를 담으면 예외 포함.
...
3. 둘 다 아니면 **제외**.
4. **애매하면 포함**으로 살리되 `reason`에 근거를 남긴다 (Phase 3에서 재검토).
   단 **부호가 바이너리 LDPC도 아니고 NAND/스토리지 직접도 아닌데 기법 이식성만 애매한 경우는 제외**한다(약한 in 억제).
