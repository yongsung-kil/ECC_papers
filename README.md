...
- 사람이 읽는 `papers/.../catalog.md|csv`로 출력되고, GitHub에 백업됩니다.
...
| **2. 1차 선별** | 초록만 보고 **NAND 적용 가능성**을 점수화(통과/제외) | `src/review.py` + `criteria/selection_criteria.md` |
...
# 카탈로그 재생성 (선별 결과 등 DB 변경을 papers/ 에 반영)
...
```
src/        db.py · review.py(선별) · catalog.py(카탈로그 생성)
