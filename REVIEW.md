...
3. **판단 → JSON** — 각 논문을 기준서대로 판단해 `_judgments.json`에 저장:
   ```json
   [{"id":"...","score":0.0,"decision":"in|out","reason":"한 줄 근거"}]
   ```
   - 통과(score ≥ 0.40): NAND/스토리지 직접(A/B), 이식 가능 디코더·HW·코드설계(C/D/E)
   - 제외(score < 0.40): 양자 LDPC·QKD·보안, **소스코딩(압축·양자화)**, 무선부수 언급, 순수이론
   - 애매하면 약한 통과(0.40)로 살리고 `reason`에 근거.
