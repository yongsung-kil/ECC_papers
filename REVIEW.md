...
- **판단 규칙**: `criteria/stage1/selection_criteria.md` ← 각 sub-agent가 읽음
- **판단 실행**: `src/stage1/select_workflow.js`의 sub-agent (각자 자기 파일만 읽고 in/out)
- **DB 입출력**: `src/stage1/review.py` (batch=분할, apply=기록, stats=현황)
...
1. **배치 분할** — 메인은 초록을 읽지 않는다(파일로만 분할):
   ```bash
   PYTHONIOENCODING=utf-8 python -m src.stage1.review batch -n 100 --per 10 [--year 2017] [--tag A]
   ```
...
2. **병렬 판정** (확인 없이 바로 Workflow 실행):
   ```
   Workflow({ scriptPath: "src/stage1/select_workflow.js",
              args: { dir: "<RUN_DIR>", nAgents: N, criteriaFile: "criteria/stage1/selection_criteria.md" } })
   ```
...
3. **검증 + DB 기록**:
   - 먼저 `judgments`를 **`<RUN_DIR>/judgments.json`** 에 저장한 뒤 **무결성 검증**:
     ```bash
     PYTHONIOENCODING=utf-8 python -m src.stage1.review verify <RUN_DIR>
     ```
     (DB없는 id·배치밖 id·decision오류·중복을 검사. ❌ 하드에러면 **apply 금지**하고 사용자에게 보고.)
   - 검증 통과(✅)면 판정 결과(in/out 수, 샘플)를 **사용자에게 보고하고 기록 여부를 확인받는다.**
   - 승인되면:
     ```bash
     PYTHONIOENCODING=utf-8 python -m src.stage1.review apply <RUN_DIR>/judgments.json
     ```
...
5. **진행맵 갱신**:
   ```bash
   PYTHONIOENCODING=utf-8 python -m src.report.progress
   ```
