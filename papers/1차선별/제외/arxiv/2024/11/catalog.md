...
- **Status**: ❌
- **Reason**: 실수(real-number) LDPC 부호 대상 GDSU 디코더 — 바이너리 LDPC ECC 아님(criteria 비이진/바이너리 한정), NAND 하드정보 ECC로 이식 불가
...
- **Status**: ❌
- **Reason**: 양자 코드 논리 게이트(코호몰로지·대칭) — 양자 전용, 고전 바이너리 디코더로 이식 불가
...
- **Status**: ❌
- **Reason**: qLDPC single-shot 논리 상태 준비, 양자 FT — 양자 전용 개념 의존, 이식 기법 없음
...
- **Status**: ❌
- **Reason**: Quantum LDPC list decoding — 양자 전용 구성/디코더, 바이너리 NAND ECC 무관
...
- **Status**: ❌
- **Reason**: CV-QKD reverse reconciliation, 표준 rate-1/2 바이너리 LDPC를 베이스라인으로만 사용 — 새 디코더/코드/HW 기여 없음
...
- **Status**: ❌
- **Reason**: qLDPC 기반 FTQC space/time overhead 증명 — 양자 전용, 이식 기법 없음
...
- **Status**: ❌
- **Reason**: Quantum LDPC 거의 선형 거리 구성(homological product) — 양자 전용 구성, 바이너리 NAND 무관
...
- **Status**: ❌
- **Reason**: Chamon 양자 코드용 matching+BP 디코더 — 양자 신드롬 구조 의존, 고전 이식 불가
...
- **Status**: ❌
- **Reason**: LDPC 스태빌라이저 코드의 gapped 양자 위상 안정성 — 양자물리 이론, ECC 기법 없음
...
- **Status**: ❌
- **Reason**: concatenated/qLDPC magic state 자원 추정 도구 — 양자 자원 추정, 디코더/HW/구성 기여 없음
...
- **Status**: ❌
- **Reason**: 5G NR LDPC IP코어를 그대로 갖다 쓴 W2A-OWC FPGA 시스템 — 떼어낼 새 디코더/구성/HW 기여 없음, 표준 부호 재사용
...
- **Status**: ❌
- **Reason**: RIS 보조 다중사용자 시스템의 IDD 응용 — LDPC는 베이스라인, 무선 응용 특이적이며 이식 가능한 새 ECC 기법 없음
...
- **Status**: ❌
- **Reason**: 양자 LDPC를 물질의 안정 상으로 보는 순수 이론 — 양자 EC, 디코더/HW/구성 기여 없음, 양자 전용 개념 의존
