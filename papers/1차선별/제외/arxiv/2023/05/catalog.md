...
- **Status**: ❌
- **Reason**: 양자 LDPC용 decoupling 표현 기반 BP. symplectic/Pauli Y오류 등 양자 전용 개념 의존, 고전 바이너리 LDPC 이식 불가 → 제외
...
- **Status**: ❌
- **Reason**: CV-QKD non-Gaussian reconciliation; MET-LDPC layered BP는 표준 기법 재사용, 양자 QKD 응용 특이적, 떼어낼 새 고전 ECC 기여 없음
...
- **Status**: ❌
- **Reason**: 비이진(NB) LDPC 코드 설계가 핵심 기여 — 비이진 LDPC는 제외 대상
...
- **Status**: ❌
- **Reason**: 양자 locally testable codes의 distance balancing soundness 이론 — 양자 EC, 고전 바이너리 LDPC로 이식 가능한 디코더/구성 없음
