...
- **Status**: ❌
- **Reason**: 양자 LDPC 메모리를 surface code와 concatenate한 hierarchical code — 양자 fault-tolerance 전용, 고전 바이너리 LDPC 이식 기여 없음
...
- **Status**: ❌
- **Reason**: 5G MIMO QAM 기하 성형(constellation shaping)이 핵심; LDPC는 realistic 베이스라인 부수 언급, 떼어낼 ECC 기법 없음
...
- **Status**: ❌
- **Reason**: 5G probability shaping(MCS 개선)이 핵심; LDPC는 베이스라인 부수 언급, 새 디코더/구성/HW 없음
