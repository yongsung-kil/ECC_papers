...
- **Status**: ❌
- **Reason**: LDPC 기반 대수적 동역학계의 엔트로피 순수이론, 디코더/HW/구성 없음
- **ID**: arxiv:2312.17387v3
- **Type**: preprint
- **Published**: 2023-12-28
- **Authors**: Tim Austin, Lewis Bowen, Christopher Shriver
- **PDF**: https://arxiv.org/pdf/2312.17387v3
- **Abstract**: We construct an explicit algebraic example of a subshift of finite type over a group $Γ$ with an invariant Markov measure which has completely positive sofic entropy (with respect to `most' sofic approximations) and yet does not have a direct Bernoulli factor, because its model spaces shatter into exponentially many clusters of sub-exponential size. The example and its analysis are related to random low-density parity-check (LDPC) codes.

## The linear time encoding scheme fails to encode

- **Status**: ❌
- **Reason**: 기존 선형시간 인코딩 논문의 오류 지적(반례), 새 인코더/디코더/구성 기여 없음
- **ID**: arxiv:2312.16125v1
- **Type**: preprint
- **Published**: 2023-12-26
- **Authors**: Yotam Dikstein, Irit Dinur, Shiri Sivan
- **PDF**: https://arxiv.org/pdf/2312.16125v1
- **Abstract**: We point out an error in the paper "Linear Time Encoding of LDPC Codes" (by Jin Lu and José M. F. Moura, IEEE Trans). The paper claims to present a linear time encoding algorithm for every LDPC code. We present a family of counterexamples, and point out where the analysis fails. The algorithm in the aforementioned paper fails to encode our counterexample, let alone in linear time.

## Outer Channel of DNA-Based Data Storage: Capacity and Efficient Coding Schemes

- **Status**: ❌
- **Reason**: DNA 저장 외부채널 — 비이진 random linear code 기반이고 LDPC는 검증용 베이스라인, 이식할 바이너리 LDPC 기여 없음
- **ID**: arxiv:2312.11854v2
- **Type**: preprint
- **Published**: 2023-12-19
- **Authors**: Xuan He, Yi Ding, Kui Cai +3
- **PDF**: https://arxiv.org/pdf/2312.11854v2
...
- **Status**: ❌
- **Reason**: 양자 fracton/product code 연구, 양자 전용 개념 의존 — 고전 바이너리 LDPC로 이식 불가
- **ID**: arxiv:2312.08462v2
- **Type**: preprint
- **Published**: 2023-12-13
- **Authors**: Yi Tan, Brenden Roberts, Nathanan Tantivasadakarn +2
- **PDF**: https://arxiv.org/pdf/2312.08462v2
