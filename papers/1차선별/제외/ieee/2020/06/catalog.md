...
- **Status**: ❌
- **Reason**: 5G AI 기술 개관 논문; LDPC ECC로 떼어낼 구체적 새 디코더/구성 기법 없음
- **ID**: ieee:9108219
- **Type**: journal
- **Published**: June 2020
- **Authors**: Chuan Zhang, Yeong-Luh Ueng, Christoph Studer +1
- **PDF**: https://ieeexplore.ieee.org/document/9108219
...
- **Status**: ❌
- **Reason**: 5G NOMA 방송/유니캐스트 융합 PHY/MAC; LDPC 무관, ECC 기법 없음
- **ID**: ieee:9067029
- **Type**: journal
- **Published**: June 2020
- **Authors**: Eneko Iradier, Jon Montalban, Lorenzo Fanari +4
- **PDF**: https://ieeexplore.ieee.org/document/9067029
...
- **Status**: ❌
- **Reason**: device-edge 협업추론용 JSCC 특징압축으로, 채널코딩 ECC가 아님
- **ID**: ieee:9145068
- **Type**: conference
- **Published**: 7-11 June 
- **Authors**: Jiawei Shao, Jun Zhang
- **PDF**: https://ieeexplore.ieee.org/document/9145068
...
- **Status**: ❌
- **Reason**: 5G NR 시스템레벨 PHY abstraction(EESM/BLER LUT) — ECC 디코더/구성 기법 없음, 무선 시뮬레이션
- **ID**: ieee:9149444
- **Type**: conference
- **Published**: 7-11 June 
- **Authors**: Sandra Lagen, Kevin Wanuga, Hussain Elkotby +3
- **PDF**: https://ieeexplore.ieee.org/document/9149444
...
- **Status**: ❌
- **Reason**: SCMA 다차원 성상 설계로 LDPC는 5G 베이스라인 부수 언급, 떼어낼 ECC 기법 없음
- **ID**: ieee:9145403
- **Type**: conference
- **Published**: 7-11 June 
- **Authors**: Monirosharieh Vameghestahbanati, Ian Marsland, Ramy H. Gohary +1
- **PDF**: https://ieeexplore.ieee.org/document/9145403
- **Abstract**: The design of multidimensional constellations (MdCs) is an integral part of the construction of a sparse code multiple access (SCMA) system. Since the optimal maximum likelihood (ML) receiver for SCMA is too complex in most applications, one highly popular detection technique is the message passing algorithm (MPA), which exploits the sparsity structure of SCMA. MPA is a near-optimal technique, where its performance improves with increasing the signal-to-noise ratio (SNR) and the number of iterations. When the number of MPA iterations has to be limited, the performance gap between ML and MPA becomes significant, specially at low-to-medium SNRs. In this paper, we design novel SCMA MdCs based on optimized unitary rotations on hypercubes. The proposed MdCs provide substantial performance gains in comparison to the best known SCMA MdCs in the literature, specially in low-to-medium SNR regions when the number of MPA iterations has to be low, and in the presence of 5G-compliant LDPC codes.

## Performance Analysis of LDPC Coded Massive MIMO-OFDM System

- **Status**: ❌
- **Reason**: LDPC 코딩 massive MIMO-OFDM 성능 분석, LDPC는 표준 베이스라인·새 기법 없음
- **ID**: ieee:9154160
- **Type**: conference
- **Published**: 5-7 June 2
- **Authors**: Aravinda Babu Tummala, Deergha Rao Korrai
- **PDF**: https://ieeexplore.ieee.org/document/9154160
- **Abstract**: Multiple Input Multiple Output Orthogonal Frequency Division Multiplexing (MIMO OFDM) wireless communication is well known in the literature. However, the problems occur in classical MIMO system can be overcome with large number of array antennas such systems, termed as Massive MIMO. But, the latency may be more for these systems using traditional equalizers such as Zero Forcing (ZF) and Minimum Mean Square Error (MMSE). Hence, this paper proposes LDPC coded Massive MIMO OFDM system using Approximate Message Passing (AMP) equalizer. The performance of the proposed system is analysed through simulations. In this simulation, different transmit and receive antennas (64,128), (64,256), (64,512) and (64, 1024) and 16QAM are used. Finally, the performance of LDPC coded and uncoded massive MIMO OFDM using AMP equalizer is analyzed in comparison with ZF and MMSE equalizers using BER and latency as performance measures.

## Performance Analysis of LDPC in Ultraviolet Communication System

- **Status**: ❌
- **Reason**: 자외선 통신 LDPC 적용 BER 성능평가. 표준 LDPC 단순 적용, 신규 디코더·구성 없음
- **ID**: ieee:9182468
- **Type**: conference
- **Published**: 27-29 June
- **Authors**: Yiwei Peng, Dong Zhou, Feiyue Yang
- **PDF**: https://ieeexplore.ieee.org/document/9182468
- **Abstract**: In this paper Low-Density Parity-Check (LDPC) codes are applied in ultraviolet communication. We investigate the performance of LDPC in ultraviolet (UV) communications system in terms of the bit-error-ratio (BER) for different code length and different decoding algorithm. The performance of the LDPC UV system is demonstrated with simulation and experimental results.

## BER Comparison of Coding Schemes in LowPower Regime

- **Status**: ❌
- **Reason**: Turbo/LDPC/Raptor BER 비교(저전력). 표준 부호 단순 비교, 신규 디코더·구성 없음
- **ID**: ieee:9152836
- **Type**: conference
- **Published**: 26-28 June
- **Authors**: Ikhlas M. Farhan, Dhafer R. Zaghar, Hadeel N. Abdullah
- **PDF**: https://ieeexplore.ieee.org/document/9152836
- **Abstract**: This paper investigates the application of advanced forward error correction techniques mainly: Turbo code, Low-Density Parity Checks (LDPC) code, and Raptor code for wireless communication systems in the low power regime (LPR). These codes are under consideration for many recent technologies. Different code parameters such as code rate and several decoding iterations are used to show their effect on the performance of digital systems. Simulation results showed that the Raptor code is performed better than Turbo code and LDPC code over binary input additive white Gaussian noise (BI_AWGN), for the same coding rate and the number of decoding iterations. Considering bit error rate (BER) performance, Raptor code with rate1/2 and 1/3 provided better performance than Turbo and LDPC for the SNR up to -5dB for the AWGN channel. Finally, in comparison with Turbo and LDPC, Raptor code shows better performance for a large number of decoding iterations and low coding rates.

## Asymptotic Absorbing Set Enumerators for Non-Binary Protograph-Based LDPC Code Ensembles

- **Status**: ❌
- **Reason**: Non-Binary protograph LDPC의 absorbing set enumerator — 비이진 LDPC 대상이라 제외
- **ID**: ieee:9174036
- **Type**: conference
- **Published**: 21-26 June
- **Authors**: Emna Ben Yacoub, Gianluigi Liva
- **PDF**: https://ieeexplore.ieee.org/document/9174036
- **Abstract**: The finite-length absorbing set enumerators for non-binary protograph based low-density parity-check (LDPC) ensembles are derived. An efficient method for the evaluation of the asymptotic absorbing set distributions is presented and evaluated.

## A Low Complexity Decoding Algorithm for NB-LDPC Codes over Quadratic Extension Fields

- **Status**: ❌
- **Reason**: NB-LDPC(non-binary GF) 디코더 — 비이진 LDPC, 기준상 제외
- **ID**: ieee:9174489
- **Type**: conference
- **Published**: 21-26 June
- **Authors**: V. B. Wijekoon, Emanuele Viterbo, Yi Hong
- **PDF**: https://ieeexplore.ieee.org/document/9174489
...
- **Status**: ❌
- **Reason**: non-binary LDPC convolutional codes 구성 — 비이진 LDPC, 기준상 제외
- **ID**: ieee:9174510
- **Type**: conference
- **Published**: 21-26 June
- **Authors**: Gianira Nicoletta Alfarano, Julia Lieb, Joachim Rosenthal
- **PDF**: https://ieeexplore.ieee.org/document/9174510
...
- **Status**: ❌
- **Reason**: 유한블록길이/오류지수 순수 이론 bound, 디코더·HW·구성으로 이어지지 않음
- **ID**: ieee:9173963
- **Type**: conference
- **Published**: 21-26 June
- **Authors**: Yuxin Liu, Michelle Effros
- **PDF**: https://ieeexplore.ieee.org/document/9173963
...
- **Status**: ❌
- **Reason**: Polar 코드 기반 unsourced random access 기법, LDPC는 베이스라인, 떼어낼 바이너리 LDPC BP 기법 없음
- **ID**: ieee:9174247
- **Type**: conference
- **Published**: 21-26 June
- **Authors**: Kirill Andreev, Evgeny Marshakov, Alexey Frolov
- **PDF**: https://ieeexplore.ieee.org/document/9174247
...
- **Status**: ❌
- **Reason**: polar-coded modulation 프레임워크 — 비-LDPC, 떼어낼 LDPC BP 디코더 기법 없음
- **ID**: ieee:9174433
- **Type**: conference
- **Published**: 21-26 June
- **Authors**: Jincheng Dai, Kai Niu, Zhongwei Si
- **PDF**: https://ieeexplore.ieee.org/document/9174433
...
## Approximate 3D-TLC NAND Flash Write with Initial Error Injection for Application-level Reliability Improvement of Machine Learning-based Computing

- **Status**: ❌
- **Reason**: 3D-TLC NAND이지만 ECC 불필요 가정의 근사 오류주입(ML 내성) — LDPC ECC 기법 아님
- **ID**: ieee:9131417
- **Type**: conference
- **Published**: 13-14 June
- **Authors**: Shun Suzuki, Hiroki Aihara, Keita Mizushina +2
- **PDF**: https://ieeexplore.ieee.org/document/9131417
- **Abstract**: In conventional SSD controller, error correcting code (ECC) is required to correct errors in 3D-TLC NAND flash memories. However, in machine learning (ML)-based computing, some errors can be tolerated and ECC is not necessary. By utilizing the error tolerance, this paper proposes Approximate Initial Error Injection (AIEI) which initially injects errors to memory cells to improve the application-level reliability after the data-retention (D.R.). AIEI initially injects 16.7% of errors to 3D-TLCNAND flash cells. Then, after the D.R., measured bit-error rate (BER) decreases by 34%. Measured acceptable D.R. time is extended by 4.9-times.

## The Proposed Simplified Iterative Decoding for 2D-ISI Magnetic Recording Channels

- **Status**: ❌
- **Reason**: 2D-ISI 자기기록용 비이진(non-binary) LDPC의 EMS 디코딩 — 비이진 LDPC라 제외
- **ID**: ieee:9141740
- **Type**: conference
- **Published**: 12-14 June
- **Authors**: Jiayue Chen, Feng Wang, Pingping Chen +1
- **PDF**: https://ieeexplore.ieee.org/document/9141740
- **Abstract**: 2D inter-symbol-interference (2D-ISI) channel models ultra-high-density magnetic recording systems, which suffer from ISI in the down track direction and inter-tack interference along the cross-track direction. These interferences seriously degrade the performance of the magnetic recording systems. Low-density parity check (LDPC) codes was used in storage systems However, although non-binary LDPC codes with log-BP decoding was designed for 2D-ISI channels, it has a drawback of a high decoding complexity. To address this issue, this paper proposes a simplified iterative decoding for 2D-ISI channels. In specific, we use extended min-sum (EMS) decoding with truncation of messages to simplify computation and optimize scale factor in outer iteration by the EXIT analysis to improve the performance. Finally, the simulated results show that the proposed scheme with reduced complexity for 2D-ISI system can maintain an approximate performance to traditional one. Even at high SNR regions, the proposed scheme will be better than previously decoding algorithm.

## Key Physical Layer Technologies of Broadband Ad Hoc Network in Low-altitude

- **Status**: ❌
- **Reason**: 저고도 ad hoc 망 물리계층 설계, LDPC는 OFDM과 조합 부수 언급 — 떼어낼 신규 디코더/HW/코드설계 기법 없음
- **ID**: ieee:9084769
- **Type**: conference
- **Published**: 12-14 June
- **Authors**: Wang Zhenyi, Zouli, Liao Feifan
- **PDF**: https://ieeexplore.ieee.org/document/9084769
- **Abstract**: Ad hoc network technology is more and more used in tactical communication and emergency communication systems due to its maneuverability, flexibility, high efficiency, convenience, and rapid coverage of the support area. It can rely on low-altitude small drones as carriers to carry mobile communication repeaters to quickly form a regional low-altitude broadband ad hoc network. This paper analyzes the application scenarios of low-altitude broadband ad hoc networks, and optimizes the physical layer parameters of 802.11 series wireless communications for low-altitude channels, and uses low-density parity-check (LDPC) in combination with Orthogonal Frequency Division Multiplexing(OFDM), Coded Orthogonal Frequency Division Multiplexing (COFDM) technology based on software, proposes a physical layer architecture of low-altitude wideband ad hoc network based on software radio, and gives specific parameters under 5MHz bandwidth. Through simulation experiments and hardware Verify that the physical layer architecture works well.

## Performance of Polar Codes in Cooperative Transmission Systems

- **Status**: ❌
- **Reason**: polar 코드 SCL 협력전송, LDPC는 비교 베이스라인 언급뿐 — 비-LDPC, 이식할 BP 기법 없음
- **ID**: ieee:9179453
- **Type**: conference
- **Published**: 12-13 June
- **Authors**: Dame Dia, Mamadou Diallo Diouf, Idy Diop
- **PDF**: https://ieeexplore.ieee.org/document/9179453
- **Abstract**: The incessant increase in service needs and the number of subscribers is a major subject of concern to operators of mobile communication networks. Cooperative systems are one of the solutions proposed by the communities to meet these needs. In this proposal, we focus on the successive cancellation list (SCL) decoder for polar codes on cooperative communication which is composed of three blocks: the transmission module, a relay block and a reception node. The transmission on the channel is divided into two times. Data coded with a polar code is transmitted simultaneously by the source to the reception system and to the relay bloc. The relay processes its version with an SCL protocol before proceeding to polar coding then transmits them to the destination. After a SCL decoding, the reception uses the MRC (Maximum Ratio Combining) combination method to compare the two versions of the signals and extracts the useful information. We propose a protocol for the construction of polar codes to reach the theoretical capacity in the half-duplex relay channel. We also demonstrate that the polar code (1024, 512) is decoded by the decoder. SC-List of the proposed model with a very large size of the list (L=32) can reach a bit rate of 10-4 to 2dB. The simulation results obtained show proven performance compared to other existing codes such as the turbo code and the low-density parity check) codes.

## Bit Stream Multiplexing in 5G networks

- **Status**: ❌
- **Reason**: 5G eMBB/URLLC 멀티플렉싱+TCM, LDPC는 부수 언급. 떼어낼 디코더/HW/코드설계 기여 없음 — 무선응용 특이적
- **ID**: ieee:9131511
- **Type**: conference
- **Published**: 1-5 June 2
- **Authors**: I. A. Pastushok, N. A. Boikov, N. A. Yankovskii
- **PDF**: https://ieeexplore.ieee.org/document/9131511
- **Abstract**: This paper is a continuation of the research on enhanced mobile broadband (eMBB) and ultra-low latency (URLLC) streaming multiplexing techniques in the fifthgeneration wireless downlink. Based on a previous research, the following approach to multiplexing eMBB and URLLC data streams based on Trellis-Coded Modulation (TCM) using LDPC codes for fifth generation networks in hard decision mode is proposed. We are considered several options for TCM settings and capabilities, their impact on the quality of data transmission and the performance of the proposed transmission method.

## About Using Matched Metrics in Channels with Memory for Gilbert Codes

- **Status**: ❌
- **Reason**: Gilbert 코드(비-LDPC)와 메모리 채널 매칭 메트릭. LDPC 이식 기법 없음
- **ID**: ieee:9131549
- **Type**: conference
- **Published**: 1-5 June 2
- **Authors**: A. M. Veresova, A. A. Ovchinnikov
- **PDF**: https://ieeexplore.ieee.org/document/9131549
- **Abstract**: The main concepts of the metric description of memory channels are considered, as well as a specific metric for such channels. The spectral properties of the Gilbert code in relation to the Markov metric are analyzed. It is shown that using a matched metric increases the efficiency of channel usage.
