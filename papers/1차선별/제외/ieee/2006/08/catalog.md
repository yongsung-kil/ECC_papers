...
- **Status**: ❌
- **Reason**: q-regular 이분그래프 기반 LDPC 차원 증명, 순수 이론(차원 추측 증명)으로 디코더/HW/구성 이식 없음
- **ID**: ieee:1661850
- **Type**: journal
- **Published**: Aug. 2006
- **Authors**: P. Sin, Q. Xiang
- **PDF**: https://ieeexplore.ieee.org/document/1661850
- **Abstract**: An explicit construction of a family of binary low-density parity check (LDPC) codes called$hbox LU(3,q)$, where$q$is a power of a prime, was recently given. A conjecture was made for the dimensions of these codes when$q$is odd. The conjecture is proved in this note. The proof involves the geometry of a four-dimensional (4-D) symplectic vector space and the action of the symplectic group and its subgroups.

## Product code optimization for determinate state LDPC decoding in robust image transmission

- **Status**: ❌
- **Reason**: LDPC+RS product code 영상전송, determinate symbol 활용은 응용특이적이고 RS 결합이라 NAND 이식성 약함
- **ID**: ieee:1658078
- **Type**: journal
- **Published**: Aug. 2006
- **Authors**: N. Thomos, N.V. Boulgouris, M.G. Strintzis
- **PDF**: https://ieeexplore.ieee.org/document/1658078
...
- **Status**: ❌
- **Reason**: Slepian-Wolf 분산 소스 압축, LDPC를 syndrome-former로 사용 — 소스코딩이라 채널 ECC 아님
- **ID**: ieee:1661834
- **Type**: journal
- **Published**: Aug. 2006
- **Authors**: T.P. Coleman, A.H. Lee, M. Medard +1
- **PDF**: https://ieeexplore.ieee.org/document/1661834
...
- **Status**: ❌
- **Reason**: 100Gb/s OFDM 광전송 응용, LDPC는 부수 언급뿐 떼어낼 디코더/구성 기법 없음
- **ID**: ieee:1652957
- **Type**: journal
- **Published**: Aug. 2006
- **Authors**: I.B. Djordjevic, B. Vasic
- **PDF**: https://ieeexplore.ieee.org/document/1652957
...
- **Status**: ❌
- **Reason**: 차분 turbo 변조+APP 채널추정, 비-LDPC(turbo/8PSK)이고 떼어낼 LDPC 기법 없음
- **ID**: ieee:1673672
- **Type**: journal
- **Published**: Aug. 2006
- **Authors**: S.L. Howard, C. Schlegel
- **PDF**: https://ieeexplore.ieee.org/document/1673672
...
- **Status**: ❌
- **Reason**: 채널코딩 error exponent와 신호성좌 설계, LDPC 무관 순수 이론 bound
- **ID**: ieee:1665017
- **Type**: journal
- **Published**: Aug. 2006
- **Authors**: Jianyi Huang, S. Meyn, M. Medard
- **PDF**: https://ieeexplore.ieee.org/document/1665017
- **Abstract**: This paper concerns error exponents and the structure of input distributions maximizing the random coding exponent for a stochastic channel model. The following conclusions are obtained under general assumptions on the channel statistics. 1) The optimal distribution has a finite number of mass points, or in the case of a complex channel, the amplitude has finite support. 2) A new class of algorithms is introduced based on the cutting-plane method to construct an optimal input distribution. The algorithm constructs a sequence of discrete distributions, along with upper and lower bounds on the random coding exponent at each iteration. 3) In some numerical example considered, the resulting code significantly outperforms traditional signal constellation schemes such as quadrature amplitude modulation and phase-shift keying for all rates below the capacity.

## Design, Simulation and Hardware Implementation of a Digital Television System: Performance Evaluation

- **Status**: ❌
- **Reason**: DTV 변조 시스템 채널모델 기반 성능평가만; 신규 디코더/구성/HW 기여 없음, 떼어낼 LDPC 기법 없음
- **ID**: ieee:4100554
- **Type**: conference
- **Published**: 28-31 Aug.
- **Authors**: Fabricio C. A. Oliveira, Dayan A. Guimaraes, Geraldo G. R. Gomes +3
- **PDF**: https://ieeexplore.ieee.org/document/4100554
- **Abstract**: In this paper we present the tests and performance evaluations carried out on the modulation system for the terrestrial digital television system (MI-SBTVD). All the results were obtained using computer simulated models, portraying relevant operating scenarios based on well-established channel models. The results obtained in this work have allowed us to estimate the expected system performance on working conditions as different as, for example, reception through single, fixed external antenna and single-antenna mobile reception

## Image Communications Based on Adaptive Segmentation and WIPC-LDPC over Wireless Channels

- **Status**: ❌
- **Reason**: 이미지 전송 JSCC + 불균등보호용 WIPC-LDPC; JSCC 응용 특이적이고 LDPC가 베이스라인, 떼어낼 ECC 기법 없음
- **ID**: ieee:4097894
- **Type**: conference
- **Published**: 20-23 Aug.
- **Authors**: Rui Guo, Ji-lin Liu
- **PDF**: https://ieeexplore.ieee.org/document/4097894
- **Abstract**: In order to improve error resilient ability and transmission efficiency for image transmission over wireless channels, a joint source channel coding (JSCC) scheme was proposed in this paper, in which adaptive image segmentation and irregular low-density parity check (LDPC) was fitted together. At first, an adaptive image segmentation method according to the pixel intensity was designed to segment an image into different region of interest (ROI) with different level of significance. Then, a novel irregular LDPC with weight-increasing parity-check (WIPC) matrix (WIPC-LDPC) was put forward, which can easily realize unequal error protection (UEP) scheme. At last, the adaptive segmentation and WIPC-LDPC coding were combined together to achieve JSCC: the different significance level ROI from the adaptive segmentation were coded by WIPC-LDPC with dissimilar degree bit nodes to achieve different level of protection. Simulation results show that the proposed scheme can support robust image transmission with not only high objective PSNR but also excellent subjective visual quality

## Multiple Hybrid Concatenated Single Parity Check Codes

- **Status**: ❌
- **Reason**: Single Parity Check 연접부호 구조 비교(비-LDPC); BER bound 분석으로 바이너리 LDPC BP에 이식할 디코더 기법 없음
- **ID**: ieee:4097851
- **Type**: conference
- **Published**: 20-23 Aug.
- **Authors**: Jiaxuan Yang, Chuanying Jia
- **PDF**: https://ieeexplore.ieee.org/document/4097851
- **Abstract**: Concatenated codes have not only the generally known serial and parallel concatenated coding structures, but also the uncommon hybrid concatenated coding structure. Utilizing SPC codes as constituent codes, this paper considers the three coding structures in a uniform way. A uniform description of the three coding structures is illustrated and a kind of generalized iterative decoding structure suitable for the three coding structures is also presented. In addition, the comparison of upper bounds to the bit-error-probability for multiple concatenated SPC codes is performed with the three coding structures. The bounds and simulation results show that different coding structures have different but regular performance variation: (1) the performance of various hybrid concatenated SPC codes is always lying in between the performance of parallel and serial concatenated SPC codes; (2) when code rate increases, the performance crossover between parallel and serial concatenated SPC codes moves to lower bit-error rate (BER) region and their performance difference decreases.
