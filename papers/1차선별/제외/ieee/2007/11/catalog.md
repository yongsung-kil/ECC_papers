...
- **Status**: ❌
- **Reason**: regular LDPC ensemble의 error rate 하한/error exponent 신뢰구간; 순수 이론 bound로 디코더/HW/구성으로 이어지지 않음
- **ID**: ieee:4373388
- **Type**: journal
- **Published**: Nov. 2007
- **Authors**: Ohad Barak, David Burshtein
- **PDF**: https://ieeexplore.ieee.org/document/4373388
...
- **Status**: ❌
- **Reason**: 아날로그 정보원용 ECC, 카오스 동역학 기반 코드. LDPC는 비교 대상일 뿐 NAND 바이너리 LDPC에 이식할 디코더/HW/구성 신규 기법 없음
- **ID**: ieee:4383303
- **Type**: journal
- **Published**: Nov. 2007
- **Authors**: Isaac Rosenhouse, Anthony J. Weiss
- **PDF**: https://ieeexplore.ieee.org/document/4383303
...
## Space Time Serial Concatenated Turbo Coded Modulation

- **Status**: ❌
- **Reason**: Space-Time Turbo coded modulation - 비-LDPC(turbo) 무선 변조, 떼어낼 LDPC BP 기법 없음
- **ID**: ieee:4487374
- **Type**: conference
- **Published**: 4-7 Nov. 2
- **Authors**: Daniel N. Liu, Michael P. Fitz
- **PDF**: https://ieeexplore.ieee.org/document/4487374
- **Abstract**: Space-Time trellis-coded modulation (ST-TCM) which combines modulation and coding in a single entity delivers exceptional performance in static enviroment such as AWGN and flat fading channels. On the other hand, bit- interleaved coded modulation (BICM) which gives up the joint decoder/demodulator architecture in favor of two separate entities outperforms ST-TCM in selective fading channels, but suffers performance loss in static channels. This paper proposes a space-time serial concatenated coding schemes which consist of two ";simple"; constituent encoders and an interleaver namely: ST serial concatenated Turbo-coded modulation (ST-SCTCM). Both theory and simulation results demonstrate that ST-SCTCM outperforms either ST-TCM or BICM across different channel conditions.

## A soft multiuser demodulation for non-binary LDPC coded FFH MA/MFSK systems

- **Status**: ❌
- **Reason**: non-binary LDPC 기반 다중사용자 복조 — 비이진 LDPC라 제외
- **ID**: ieee:4429157
- **Type**: conference
- **Published**: 30 Oct.-2 
- **Authors**: Szu-Lin Su, Her-Chang Tsai, Ye-Shun Shen
- **PDF**: https://ieeexplore.ieee.org/document/4429157
...
- **Status**: ❌
- **Reason**: RS+LDPC concatenation의 SFR/버스트오류 성능평가만; 새 디코더/구성/HW 없이 표준 부호 조합 평가
- **ID**: ieee:4437835
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Sundararajan Sankaranarayanan, Alexander Kuznetsov, Deepak Sridhara
- **PDF**: https://ieeexplore.ieee.org/document/4437835
- **Abstract**: In this paper, we investigate the sector failure rate (SFR) and the burst-error correction capability of two coding solutions in magnetic storage systems. The two-level solution involves concatenating an outer RS code with an inner LDPC code whereas the one-level solution is an RS-only or an LDPC-only system. Although the outer RS code incurs an additional rate loss, it is expected to gain in the presence of burst errors. The rate of the LDPC code and the i-level of the RS code can be chosen optimally to suit the operating condition of the channel. These solutions have also been studied in various technical papers in literature. Previously mentioned performance measures are used to evaluate strengths and weaknesses of these two solutions.

## Outage Probability of LDPC-Coded Systems

- **Status**: ❌
- **Reason**: fading 채널 outage probability 이론분석, 디코더/HW/구성으로 안 이어지는 순수 성능지표
- **ID**: ieee:4411528
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Jinho Kim, Wayne E. Stark
- **PDF**: https://ieeexplore.ieee.org/document/4411528
- **Abstract**: Outage probability is an important performance measure of wireless communication systems operating in a fading environment. In this paper, we study the outage probability of low-density parity-check (LDPC) coded systems for slow fading channels (nonergodic channels) under the assumption that the transmitter knows only the channel statistics while the receiver has perfect or imperfect channel side information (CSI). The fading coefficient is assumed to be random but stays constant over a LDPC codeword. We define the outage event as the set of channel realizations which disables successful decoding. Based on a density evolution algorithm and stability condition, closed- form outage probabilities are derived for both Rayleigh and Rician fading channels. The effect of channel estimation error on the outage probability is also investigated assuming Gaussian estimation error.

## Ensemble Enumerators for Protograph-Based Generalized LDPC Codes

- **Status**: ❌
- **Reason**: protograph G-LDPC 앙상블 weight/stopping set enumerator - 순수 이론 거리분석, 디코더/HW/구성 신규 없음
- **ID**: ieee:4411197
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Shadi Abu-Surra, William E. Ryan, Dariush Divsalar
- **PDF**: https://ieeexplore.ieee.org/document/4411197
...
- **Status**: ❌
- **Reason**: dense parity check 기반 무선 비밀공유(보안), 채널 ECC 아니고 이식 가능 LDPC 기법 없음
- **ID**: ieee:4410928
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Sheng Xiao, Hossein Pishro-Nik, Weibo Gong
- **PDF**: https://ieeexplore.ieee.org/document/4410928
...
- **Status**: ❌
- **Reason**: CSI 기반 rate-adaptive queued-code(무선 전송 속도적응), 떼어낼 신규 디코더/구성 기법 없음
- **ID**: ieee:4411527
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Satashu Goel, Rohit Negi
- **PDF**: https://ieeexplore.ieee.org/document/4411527
...
- **Status**: ❌
- **Reason**: 자기기록 채널 정보이론 capacity 리뷰 강의; 떼어낼 디코더/HW/코드설계 기법 없는 순수 이론·서베이
- **ID**: ieee:4437833
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Paul H. Siegel
- **PDF**: https://ieeexplore.ieee.org/document/4437833
- **Abstract**: In this lecture, recent advances in the computation of the information-theoretic capacity of magnetic recording channels are reviewed. Capacity estimates are compared to densities achieved using advanced coding and detection methods. Extensions to more realistic channel models, as well as to exploratory storage technologies are also considered.

## LDPC-Coded MIMO Optical Communication Over the Atmospheric Turbulence Channel

- **Status**: ❌
- **Reason**: 광 MIMO 무선응용, pairwise-balanced design 코드설계지만 응용특이적이고 떼어낼 신규 바이너리 LDPC 기법 없음
- **ID**: ieee:4411334
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Ivan B. Djordjevic, Stojan Denic, Jaime Anguita +2
- **PDF**: https://ieeexplore.ieee.org/document/4411334
- **Abstract**: We describe a multiple optical sources - multiple detectors scheme, based on either repetition MIMO or space-time coding and low-density parity-check (LDPC) codes. The proposed scheme is able to operate under strong atmospheric turbulence and provides excellent coding gains. The LDPC codes are designed using the concept of pairwise-balanced designs. Bit-error rates and achievable information rates are reported assuming non-ideal photodetection. To improve the spectral efficiency we employ the concept of bit-interleaved LDPC-coded modulation based on pulse-amplitude modulation.

## List-Message Passing Achieves Capacity on the q-ary Symmetric Channel for Large q

- **Status**: ❌
- **Reason**: q-ary symmetric channel용 list-message-passing 디코더로 비이진(q-ary) 대상, 바이너리 LDPC 아님
- **ID**: ieee:4410970
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Fan Zhang, Henry D. Pfister
- **PDF**: https://ieeexplore.ieee.org/document/4410970
...
- **Status**: ❌
- **Reason**: Raptor 코드 분산소스코딩(DSC), rateless+소스코딩 - 채널 ECC 아님
- **ID**: ieee:4411215
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: M. Fresia, L. Vandendorpe
- **PDF**: https://ieeexplore.ieee.org/document/4411215
...
- **Status**: ❌
- **Reason**: SPC product code의 LP 디코딩, 비-LDPC 부호 특화 - 바이너리 LDPC BP 이식 기법 아님
- **ID**: ieee:4411213
- **Type**: conference
- **Published**: 26-30 Nov.
- **Authors**: Kai Yang, Xiaodong Wang, Jon Feldman
- **PDF**: https://ieeexplore.ieee.org/document/4411213
