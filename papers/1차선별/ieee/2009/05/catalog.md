# IEEE Xplore — 2009-05 (1차선별 통과)


## Large girth quasi-cyclic LDPC codes based on the chinese remainder theorem

- **Status**: ✅
- **Reason**: E: CRT 기반 QC-LDPC 결합으로 girth 유지하며 코드길이 확장하는 신규 구성법. 바이너리 QC-LDPC 코드 설계 기법 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:4939346
- **Type**: journal
- **Published**: May 2009
- **Authors**: Xueqin Jiang, Moon Ho Lee
- **PDF**: https://ieeexplore.ieee.org/document/4939346
...
- **Status**: ✅
- **Reason**: LDPC trapping set/stopping set 전수 탐색 알고리즘, error floor 최적화 — 유한길이 코드설계(E)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:4839029
- **Type**: journal
- **Published**: May 2009
- **Authors**: Chih-Chun Wang, Sanjeev R. Kulkarni, H. Vincent Poor
- **PDF**: https://ieeexplore.ieee.org/document/4839029
...
- **Status**: ✅
- **Reason**: min-sum 메시지패싱 수렴성 이론 — BP/min-sum 디코더(C)에 이식 가능, 애매하나 디코더 기법이라 살림
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:4839028
- **Type**: journal
- **Published**: May 2009
- **Authors**: Ciamac C. Moallemi, Benjamin Van Roy
- **PDF**: https://ieeexplore.ieee.org/document/4839028
- **Abstract**: We establish the convergence of the min-sum message passing algorithm for minimization of a quadratic objective function given a convex decomposition. Our results also apply to the equivalent problem of the convergence of Gaussian belief propagation.

## Construction of irregular structured LDPC codes with low encoding complexity for high-rate applications

- **Status**: ✅
- **Reason**: 신규 비정규 structured LDPC 구성(저복잡도 인코딩, 가변 rate/길이/sparsity) — 코드설계 E 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5137202
- **Type**: conference
- **Published**: 6-9 May 20
- **Authors**: Puripong Suthisopapan, Virasit Imtawil
- **PDF**: https://ieeexplore.ieee.org/document/5137202
- **Abstract**: This paper presents a method to construct irregular structured low-density parity-check (LDPC) codes based on choosing different identity matrices. The parity-check matrix of the proposed code can be easily generated with the flexible parameters such as code rate, block length or sparsity. The proposed code is well structured which can provide a low-complexity implementation. The encoder of the proposed code has linear encoding complexity making it suitable for hardware implementation. When iteratively decoded with the sum-product algorithm (SPA), the constructed irregular LDPC codes exhibit a very good performance especially for high rate codes.

## Research on LDPC codes based on VCDS

- **Status**: ✅
- **Reason**: 감소복잡도 디코더(BP-based/TSP/TDMP) 비교 및 병렬도 조정 가능한 확장형 디코더 HW 아키텍처 제시 — 이식 가능 디코더+HW(C/D)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5138342
- **Type**: conference
- **Published**: 25-27 May 
- **Authors**: Guochun Wan, Yanfen Luo, Decun Dong +1
- **PDF**: https://ieeexplore.ieee.org/document/5138342
- **Abstract**: For the realization of a safe train traffic environment, a wireless communication system which makes an information network between ground and vehicles has been needed. Low-Density Parity-Check (LDPC) codes have been widely considered as error-correcting codes for next generation communication systems. In this paper, Vehicle Condition Detect System (VCDS) channel coding methods were given according to [3].Then some reduced complexity decoding algorithms (BP-based, TSP, TDMP) are compared with the original BP algorithm. We find the optimum algorithms which are improvement in coding gain and suitable for VCDS credible information transmission. In the aspect of the hardware design, We analysis a highly efficient scalable decoder architecture for LDPC codes. This architecture adjusts the parallel processing factor based on the serial frame to increase decode speed.

## Row Division Method to Generate QC-LDPC Codes

- **Status**: ✅
- **Reason**: large girth QC-LDPC 구성 신규 방법(Row Division)으로 girth-12 코드 생성 — E 이식 가능 코드 설계, 응용(AJ)은 부수적
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5072428
- **Type**: conference
- **Published**: 24-28 May 
- **Authors**: Abid Yahya, Othman Sidek, M. F. M. Salleh +1
- **PDF**: https://ieeexplore.ieee.org/document/5072428
- **Abstract**: To cut down the hardware implementation cost this paper presents an efficient method to construct large girth Quasi-Cyclic low density parity check (QC-LDPC) codes. The row groups are paired two times the row weight, which has the complexity as compared to the connection of individual columns and rows. The complexity of directing within groups estimates on the transposition employed to connect rows and columns between groups. The newly obtained codes are incorporated with diversity techniques and are employed as a forward error correction codes to cater for some anti-jamming (AJ) competences in the presence of partial band noise jamming (PBNJ). The robustness of two girth-twelve QC-LDPC codes is evidenced by comparing it with randomly constructed LDPC codes and QC-LDPC codes proposed in [14].

## New LDPC Codes Based on Serial Concatenation

- **Status**: ✅
- **Reason**: 직렬연접 기반 신규 LDPC 부호족 구성(rate-compatible, 저복잡도 인코딩) — E 코드 설계 신규 기여
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5072451
- **Type**: conference
- **Published**: 24-28 May 
- **Authors**: Marco Baldi, Giovanni Cancellieri, Franco Chiaraluce
- **PDF**: https://ieeexplore.ieee.org/document/5072451
...
- **Status**: ✅
- **Reason**: 고병렬 저메모리 LDPC 디코더 HW 설계(D), 802.15 응용이나 아키텍처 이식 가능
- **알고리즘 기여**: 🔧 하드웨어 수정만
- **ID**: ieee:5118294
- **Type**: conference
- **Published**: 24-27 May 
- **Authors**: Jin Sha, Jun Lin, Li Li +2
- **PDF**: https://ieeexplore.ieee.org/document/5118294
- **Abstract**: This paper presents an efficient decoder design for the LDPC codes in IEEE 802.15 standard. This decoder features by high parallel level, low message memory requirement and code rate flexibility. By processing 72 columns and 72 rows in parallel, it can reach a throughput of 2.8 Gbps to fulfill the standard requirement. Furthermore, the decoder supports three different code rates by employing flexible check node processor units.

## A Novel Log-BP Decoding Algorithm for LDPC Codes

- **Status**: ✅
- **Reason**: log-BP 단순화+개선된 layered-iterative 디코딩으로 반복횟수·HW 자원 절감 — 디코더 알고리즘(C) 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5319115
- **Type**: conference
- **Published**: 19-21 May 
- **Authors**: Jun Wang, Shu-Hui Yang
- **PDF**: https://ieeexplore.ieee.org/document/5319115
...
- **Status**: ✅
- **Reason**: 완전 프로그래머블 부분병렬 LDPC 디코더 HW 아키텍처+매핑/스케줄링, 병렬화 기법 이식 가능(D)
- **알고리즘 기여**: 🔧 하드웨어 수정만
- **ID**: ieee:5172542
- **Type**: conference
- **Published**: 17-20 May 
- **Authors**: Christiane Beuschel, Hans-Jorg Pfleiderer
- **PDF**: https://ieeexplore.ieee.org/document/5172542
- **Abstract**: In this article we present a fully programmable and scalable partly-parallel LDPC decoder architecture together with an optimum mapping and scheduling algorithm. The proposed algorithm exploits the full parallelism of the architecture at any time for any code, which means that the mapping algorithm achieves 100% utilization of the architecture. The proposed design is fully programmable and can be reconfigured for a different LDPC code by changing the initialization of the control memory. Thus the architecture can be used for a multi-standard decoder which supports decoding of any structured or unstructured LDPC code. Furthermore, the parallelism of the architecture is unconstrained and fully scalable which allows to exchange hardware cost and throughput with fine granularity. In contrast to previously proposed programmable designs our approach uses parallel variable and check node processing and thus doubles the data throughput.

## A Fast Double Layered Decoding Algorithm for Quasi-Cyclic LDPC Codes

- **Status**: ✅
- **Reason**: QC-LDPC용 double layered 디코딩 알고리즘 제안 - 이식 가능 디코더(C), NAND LDPC에 적용 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5232407
- **Type**: conference
- **Published**: 16-17 May 
- **Authors**: Zhixiong Chen, Jinsha Yuan
- **PDF**: https://ieeexplore.ieee.org/document/5232407
- **Abstract**: Most quasi-cyclic LDPC codes decoders of IEEE 802.11n and IEEE 802.16e standards adopt Layered decoding algorithm recently. A fast double layered decoding algorithm for quasi-cyclic LDPC codes in which two row groupings of check nodes are processed synchronously in order to speed the decoding processing is proposed. Compared to BP algorithm and normal Layered decoding algorithm, the proposed algorithm provides a faster decoding speed and more favorable tradeoffs among complexity, decoding speed and error correcting performance. Simulation results show validity and efficiency of the proposed decoding algorithm.

## Research of an Improved BP Decoding Algorithm

- **Status**: ✅
- **Reason**: 개선 BP: 신뢰도 높은 비트는 갱신 생략, 오류비트만 갱신해 복잡도/지연 감소 — 이식 가능 BP 디코더 기법(C)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5175070
- **Type**: conference
- **Published**: 16-17 May 
- **Authors**: Guoqiang Wu, Zhaowei Sun
- **PDF**: https://ieeexplore.ieee.org/document/5175070
- **Abstract**: That the computation load is big is a shortcoming of traditional BP decoding algorithm. According to this shortcoming we give an improved BP decoding algorithm. The decoding algorithm in the iterative decoding process, only need to update the possibility of a wrong bit of information without having to update the information whose reliability is very high, raises the decoding efficiency and reduces decoding time delay. The simulation result shows: the calculation load of improved BP decoding algorithms descends distinctively with the increasing iteration numbers; Along with the times increasing of LDPC code iterations upper limit, coding performance can be obviously improved.

## Regular LDPC Codes from Strongly Regular (a ,ß )-Geometries

- **Status**: ✅
- **Reason**: strongly regular geometry 기반 새 정규 LDPC 구성+최소거리/girth bound - 이식 가능 코드설계(E)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5231310
- **Type**: conference
- **Published**: 15-17 May 
- **Authors**: Xiuli Li, Zhengduo Shan
- **PDF**: https://ieeexplore.ieee.org/document/5231310
- **Abstract**: By considering a class of combinatorial structures, known as strongly regular (a, beta) -geometries, we define a class of low-density parity-check (LDPC) codes. We derive bounds on minimum distance, rank and girth for the codes from strongly regular (a, beta) -geometries, which have not previously been proposed for use with iterative decoding. In further, we present performance results for the class of codes.

## Classes of LDPC codes constructed from elements of finite fields and cyclotomic cosets

- **Status**: ✅
- **Reason**: 유한체·cyclotomic coset 기반 4-cycle-free 정규 LDPC 구성 신규 대수 기법(E), 바이너리
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5069527
- **Type**: conference
- **Published**: 13-15 May 
- **Authors**: Salah A. Aly, Hussein Maza'ar
- **PDF**: https://ieeexplore.ieee.org/document/5069527
- **Abstract**: In this paper, we derive two algebraic methods for constructing regular low density parity check (LDPC) codes - one based on elements of finite fields and the other directly based on cyclotomic cosets. We show that the constructed codes have high rates and are free of cycles of length four; consequently, they can be decoded using standard iterative decoding algorithms. In addition we compute the exact dimension and establish bounds on the minimum and stopping distances of the constructed codes.

## On the correlation between error weights and syndrome weights for belief propagation decoding of LDPC codes

- **Status**: ✅
- **Reason**: SPA 디코딩에서 오류무게-신드롬무게 상관 분석 기반 수정 SPA 디코더 제안, 바이너리 LDPC BP 디코더 개선(C)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5069516
- **Type**: conference
- **Published**: 13-15 May 
- **Authors**: Irina Adjudeanu, Jean-Yves Chouinard, Paul Fortier
- **PDF**: https://ieeexplore.ieee.org/document/5069516
- **Abstract**: This paper analyzes the correlation between the error weights and the syndrome weights during the sum product algorithm (SPA) decoding process of low density parity check codes (LDPC) in order to improve the error performance and the decoding speed. In this paper, we propose a modified SPA decoding algorithm to improve the decoding performance of the non-decoded codewords and investigate if this new algorithm takes indeed the best decoding decision for each codeword. The effect of the averaged decoding over the error weights and syndrome weights are analyzed as well as their correlation. The correlation coefficients between them are computed and a weight enumerator polynomial is used to jointly represent the error and syndrome weight distribution for each codeword.

## 415 Gbit/in.2 recording in coaxial holographic storage using low-density parity-check codes

- **Status**: ✅
- **Reason**: 홀로그래픽 스토리지에 LDPC 적용+E(16,3,8) 코드 설계로 스토리지 ECC 코드설계 기여 가능성, Phase3 재검토
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:5031755
- **Type**: conference
- **Published**: 10-13 May 
- **Authors**: Kenji Tanaka, Masaaki Hara, Kazutatsu Tokuyama +5
- **PDF**: https://ieeexplore.ieee.org/document/5031755
- **Abstract**: We have applied LDPC codes into our coaxial recording system, and revealed its fundamental characteristics. In addition, by applying the E(16, 3, 8) code and optimizing optical conditions, we have successfully improved the recording density, achieving the raw data density of 415 Gbit/in.2.
