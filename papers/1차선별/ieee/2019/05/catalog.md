...
- **Status**: ✅
- **Reason**: 신경망 BP(가중치 학습)로 CRC-polar factor graph 외재정보 교환 — neural BP 기법이 바이너리 LDPC BP에 이식 가능(C)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:8761399
- **Type**: conference
- **Published**: 20-24 May 
- **Authors**: Nghia Doan, Seyyed Ali Hashemi, Elie Ngomseu Mambou +2
- **PDF**: https://ieeexplore.ieee.org/document/8761399
...
- **Status**: ✅
- **Reason**: 이중 표준 LDPC용 재구성 가능 CNU(체크노드유닛) HW 설계; 이식 가능 디코더 HW 아키텍처(D)
- **알고리즘 기여**: 🔧 하드웨어 수정만
- **ID**: ieee:8992036
- **Type**: conference
- **Published**: 20-22 May 
- **Authors**: Hsin-Hao Su, Tang-Syun Chen, Cheng-Hung Lin
- **PDF**: https://ieeexplore.ieee.org/document/8992036
- **Abstract**: With the development of multimedia technology, many products require fast data transmission and good quality, such as 4K high-definition video, Augmented Reality (AR) or Virtual Reality (VR). The recent low-density parity check (LDPC) decoder designs were designed for individual implementations of IEEE 802.11ad and IEEE 802.15.3c. In this paper, the same set of key module of LDPC decoder, i.e. check node unit (CNU), is proposed for both standards. In IEEE 802.15.3c, the hardware usage rate achieves 100% and 90% for the rate-7/8 and rate-1/2 LDPC decoding, respectively, using the proposed CNU design. In IEEE 802.11ad, the hardware usage rate achieves 96% and 90% for the rate-13/16 and rate-1/2 LDPC decoding, respectively.

## Improved Decoding Method for LDPC-COFDM System

- **Status**: ✅
- **Reason**: 신드롬 기반 버스트오류 식별+LLR-ZF로 LDPC 디코딩 개선 - LDPC 디코더 기법(C) 이식 가능성
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:8803865
- **Type**: conference
- **Published**: 19-22 May 
- **Authors**: Lintao Li, Yao Zhao
- **PDF**: https://ieeexplore.ieee.org/document/8803865
- **Abstract**: In this paper, performance of LDPC-coded OFDM system at the presence of DF and NBI has been studied. As DF and NBI will always lead to burst errors at the receiver’s end, these un-decodable frames can make the BER performance suffering significantly degradation. In order to eliminate this problem, an improved decoding method for LDPC codes in LDPC-COFDM system is presented in this study. Firstly, a novel burst error identification scheme based on syndrome is produced. Secondly, a LLR-ZF method that can improve the decoding performance is proposed. Finally, simulations to testify the effectiveness of the proposed strategy are carried out.

## Hardware-friendly LDPC Decoding Scheduling for 5G HARQ Applications

- **Status**: ✅
- **Reason**: C/D: 5G HARQ용 hardware-friendly LDPC 디코딩 스케줄링(layered, punctured node 복원) — 디코더 스케줄·HW 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:8682481
- **Type**: conference
- **Published**: 12-17 May 
- **Authors**: Cing-Yi Liang, Mao-Ruei Li, Huang-Chang Lee +2
- **PDF**: https://ieeexplore.ieee.org/document/8682481
