# IEEE Xplore — 2024-12 (1차선별 통과)


## Rate-Compatible LDPC Codes Based on Primitive Polynomials and Golomb Rulers

- **Status**: ✅
- **Reason**: 새 바이너리 LDPC 코드설계(E): primitive polynomial+Golomb ruler 기반 rate-compatible 구성, 인코딩/디코딩 복잡도·error rate 분석 포함 — 이식 가능 코드설계 기여
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:10559615
- **Type**: journal
- **Published**: Dec. 2024
- **Authors**: Massimo Battaglioni, Marco Baldi, Franco Chiaraluce +1
- **PDF**: https://ieeexplore.ieee.org/document/10559615
...
- **Status**: ✅
- **Reason**: 새 dual-diagonal QC-LDPC 코드 설계(girth-8, GCD 프레임워크 음수 계수), 바이너리 코드 구성 기법 — E 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:10705346
- **Type**: journal
- **Published**: Dec. 2024
- **Authors**: Guohua Zhang, Mengjuan Lou, Aijing Sun +1
- **PDF**: https://ieeexplore.ieee.org/document/10705346
...
- **Status**: ✅
- **Reason**: 짧은 LDPC OSD를 신경망+조기종료로 개선; min-sum 기반 이식 가능 디코더 알고리즘 — C
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:10706909
- **Type**: journal
- **Published**: Dec. 2024
- **Authors**: Guangwen Li, Xiao Yu
- **PDF**: https://ieeexplore.ieee.org/document/10706909
...
## Parallel Network Structure-aided Neural Shuffled Offset Min-Sum Decoder

- **Status**: ✅
- **Reason**: protograph LDPC용 신경망 shuffled offset min-sum 디코더(NSOMS) 신규 제안 — C 이식 가능 디코더
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:11100601
- **Type**: conference
- **Published**: 8-12 Dec. 
- **Authors**: Liang Lv, Yurong Wang, Yi Fang +1
- **PDF**: https://ieeexplore.ieee.org/document/11100601
- **Abstract**: The paper carries out an in-depth investigation on the neural decoder design for protograph low-density parity-check (LDPC) codes. To be specific, based on the model-driven method, we propose a new type of protograph neural decoder with the parallel network (ParNet) decoding structure, called neural shuffled offset min-sum (NSOMS) decoder. The ParNet uses the model-driven method to unfold shuffled LDPC decoding algorithms into the interpretable network. Furthermore, with the aid of the over-estimation value (OEV), we optimize the training method to accelerate the convergence speed of the proposed decoding network. Theoretical and simulation results indicate that the proposed NSOMS decoder exhibits desirable performance compared to the state-of-the-art counterparts. Hence, the proposed NSOMS decoder can be regarded as a terrific solution for high-reliability communications.

## Turbo Equalization with Coarse Quantization using the Information Bottleneck Method

- **Status**: ✅
- **Reason**: Information Bottleneck 기반 coarse 양자화 LUT 디코더/HW — LLR 양자화·LUT 디코더 기법 NAND 이식 가능(C/D), 애매하면 살림
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:10901165
- **Type**: conference
- **Published**: 8-12 Dec. 
- **Authors**: Philipp Mohr, Jasper Brüggmann, Gerhard Bauch
- **PDF**: https://ieeexplore.ieee.org/document/10901165
- **Abstract**: This paper proposes a turbo equalizer for inter-symbol interference channels (ISI) that uses coarsely quantized messages across all receiver components. Lookup tables (LUTs) carry out compression operations designed with the information bottleneck method aiming to maximize relevant mutual information. The turbo setup consists of an equalizer and a decoder that provide extrinsic information to each other over multiple turbo iterations. We develop simplified LUT structures to incorporate the decoder feedback in the equalizer with significantly reduced complexity. The proposed receiver is optimized for selected ISI channels. A conceptual hardware implementation is developed to compare the area efficiency and error correction performance. A thorough analysis reveals that LUT-based configurations with very coarse quantization can achieve higher area efficiency than conventional equalizers. Moreover, the proposed turbo setups can outperform the respective non-turbo setups regarding area efficiency and error correction capability.

## An LDPC Decoding Method Using a Neural Network as a De-Mapper

- **Status**: ✅
- **Reason**: 신경망 디맵퍼 + 수정 BP 결합 LDPC 디코더 — 이식 가능한 신경망 디코더 기법(C)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:11117413
- **Type**: conference
- **Published**: 6-8 Dec. 2
- **Authors**: Shuailong Zhu, Lijun Zhang
- **PDF**: https://ieeexplore.ieee.org/document/11117413
- **Abstract**: In this paper, we propose a novel LDPC decoding model that employs a fully connected neural network as a demapper to achieve excellent decoding performance while maintaining low computational complexity and computational effort. This neural network model does not need to rely on external coding during the training process, thus significantly reducing the required computational resources. In addition, the model can quickly converge to a parameter configuration with excellent performance through a limited number of iterations. To further enhance the decoding results, this paper also combines this neural demapper with a modified version of the confidence propagation (BP) algorithm. The integrated model achieves excellent decoding performance while keeping the computational load low, even under more stringent conditions.

## Encoding Research Based on the Multiple Sneak Path Issues in Resistive Random Access Memory

- **Status**: ✅
- **Reason**: ReRAM 스토리지에 LDPC 부호워드를 다중 어레이에 배치하는 across-array 코딩 기법 - 스토리지 ECC 코드설계 신규 기여(B/E), 애매하나 살림
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:10928671
- **Type**: conference
- **Published**: 27-29 Dec.
- **Authors**: Mengru Shao, Yanting Chen, Songhe Lu +1
- **PDF**: https://ieeexplore.ieee.org/document/10928671
- **Abstract**: Resistive Random Access Memory (ReRAM), as a non-volatile storage technology, has shown great potential in the storage field due to its high density and fast storage capabilities. However, its crossbar array structure gives rise to the issue of Sneak Path Interference (SPI) that significantly impacts data reliability. To address this challenge, this paper refines the general model of multiple sneak paths based on the number and combination types of these paths. Furthermore, an across-array coding scheme under multiple sneak paths is proposed. This scheme allocates low-density parity-check (LDPC) codewords to multiple memory arrays, reducing the correlation of channelcoded bits used for decoding, thereby achieving a diversity gain. Simulation results show that, under the multiple sneak path model, the cross-array coding scheme with LDPC coding can effectively improve the system’s error rate performance.
