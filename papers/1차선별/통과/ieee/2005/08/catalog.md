# IEEE Xplore — 2005-08 (1차선별 통과)


## Quasi-cyclic LDPC codes for fast encoding

- **Status**: ✅
- **Reason**: QC-LDPC(B-LDPC) 빠른 인코딩 구성, girth 상한·circulant 구조·error floor - 바이너리 코드설계(E)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1468309
- **Type**: journal
- **Published**: Aug. 2005
- **Authors**: Seho Myung, Kyeongcheol Yang, Jaeyoel Kim
- **PDF**: https://ieeexplore.ieee.org/document/1468309
...
- **Status**: ✅
- **Reason**: LLR-BP min-sum 정규화/오프셋 감소복잡도 디코더 기법, 바이너리 LDPC에 직접 이식 가능(C)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1495850
- **Type**: journal
- **Published**: Aug. 2005
- **Authors**: Jinghu Chen, A. Dholakia, E. Eleftheriou +2
- **PDF**: https://ieeexplore.ieee.org/document/1495850
...
- **Status**: ✅
- **Reason**: LDPC 디코딩 검증용 FPGA Gaussian noise generator HW, 디코더 시뮬/HW 인프라로 이식 가능(D)
- **알고리즘 기여**: 🔧 하드웨어 수정만
- **ID**: ieee:1512179
- **Type**: journal
- **Published**: Aug. 2005
- **Authors**: Dong-U Lee, W. Luk, J.D. Villasenor +2
- **PDF**: https://ieeexplore.ieee.org/document/1512179
...
- **Status**: ✅
- **Reason**: turbo-like LDPC + 신경망 디코딩 — 부호 비의존 신경망 디코더 기법(C) 이식 가능, 애매하면 in
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1618080
- **Type**: conference
- **Published**: 8-12 Aug. 
- **Authors**: Zijian Zhao, Xiaojuan Wu, Jun Yang
- **PDF**: https://ieeexplore.ieee.org/document/1618080
- **Abstract**: Turbo codes and LDPC codes are both superior to other error correcting codes. These two families of codes have some similarities. It seems a good idea to design new codes combining these two codes. Through the analysis of turbo codes, we can consider turbo codes as block codes and find their matrix H. In this paper, we give a new class of turbo-like LDPC codes combining the advantages of turbo and LDPC codes and a decoding method based on neural network. A lot of simulations have been made and the results reveal that these codes have robuster performance than turbo codes.

## Low-density parity-check codes based on sparse binary sequences

- **Status**: ✅
- **Reason**: sparse binary sequence 기반 QC-LDPC 구성, girth≥8 보장 — 바이너리 코드설계(E) 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1618129
- **Type**: conference
- **Published**: 8-12 Aug. 
- **Authors**: Shanbao He, Chunming Zhao, Zhihua Shi +1
- **PDF**: https://ieeexplore.ieee.org/document/1618129
- **Abstract**: In this paper, we propose a novel and simple construction of regular LDPC codes using sparse binary sequences. The distribution matrix of ones in a parity-check matrix is introduced into this new construction. The cyclic cross correlation function and the cyclic auto correlation function are employed to ensure that the girth of Tanner graph is at least eight. The new codes perform well using the sum-product algorithm for iterative decoding. Low encoding complexity can also be achieved due to the inherent quasi-cyclic structure of the LDPC codes.

## Construction and performance analysis of moderate-length rate-compatible LDPC codes

- **Status**: ✅
- **Reason**: moderate-length rate-compatible LDPC 구성(extending/information nulling) — 코드설계(E) 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1618109
- **Type**: conference
- **Published**: 8-12 Aug. 
- **Authors**: Zhu Miao, Fei Zesong, Zhao Shenghui +1
- **PDF**: https://ieeexplore.ieee.org/document/1618109
- **Abstract**: Rate-compatible coding is appropriate for communication systems that may experience a range of operating SNRs but that seek to adhere to a single underlying codec structure. This paper proposes two special and efficient approaches: extending and information nulling, to build low rate codes from high rate LDPC codes (mother codes) of moderate length (10/sup 3/<N<10/sup 4/). Both the two proposed approaches result in better performances than punctured RC-LDPC codes at low code-rates. Extending leads to good performance at rate about 0.4/spl sim/0.6, but which has not so good performance at high rates (about 0.6/spl sim/0.8). The threshold of extended code rates is pointed out for the first time, which rests with the code-rate of the mother code. Information nulling, which results in acceptable performance at high rates, and also good performance at low rates without any threshold of code rates, is confirmed to have a wider rate range and lower computation complexity than extending.

## Fast decoding algorithm for low-density parity-check codes

- **Status**: ✅
- **Reason**: 그래프 분해+양방향 메시지패싱으로 BP 수렴 2배 가속 — 디코더 알고리즘(C) 이식 가능
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1618116
- **Type**: conference
- **Published**: 8-12 Aug. 
- **Authors**: Wang Dan, Li Ping, Hu Xiao Yu +1
- **PDF**: https://ieeexplore.ieee.org/document/1618116
- **Abstract**: A fast decoding algorithm for low-density parity-check codes is presented based on graph decomposition and two-way message passing schedule. Simulations show that the convergence speed of the proposed algorithm is about twice as fast as that of the conventional belief propagation algorithm.

## An FPGA implementation of a structured irregular LDPC decoder

- **Status**: ✅
- **Reason**: irregular LDPC FPGA 디코더 + BP용 비균일 양자화 + error floor 낮춘 구성 — HW(D)·디코더(C) 직접 이식
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1618100
- **Type**: conference
- **Published**: 8-12 Aug. 
- **Authors**: Zhigang Cao, Jingyu Kang, Pingyi Fan
- **PDF**: https://ieeexplore.ieee.org/document/1618100
- **Abstract**: In this paper, a code construction method of irregular low-density parity-check (LDPC) codes is presented, which facilitates the hardware implementation of belief propagation (BP) decoders and leads to low error floors as well. To verify the effectiveness of this construction, an FPGA-based decoder is designed and implemented, where a novel non-uniform quantization scheme for BP decoding is used, so that the performance loss compared to infinite-precision decoding is less than 0.1 dB. The decoding throughput of the decoder can reach as high as 50 Mbps.

## Code design and decoder implementation of low density parity check code

- **Status**: ✅
- **Reason**: GA 기반 QC-LDPC 코드탐색(HW고려) + 파이프라인 디코더 FPGA 구현 — 코드설계+HW(D/E)
- **알고리즘 기여**: ✅ 알고리즘/코드 기여
- **ID**: ieee:1544356
- **Type**: conference
- **Published**: 15-15 Aug.
- **Authors**: Mong-Kai Ku, Huan-Sheng Li, Yi-Hsing Chien
- **PDF**: https://ieeexplore.ieee.org/document/1544356
