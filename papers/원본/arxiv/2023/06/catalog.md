...
- **Status**: ❌
- **Reason**: 5G 기지국 polar code 자원할당·비용 최적화. LDPC 부수 언급, 떼어낼 LDPC ECC 기법 없음 → 제외
...
- **Status**: ❌
- **Reason**: GF(2m) 곱셈기 BCH 기반 결함검출 HW. LDPC는 비교대상 언급뿐, 이식할 LDPC 기법 없음 → 제외
...
- **Status**: ❌
- **Reason**: 양자 Tanner LDPC 단발 디코딩. 양자 측정오류·QEC 라운드 전용 구조에 의존, 고전 바이너리 LDPC 이식성 없음 → 제외
...
- **Status**: ✅
- **Reason**: Wi-Fi6/WiMAX QC-LDPC layered min-sum 인코더/디코더 오픈소스 HW 구현(고정소수점·메모리 최적화·병렬). 이식 가능 HW/구현 기법(D)
