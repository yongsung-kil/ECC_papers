...
- **Status**: ❌
- **Reason**: LP 디코딩 threshold에 redundant check 추가 효과의 순수 이론 분석(bound) — 떼어낼 디코더/HW/구성 신규 기법 없음
...
- **Status**: ❌
- **Reason**: 광통신 TFP 시스템 BCJR 검출/equalizer 응용, irregular LDPC는 표준 베이스라인 — 떼어낼 NAND LDPC ECC 신규 기법 없음
...
- **Status**: ❌
- **Reason**: GF(q) 비이진 SC-LDPC 구성/윈도우드 복호 — 비이진 LDPC는 제외 대상
...
- **Status**: ✅
- **Reason**: MLP 신경망 기반 LDPC 디코더(C: 이식 가능 신경망 디코더) — 바이너리 LDPC 복호 대안 기법, 포함
...
- **Status**: ❌
- **Reason**: 4D 광통신 coded modulation의 bit-wise decoder/GMI metric 분석 — LDPC는 검증용 베이스라인, 떼어낼 ECC 디코더 신규 기법 없음
