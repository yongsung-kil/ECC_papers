...
- **Status**: ❌
- **Reason**: 비이진 SC-LDPC threshold saturation 증명(BEC) — 비이진 LDPC 순수 이론, 제외
...
- **Status**: ✅
- **Reason**: QC-LDPC 2단계 lifting 프로토그래프 구성으로 minimum distance·girth 개선 — 신규 바이너리 코드설계 기법(E), NAND ECC 이식 가능
...
- **Status**: ✅
- **Reason**: 짧은 블록 길이 RC irregular LDPC puncturing·extension 기법(사이클 카운팅·EMD) — 신규 바이너리 코드설계 기법(E)
...
- **Status**: ❌
- **Reason**: polar list 디코더 속도 개선 — 비-LDPC 부호, polar 전용 SC list 기법으로 LDPC BP에 이식 불가
...
- **Status**: ✅
- **Reason**: BSC용 바이너리 LDPC 디코딩에서 reinforced BP가 임계값을 높임 — 이식 가능한 BP 개선 기법(C). 통계물리 분석이 주지만 디코더 기여 있어 살림
...
- **Status**: ✅
- **Reason**: Gallager(바이너리 LDPC) error floor를 낮추는 probability-damping BP 변형 — 저복잡도·수렴개선 디코더 기법(C/E), NAND ECC 이식 가능
