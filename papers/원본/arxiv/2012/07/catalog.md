...
- **Status**: ❌
- **Reason**: LT/fountain 코드 기반 분산스토리지용 BP-XOR 코드, erasure/fountain 영역으로 NAND 채널 ECC에 이식할 기법 없음
...
- **Status**: ✅
- **Reason**: FAID(유한알파벳 반복 디코더) 신규 디코더, BSC상 BP 능가·error floor 개선·3비트 정밀도, 바이너리 LDPC에 직접 이식 가능(C)
...
- **Status**: ❌
- **Reason**: 주파수호핑 ad hoc망 최적화, LDPC는 capacity-approaching 코드로 부수 언급, 떼어낼 ECC 기법 없음
...
- **Status**: ❌
- **Reason**: 유한사영평면 기반 양자 LDPC 구성, twisted inner product 등 양자 스태빌라이저 의존, 비이진/양자 전용
