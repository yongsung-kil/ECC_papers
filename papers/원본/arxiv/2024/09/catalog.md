...
- **Status**: ❌
- **Reason**: 양자 메모리용 FTBP, space-time Tanner·degeneracy·syndrome 추출 등 양자 전용 개념 의존 — 양자 EC 제외
...
- **Status**: ❌
- **Reason**: LDPC 코드 적용 ghost imaging, ECC가 아닌 이미징 신호 redundancy 활용 — 떼어낼 NAND ECC 기법 없음
...
- **Status**: ❌
- **Reason**: 협력통신 릴레이망에 기성 LDPC 오류정정 적용, 새 기법 없음 — 무선 응용 특이적 제외
...
- **Status**: ❌
- **Reason**: 양자 LDPC용 Flag-Proxy 네트워크·syndrome 추출 스케줄링, 양자 전용 개념(flag/proxy qubit·surface/color code) 의존 — 양자 EC 제외
...
- **Status**: ❌
- **Reason**: CVQKD 정보 조정(reconciliation) — QKD 보안 응용, LDPC는 베이스라인이고 떼어낼 새 디코더/구성 기법 없음
...
- **Status**: ❌
- **Reason**: 다이아몬드 스핀-기계 공진기 양자 네트워크 — qLDPC 구현 하드웨어 플랫폼, 고전 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: qLDPC 비국소 스태빌라이저 측정/캐비티 게이트 — 양자 전용 개념(스태빌라이저)에 의존, 고전 바이너리 이식 불가
...
- **Status**: ❌
- **Reason**: qLDPC 유효거리·weight reduction·HGP — 스태빌라이저/신드롬 추출 양자 전용, 고전 이식 불가
