...
- **Status**: ❌
- **Reason**: qLDPC localized statistics 디코더; syndrome 기반 양자 EC 전용, 스태빌라이저·축퇴 의존으로 고전 이식 불가
...
- **Status**: ❌
- **Reason**: Cell-free mMIMO IDD 수신기; LDPC는 베이스라인, LLR refinement도 통신 응용 특이적, 떼어낼 ECC 기법 없음
...
- **Status**: ❌
- **Reason**: 양자 코드 통계역학 모델·임계값 이론; 순수 이론 bound, 디코더/HW/구성으로 안 이어짐, 양자 전용
...
- **Status**: ❌
- **Reason**: radial qLDPC 코드(lifted product, 스태빌라이저 회로); 양자 전용 개념 의존, 고전 바이너리 이식 불가
...
- **Status**: ❌
- **Reason**: trapped-ion 논리 큐비트 GHZ 실험(nonlocal qLDPC); 양자 하드웨어·스태빌라이저 전용, 고전 이식 기법 없음
