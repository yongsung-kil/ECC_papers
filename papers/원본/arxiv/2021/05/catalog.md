...
- **Status**: ❌
- **Reason**: mmWave WiFi PHY 레이턴시 측정; LDPC는 디코딩 반복수만 부수 언급, 떼어낼 기법 없음
...
- **Status**: ❌
- **Reason**: 블록체인 데이터 가용성 오라클용 PEG 변형 코드 구성으로 응용 특이적이며 통신비용 절감 목적, NAND ECC에 이식할 새 디코더/구성 기여 없음
...
- **Status**: ❌
- **Reason**: LDPC 패리티체크 행렬을 압축센싱 측정행렬로 재활용하는 응용으로 채널코딩 ECC가 아니며 MCMC 복구는 NAND 디코더로 이식 불가
