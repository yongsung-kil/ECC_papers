...
- **Status**: ❌
- **Reason**: erasure-biased 원자 양자 프로세서용 QEC 코드 벤치마킹 - 양자 노이즈 모델·임계값 중심, 떼어낼 고전 디코더/구성 기법 없음
...
- **Status**: ❌
- **Reason**: BB 코드 파라미터 존재성·특성화 이론(asymptotic badness) - 순수 양자 코드 이론, 디코더/HW/구성 이식 기여 없음
...
- **Status**: ✅
- **Reason**: Decision Tree Decoder - sparse 바이너리 체크행렬만 의존하는 일반 디코더, 양자 전용 개념 비의존하여 고전 바이너리 LDPC 디코더로 이식 가능(C)
...
- **Status**: ❌
- **Reason**: 비이진 NB-LDPC(GF) - non-binary LDPC는 명시적 제외 대상
...
- **Status**: ❌
- **Reason**: 5G polar/LDPC 종합 리뷰 - 서베이, 새 디코더·구성·HW 기여 없음
...
- **Status**: ❌
- **Reason**: 분자통신 시맨틱 학습 프레임워크 - LDPC는 베이스라인 비교군일 뿐, 떼어낼 ECC 기법 없음
...
- **Status**: ✅
- **Reason**: expander 기반 list-decoding capacity 달성 LDPC 명시적 구성 - 거리증폭 절차로 바이너리 LDPC 코드 설계(E), 애매하여 살림
...
- **Status**: ✅
- **Reason**: 양자 어닐링 PE 아키텍처지만 디코딩이 고전 바이너리 LDPC에서 유래(BF vs BP), 스태빌라이저/직교성 비의존 — BF 디코더 기법 이식 가능(C), 애매하므로 살림
...
- **Status**: ❌
- **Reason**: wiretap 채널 보안 leakage 상한 — 순수 이론 bound, 떼어낼 디코더/HW/구성 없음, 보안 응용
