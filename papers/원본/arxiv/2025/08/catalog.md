...
- **Status**: ❌
- **Reason**: 양자 LDPC(qLDPC) 연결성 완화 라우팅; 순수 양자 EC 회로 전용으로 제외 대상
...
- **Status**: ❌
- **Reason**: 양자 LDPC(GB/BB codes) 큐비트 연결성 라우팅; 순수 양자 EC, 제외
...
- **Status**: ❌
- **Reason**: MIMO용 polar subcode; LDPC는 비교 베이스라인일 뿐, 떼어낼 LDPC ECC 기법 없음
...
- **Status**: ❌
- **Reason**: QC-LDPC 그래프를 합성이미지 탐지(커뮤니티 검출)에 차용; ECC 디코딩 기법 아님, 표준 QC-LDPC 구조 단순 활용
...
- **Status**: ❌
- **Reason**: IRA-LDPC parity bit의 probabilistic shaping(소스/신호 셰이핑)으로 ECC 디코더/HW/코드설계 새 기여 아님; AWGN 신호 셰이핑 응용
...
- **Status**: ❌
- **Reason**: QC-LDPC 그래프를 이미지 분류 차원압축에 차용; 채널코딩 ECC 아님, 표준 QC-LDPC 구조 단순 활용
...
- **Status**: ❌
- **Reason**: 페르미온-페르미온 LDPC(양자 EC); 바이너리 채널코딩 LDPC 아님, 양자 LDPC로 제외
...
- **Status**: ❌
- **Reason**: 양자 암호해독(ECDLP/Shor) 자원추정. LDPC는 cat code 맥락 부수 언급, 고전 바이너리 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: 양자 LRC(qLRC)/qLDPC 부호 구성, Hermitian·t-design 양자 전용 개념 의존. 고전 바이너리 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: FTN 신호용 CNN ISI 검출기. LDPC는 베이스라인 부수 언급(LDPC-coded FTN), 떼어낼 ECC 기법 없음
...
- **Status**: ❌
- **Reason**: 양자 LDPC(tricycle code) magic state 생성, transversal CCZ 등 양자 전용 개념 의존. 고전 이식 불가
...
- **Status**: ❌
- **Reason**: Majorana 기반 양자부호 fault-tolerant 게이트/측정. 양자 전용 하드웨어·개념 의존
...
- **Status**: ❌
- **Reason**: 양자 fault-tolerance threshold 증명 프레임워크. qLDPC 부수 언급, 고전 바이너리 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: 폴라 vs LDPC 복잡도 비교 분석. LDPC 디코더/구성/HW 신규 기여 없음, 비교 연구일 뿐
...
- **Status**: ❌
- **Reason**: 양자 LDPC LP 디코더. OSD 후처리 자체는 고전 유래이나 분석 대상이 qLDPC 전용 에러패턴, 양자 맥락 의존
...
- **Status**: ❌
- **Reason**: 비이진(non-binary) LDPC 디코더 PiM 구현. 비이진 LDPC는 기준상 즉시 제외
...
- **Status**: ❌
- **Reason**: 분산 양자 메모리(BB qLDPC 코드) 아키텍처. 양자 스태빌라이저·flying qubit 의존, 고전 이식 불가
