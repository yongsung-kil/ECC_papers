...
- **Status**: ❌
- **Reason**: QEC용 생성형(자기회귀 신경망) ML 디코더로 surface/qLDPC 대상이며 축퇴·논리연산자 등 양자 전용 개념에 의존, 고전 바이너리 LDPC로 그대로 이식 안 됨
...
- **Status**: ❌
- **Reason**: MIMO-SCMA 수신기(EPA) 응용 특이적이며 LDPC는 결합 그래프 베이스라인일 뿐, NAND로 떼어낼 새 디코더/코드 기법 없음
...
- **Status**: ❌
- **Reason**: LDPC를 엔트로피 소스 코딩(압축)으로 사용한 goal-oriented 분류 — 채널 ECC가 아님, 소스 코딩 제외
...
- **Status**: ❌
- **Reason**: CQED 네트워크 기반 FTQC surface/qLDPC 아키텍처로 광자 손실 정보 활용 디코딩 — 양자 전용, 고전 바이너리 LDPC 이식 불가
...
- **Status**: ❌
- **Reason**: QLDPC 메모리용 Pauli 기반 계산 아키텍처(extractor) — 양자 전용 논리연산·신드롬, 고전 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: Lifted-product QLDPC 유한길이 최소거리 설계로 축퇴·스태빌라이저 등 양자 전용 개념에 의존, 고전 바이너리 LDPC 이식 아님
...
- **Status**: ❌
- **Reason**: 양자 code surgery(논리 Pauli 측정 병렬화) — 양자 전용 fault-tolerant 기법, 고전 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: QLDPC(bivariate-bicycle) 위상학적 anyon 이론 분석 — 순수 양자 위상 이론, 디코더/HW/고전 코드설계로 안 이어짐
...
- **Status**: ❌
- **Reason**: 토릭/CSS 양자코드 구성 - 스태빌라이저·anyon·위상질서 등 양자 전용 개념에 의존, 고전 바이너리 LDPC로 이식 불가
