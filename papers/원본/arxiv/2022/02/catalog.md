...
- **Status**: ❌
- **Reason**: 블록체인 데이터 가용성 공격 분석 — LDPC가 부수 도구, NAND ECC로 떼어낼 디코더/HW/구성 기법 없음
...
- **Status**: ❌
- **Reason**: 양자 CSS/hyperbolic 부호 fold-transversal Clifford 게이트 — 양자 전용 개념 의존, 고전 이식 불가
...
- **Status**: ❌
- **Reason**: LDPC-SCMA 수신기 JDD(EPA) 설계 — SCMA 다중접속 특이적, LDPC는 베이스라인이고 떼어낼 순수 ECC 디코더 기여 없음
...
- **Status**: ❌
- **Reason**: 비이진(non-binary GF(2^q)) LDPC ADMM 디코더 — 기준상 비이진 LDPC 즉시 제외
...
- **Status**: ❌
- **Reason**: 하이브리드 코드 변조: LDPC는 표준 5G outer code로 사용될 뿐, 신경망 inner code는 변조 설계이며 NAND LDPC ECC에 떼어낼 디코더/코드 기법 없음
...
- **Status**: ❌
- **Reason**: 양자 LDPC(bias-tailored lifted product) 원칙 제외; bias-tailoring은 큐비트 노이즈 비대칭/스태빌라이저 의존이라 고전 바이너리 LDPC로 그대로 이식 불가
