...
- **Status**: ❌
- **Reason**: GRAND 기반 변조/삽입-삭제 오류 처리, LDPC는 비교 베이스라인일 뿐 NAND LDPC에 이식할 디코더/코드 기여 없음
...
- **Status**: ✅
- **Reason**: Protograph LDPC 코드 설계(MPEXIT로 rate-compatible EARA 코드 구성)는 바이너리 코드설계 기법으로 NAND LDPC에 이식 가능(E), 애매하여 살림
...
- **Status**: ❌
- **Reason**: 양자 fault-tolerant 컴퓨팅 개관(qLDPC), 고전 바이너리 LDPC 이식 기법 없음
...
- **Status**: ❌
- **Reason**: 양자 LDPC 기반 얽힘 정제(GHZ distillation), 스태빌라이저·양자 전용 개념 의존, 제외
...
- **Status**: ❌
- **Reason**: 스태빌라이저 코드에 Clifford 게이트 구현(양자 EC), 양자 전용, 이식 기법 없음
...
- **Status**: ❌
- **Reason**: Cell-free MIMO IDD에서 표준 LDPC 디코더를 베이스라인으로 사용, 새 디코더/코드 기여 없는 무선 응용
...
- **Status**: ❌
- **Reason**: NLTS 해밀토니안 이론 구성, 순수 이론·양자 영역, 디코더/HW/구성 이식 없음
