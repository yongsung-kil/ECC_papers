...
- **Status**: ❌
- **Reason**: 비이진(GF(q)) LDPC용 LLR 생성 아키텍처, 비이진 LDPC는 제외 대상
- **ID**: ieee:5934668
- **Type**: journal
- **Published**: August 201
- **Authors**: Ali Al Ghouwayel, Emmanuel Boutillon
- **PDF**: https://ieeexplore.ieee.org/document/5934668
- **Abstract**: Non-Binary LDPC codes offer higher performances than their binary counterpart but suffer from higher decoding complexity. A solution to reduce the decoding complexity is to use the Extended Min-Sum algorithm. The first step of this algorithm requires the generation of the first n_m largest Log-Likelihood Ratio (LLR), sorted in increasing order, of each received symbol. In the case where GF(q) symbols are transmitted using a BPSK modulation, we propose a simple systolic architecture that generates the sorted list of symbols.

## On Throughput of MIMO-OFDM Systems with Joint Iterative Channel Estimation and Multiuser Detection under Different Multiple Access Schemes

- **Status**: ❌
- **Reason**: MIMO-OFDM 멀티유저 검출/채널추정 처리량 비교, LDPC ECC 기법 없음
- **ID**: ieee:5910096
- **Type**: journal
- **Published**: August 201
- **Authors**: Pierluigi Salvo Rossi
- **PDF**: https://ieeexplore.ieee.org/document/5910096
...
- **Status**: ❌
- **Reason**: CQED 기반 양자 LDPC 인코더/디코더 — 양자 게이트(Hadamard/CZ)·스태빌라이저·dual-containing 의존, 고전 바이너리 LDPC로 이식 불가
- **ID**: ieee:5955062
- **Type**: journal
- **Published**: Aug. 2011
- **Authors**: Ivan B. Djordjevic
- **PDF**: https://ieeexplore.ieee.org/document/5955062
...
- **Status**: ❌
- **Reason**: MBIOS 채널에서 보편적 LDPC 앙상블의 용량달성 순수 이론 bound — 디코더/HW/신규 구성으로 안 이어짐
- **ID**: ieee:5961846
- **Type**: journal
- **Published**: Aug. 2011
- **Authors**: Igal Sason, Boaz Shuval
- **PDF**: https://ieeexplore.ieee.org/document/5961846
...
- **Status**: ✅
- **Reason**: 클럭리스 완전병렬 LDPC 디코더용 저전력 비동기 인터리버 HW 아키텍처 — 병렬 디코더 HW로 이식 가능(D)
- **ID**: ieee:5712175
- **Type**: journal
- **Published**: Aug. 2011
- **Authors**: Naoya Onizawa, Vincent C. Gaudet, Takahiro Hanyu
- **PDF**: https://ieeexplore.ieee.org/document/5712175
- **Abstract**: This paper presents a low-energy asynchronous interleaver for clockless fully parallel low-density parity-check (LDPC) decoding. The proposed data-transmission circuit based on a half-duplex single-track protocol makes it possible to realize a wire-efficient asynchronous interleaver with small energy consumption. Moreover, a data-monitoring system adaptively shuts down the asynchronous data-transmission circuit if not necessary, which reduces the number of data transmissions and, hence, the energy consumed. The clockless decoder with the proposed asynchronous interleaver is evaluated using a (1056,528) irregular LDPC code under a 90-nm CMOS process. As a result, the energy dissipation per uncoded bit at Eb/No of 5 dB becomes 54 pJ/bit with an uncoded throughput of 45.5 Gbps under a postlayout simulation. This represents a 92% decrease in energy per bit and a 1143% in throughput increase with respect to our previous clockless LDPC decoder.
...
- **Status**: ✅
- **Reason**: PCM 멀티레벨 메모리 ECC에서 시간인지 양자화로 LDPC 소프트입력 조정 — LLR 양자화/소프트디코딩 기법이 NAND retention/quantization으로 이식 가능(A/C)
- **ID**: ieee:5497218
- **Type**: journal
- **Published**: Aug. 2011
- **Authors**: Wei Xu, Tong Zhang
- **PDF**: https://ieeexplore.ieee.org/document/5497218
...
- **Status**: ✅
- **Reason**: QC-LDPC multi-rate 부분병렬 디코더 HW 아키텍처(3-parallel CNP, 메모리 절감) - D 이식 가능
- **ID**: ieee:6026376
- **Type**: conference
- **Published**: 7-10 Aug. 
- **Authors**: Sung-Woo Choi, Gyung-Pyo Kim, Jin-Kyeong Kim
- **PDF**: https://ieeexplore.ieee.org/document/6026376
- **Abstract**: This paper presents a partially parallel LDPC decoder architecture for QC-LDPC codes. In particular, we introduce a check node processing element which is 3-parallel, adjustable to irregular inputs and easily expandable. Furthermore, our decoder is applicable to multi-rate system by simply writing additional data to internal RAM. In another aspect of our work, we can reduce the check-bit message memory significantly by efficient method. Implementation results show that the proposed architecture can support the data rate of 360Mbps in FPGA.

## Reduced-memory forward-backward check node processing architecture for non-binary LDPC decoding

- **Status**: ❌
- **Reason**: 비이진 NB-LDPC over GF(q) 디코더 - 비이진 LDPC 제외 대상
- **ID**: ieee:6026389
- **Type**: conference
- **Published**: 7-10 Aug. 
- **Authors**: Xinmiao Zhang, Fang Cai
- **PDF**: https://ieeexplore.ieee.org/document/6026389
...
- **Status**: ✅
- **Reason**: NAND Flash용 layered min-sum LDPC 디코더 직접 - A/C/D 명확 포함
- **ID**: ieee:6026357
- **Type**: conference
- **Published**: 7-10 Aug. 
- **Authors**: Jonghong Kim, Junhee Cho, Wonyong Sung
- **PDF**: https://ieeexplore.ieee.org/document/6026357
...
## Punctured pilots for OFDMA communications

- **Status**: ❌
- **Reason**: OFDMA pilot/puncturing 통신 응용 특이적, LDPC puncturing 부수 언급, 떼어낼 ECC 기법 없음
- **ID**: ieee:6026663
- **Type**: conference
- **Published**: 7-10 Aug. 
- **Authors**: Sung-Yoon Jung, Sunghwan Kim
- **PDF**: https://ieeexplore.ieee.org/document/6026663
- **Abstract**: In this paper, we propose the punctured pilots for channel estimation in OFDMA systems. These are pilots additionally inserted, after puncturing the modulated OFDMA symbols, to enhance channel estimation performance, without lessening bandwidth efficiency. In addition, puncturing scheme of forward error correcting (FEC) codes, especially focused on LDPC codes, is also proposed to prevent the performance degradation due to the codeword bits loss caused by punctured OFDMA symbols. Simulation results show that the enhanced channel estimation induces a lower bit error rate (BER) in harsh channel environments.

## Non-binary quasi-cyclic quantum LDPC codes

- **Status**: ❌
- **Reason**: 비이진(extended field 2^p) LDPC 기반 양자코드 - 비이진+양자 이중 제외
- **ID**: ieee:6034212
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Kenta Kasai, Manabu Hagiwara, Hideki Imai +1
- **PDF**: https://ieeexplore.ieee.org/document/6034212
...
- **Status**: ❌
- **Reason**: 양자 LDPC(SC QC qLDPC), 최소거리·스태빌라이저 의존 양자 전용 구성 - 양자EC 원칙 제외
- **ID**: ieee:6034208
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Manabu Hagiwara, Kenta Kasai, Hideki Imai +1
- **PDF**: https://ieeexplore.ieee.org/document/6034208
...
- **Status**: ❌
- **Reason**: fractional distance LP bound 및 nonbinary/generalized LDPC fundamental polytope, 비이진 포함+순수 이론 bound
- **ID**: ieee:6033739
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: David Burshtein, Idan Goldenberg
- **PDF**: https://ieeexplore.ieee.org/document/6033739
...
- **Status**: ❌
- **Reason**: LDPC 앙상블 measure 집중부등식 순수 이론 결과, 디코더/HW/구성으로 안 이어짐
- **ID**: ieee:6033740
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Igal Sason, Ronen Eshel
- **PDF**: https://ieeexplore.ieee.org/document/6033740
...
- **Status**: ❌
- **Reason**: expander/LDPC 최소거리 bound가 GF(q) 및 Reed-Solomon 구성요소 기반-비이진+순수 이론 bound
- **ID**: ieee:6033768
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Alexey Frolov, Victor Zyablov
- **PDF**: https://ieeexplore.ieee.org/document/6033768
...
- **Status**: ✅
- **Reason**: Hamming 기반 QC-LDPC base matrix 구성으로 large min distance/girth 14 달성하는 새 코드설계 (E)
- **ID**: ieee:6034273
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Irina E. Bocharova, Florian Hug, Rolf Johannesson +1
- **PDF**: https://ieeexplore.ieee.org/document/6034273
...
- **Status**: ✅
- **Reason**: 바이너리 LDPC(SPC/Hamming 구성) 하드결정 반복디코딩의 정정 가능 오류 비율 신규 하한 - 코드설계/디코더 분석 이식 가능(E)
- **ID**: ieee:6034144
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Pavel Rybin, Victor Zyablov
- **PDF**: https://ieeexplore.ieee.org/document/6034144
...
- **Status**: ✅
- **Reason**: linear 인코딩 복잡도 partially-regular 바이너리 LDPC 신규 구성, 개선된 임계값 - 코드설계 이식 가능(E)
- **ID**: ieee:6034184
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Dmitri K. Zigangirov, Kamil Sh. Zigangirov, Daniel J. Costello
- **PDF**: https://ieeexplore.ieee.org/document/6034184
...
- **Status**: ❌
- **Reason**: 릴레이 채널용 bilayer LDPC convolutional, 채널적응 구성-무선 릴레이 응용 특이적, BEC 한정 임계값
- **ID**: ieee:6033784
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Zhongwei Si, Ragnar Thobaben, Mikael Skoglund
- **PDF**: https://ieeexplore.ieee.org/document/6033784
...
- **Status**: ✅
- **Reason**: 바이너리 LDPC용 신규 two-bit bit flipping 디코딩 알고리즘 — 보장 정정능력 2배, 저복잡도 BP 근접, 이식 가능 디코더(C)
- **ID**: ieee:6033902
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Dung Viet Nguyen, Bane Vasić, Michael W. Marcellin
- **PDF**: https://ieeexplore.ieee.org/document/6033902
...
- **Status**: ✅
- **Reason**: LDPC-CC의 Tanner 그래프 변환으로 girth 확대, dynamic scheduling+error floor 개선 디코딩 — 이식 가능 디코더/코드설계(C/E)
- **ID**: ieee:6033864
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Jian-Jia Weng, Mu-Chen Wu, Chung-Hsuan Wang +2
- **PDF**: https://ieeexplore.ieee.org/document/6033864
...
- **Status**: ✅
- **Reason**: spatially-coupled MacKay-Neal/HA 부호 구성, 바이너리 LDPC 코드설계 기법 - 이식 가능(E)
- **ID**: ieee:6034233
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Kenta Kasai, Kohichi Sakaniwa
- **PDF**: https://ieeexplore.ieee.org/document/6034233
...
- **Status**: ✅
- **Reason**: 바이너리 BEC상 convolutional/SC-LDPC ensemble의 scaling·threshold saturation 분석 — 유한길이/SC-LDPC 코드 설계 가이드(E)
- **ID**: ieee:6033863
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Pablo M. Olmos, Rüdiger Urbanke
- **PDF**: https://ieeexplore.ieee.org/document/6033863
...
- **Status**: ✅
- **Reason**: voltage graph 기반 QC-LDPC large girth(최대24) 구성, 바이너리 블록부호 - 이식 가능 코드설계(E)
- **ID**: ieee:6034230
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Irina E. Bocharova, Florian Hug, Rolf Johannesson +2
- **PDF**: https://ieeexplore.ieee.org/document/6034230
...
- **Status**: ❌
- **Reason**: noisy Slepian-Wolf 소스코딩/JSCC 문제에 SC-LDPC 적용, 채널 ECC가 아닌 분산소스코딩 응용
- **ID**: ieee:6034032
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Arvind Yedla, Henry D. Pfister, Krishna R. Narayanan
- **PDF**: https://ieeexplore.ieee.org/document/6034032
...
- **Status**: ✅
- **Reason**: projective plane 기반 LDPC 구성 신기법, 바이너리 - 이식 가능 코드설계(E), 애매시 in
- **ID**: ieee:6034231
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Sergei Kovalev, Victor Y. Krachkovsky
- **PDF**: https://ieeexplore.ieee.org/document/6034231
...
- **Status**: ✅
- **Reason**: QC/SCB-LDPC absorbing set 제거로 error-floor 개선하는 코드설계 기법 — NAND ECC 직접 이식(E)
- **ID**: ieee:6034068
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Jiadong Wang, Lara Dolecek, Zhengya Zhang +1
- **PDF**: https://ieeexplore.ieee.org/document/6034068
...
- **Status**: ❌
- **Reason**: 비이진(GF/Z4) LDPC LP 디코딩의 check node BCJR 처리 — 비이진 LDPC 대상이므로 제외
- **ID**: ieee:6033826
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Mayur Punekar, Mark F. Flanagan
- **PDF**: https://ieeexplore.ieee.org/document/6033826
...
- **Status**: ❌
- **Reason**: Slepian-Wolf 분산소스코딩에 RS 코드 적용 — 소스코딩+비-LDPC 부호, 떼어낼 LDPC ECC 기법 없음
- **ID**: ieee:6033873
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Shizheng Li, Aditya Ramamoorthy
- **PDF**: https://ieeexplore.ieee.org/document/6033873
...
- **Status**: ✅
- **Reason**: 바이너리 선형부호 ALP 디코딩용 적응 cut 생성(RPC) 알고리즘, LDPC 시뮬레이션-디코더 성능 개선 기법 이식 가능(C)
- **ID**: ieee:6033822
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Xiaojie Zhang, Paul H. Siegel
- **PDF**: https://ieeexplore.ieee.org/document/6033822
...
- **Status**: ❌
- **Reason**: Davey-MacKay 삽입/삭제 채널용 inner decoder, LDPC는 outer 베이스라인 - 채널특이적, 떼어낼 NAND ECC 기법 약함
- **ID**: ieee:6034232
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Xiaopeng Jiao, Marc A. Armand
- **PDF**: https://ieeexplore.ieee.org/document/6034232
...
- **Status**: ✅
- **Reason**: LDPC convolutional codes의 message passing 스케줄링(sliding window decoding)으로 디코딩 복잡도 저감 — 이식 가능 디코더 알고리즘(C)
- **ID**: ieee:6033865
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Michael Lentmaier, Maria Mellado Prenda, Gerhard P. Fettweis
- **PDF**: https://ieeexplore.ieee.org/document/6033865
...
- **Status**: ❌
- **Reason**: Cayley graph 기반 양자 LDPC 구성, 양자코드 파라미터/최소거리에 의존 - 양자EC 원칙 제외
- **ID**: ieee:6034209
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Alain Couvreur, Nicolas Delfosse, Gilles Zemor
- **PDF**: https://ieeexplore.ieee.org/document/6034209
...
- **Status**: ✅
- **Reason**: 결합(spatially-coupled) LDPC 앙상블 growth rate 분석, SC-LDPC 코드설계 이론으로 바이너리 LDPC에 이식 가능(E)
- **ID**: ieee:6034145
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: S. Hamed Hassani, Nicolas Macris, Ryuhei Mori
- **PDF**: https://ieeexplore.ieee.org/document/6034145
...
- **Status**: ✅
- **Reason**: 비휘발성 메모리 동적 읽기 임계값 기반 ECC — NAND read-retry/임계값 설정과 직결(A)
- **ID**: ieee:6033936
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Hongchao Zhou, Anxiao Jiang, Jehoshua Bruck
- **PDF**: https://ieeexplore.ieee.org/document/6033936
...
- **Status**: ❌
- **Reason**: CDMA 멀티유저 검출에 spatial coupling 적용, 무선 통신 응용 특이적이며 떼어낼 LDPC ECC 기법 없음
- **ID**: ieee:6033789
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Keigo Takeuchi, Toshiyuki Tanaka, Tsutomu Kawabata
- **PDF**: https://ieeexplore.ieee.org/document/6033789
...
- **Status**: ❌
- **Reason**: Compute-and-forward용 ℤq 선형코드(비프라임 비이진) — 비이진+무선응용, 떼어낼 바이너리 LDPC 기법 없음
- **ID**: ieee:6033876
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Or Ordentlich, Jiening Zhan, Uri Erez +2
- **PDF**: https://ieeexplore.ieee.org/document/6033876
...
- **Status**: ❌
- **Reason**: compressive sensing GAMP용 최적 양자화기 설계, 채널 ECC가 아닌 신호복원이라 떼어낼 LDPC ECC 기법 없음
- **ID**: ieee:6034168
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Ulugbek Kamilov, Vivek K Goyal, Sundeep Rangan
- **PDF**: https://ieeexplore.ieee.org/document/6034168
...
- **Status**: ❌
- **Reason**: LP 디코딩 가능 순열부호(permutation codes)-비-LDPC 부호, 유클리드거리 LP 디코딩이라 바이너리 LDPC BP 이식 불가
- **ID**: ieee:6033769
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Tadashi Wadayama, Manabu Hagiwara
- **PDF**: https://ieeexplore.ieee.org/document/6033769
...
- **Status**: ❌
- **Reason**: MIMO-ISI 선형 프리코딩+ILMMSE 검출 rate 분석, 무선 응용 특이적이며 LDPC ECC 기법 없음
- **ID**: ieee:6034109
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Xiaojun Yuan, Li Ping, Aleksandar Kavcic
- **PDF**: https://ieeexplore.ieee.org/document/6034109
- **Abstract**: In this paper, we consider the performance analysis of multiple-input multiple-output (MIMO) inter-symbol interference (ISI) systems involving linear-precoding (LP) and iterative linear minimum mean-square error (ILMMSE) detection. The main contribution of this paper is an area theorem to evaluate the achievable rate of the proposed LP-ILMMSE scheme. Based on this area theorem, we further optimize the linear precoder to maximize the achievable rate. Numerical results are provided to verify our analysis.

## Threshold saturation on channels with memory via spatial coupling

- **Status**: ✅
- **Reason**: spatial coupling으로 BP threshold saturation — SC-LDPC 코드설계 기법(E), 바이너리 채널 대상
- **ID**: ieee:6034031
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Shrinivas Kudekar, Kenta Kasai
- **PDF**: https://ieeexplore.ieee.org/document/6034031
...
- **Status**: ❌
- **Reason**: MIMO 공간시간 선형프리코딩+LMMSE 검출, FEC는 부수 언급, 떼어낼 LDPC ECC 기법 없음(무선응용 특이적)
- **ID**: ieee:6033746
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Xiaojun Yuan, Li Ping
- **PDF**: https://ieeexplore.ieee.org/document/6033746
...
- **Status**: ❌
- **Reason**: compressed sensing 복원 알고리즘의 density evolution 분석, 채널 디코딩이 아닌 신호복원 응용으로 NAND ECC 이식 기법 없음
- **ID**: ieee:6034172
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Yaser Eftekhari, Anoosheh Heidarzadeh, Amir H. Banihashemi +1
- **PDF**: https://ieeexplore.ieee.org/document/6034172
...
- **Status**: ❌
- **Reason**: product code용 MinSum 리스트 디코더, 비-LDPC 부호(product code) 전용이라 원칙 제외
- **ID**: ieee:6033741
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: Alexander Barg, Gilles Zémor
- **PDF**: https://ieeexplore.ieee.org/document/6033741
...
- **Status**: ❌
- **Reason**: LDPCC 앙상블 free distance/trapping set 성장률 점근적 bound 분석, 순수 이론 bound로 디코더/HW/구성으로 안 이어짐
- **ID**: ieee:6033700
- **Type**: conference
- **Published**: 31 July-5 
- **Authors**: David G. M. Mitchell, Ali E. Pusane, Michael Lentmaier +1
- **PDF**: https://ieeexplore.ieee.org/document/6033700
...
- **Status**: ❌
- **Reason**: 압축센싱(스펙트럼 센싱)용 LDPC-like 측정행렬, 채널 ECC 아님
- **ID**: ieee:6158295
- **Type**: conference
- **Published**: 17-19 Aug.
- **Authors**: Xuan Sun, Zheng Zhou, Lei Shi +1
- **PDF**: https://ieeexplore.ieee.org/document/6158295
...
## The modified reliability-based iterative majority-logic decoding algorithm for structured LDPC codes

- **Status**: ✅
- **Reason**: 구조화 LDPC용 수정 reliability-based iterative majority-logic 디코더(MRBI-MLGD) 정수 메시지패싱 알고리즘(C 이식 가능).
- **ID**: ieee:6023701
- **Type**: conference
- **Published**: 12-14 Aug.
- **Authors**: Haiqiang Chen, Jinfeng Li, Tuanfa Qin
- **PDF**: https://ieeexplore.ieee.org/document/6023701
- **Abstract**: In this paper, we present a modified reliability-based iterative majority-logic decoding (MRBI-MLGD) algorithm for structured LDPC codes. The MRBI-MLGD algorithm uses the initial rather than the last updated message value in the iteration procedure, which works resembling a turbo decoder. Furthermore, the MRBI-MLGD algorithm is an integer message passing algorithm and mainly requires logical and integer operations. Simulation results show that, compared to the original one, the MIRB-MLGD algorithm can achieve better performance with slightly increased computational complexity.
