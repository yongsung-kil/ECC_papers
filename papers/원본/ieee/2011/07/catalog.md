...
- **Status**: ❌
- **Reason**: 블록페이딩 릴레이용 root-LDPC rate-compatible 구성, 페이딩 다이버시티 응용 특이적
- **ID**: ieee:5895062
- **Type**: journal
- **Published**: July 2011
- **Authors**: Dieter Duyck, Joseph Jean Boutros, Marc Moeneclaey
- **PDF**: https://ieeexplore.ieee.org/document/5895062
...
- **Status**: ❌
- **Reason**: 비이진 NB-LDPC Min-max 디코더 VLSI(GF(25)) - 비이진 LDPC는 제외 대상
- **ID**: ieee:5460996
- **Type**: journal
- **Published**: July 2011
- **Authors**: Xinmiao Zhang, Fang Cai
- **PDF**: https://ieeexplore.ieee.org/document/5460996
...
- **Status**: ❌
- **Reason**: MIMO 양방향 릴레이 용량/DM-tradeoff, LDPC 무관
- **ID**: ieee:5895098
- **Type**: journal
- **Published**: July 2011
- **Authors**: Rahul Vaze, Robert W. Heath
- **PDF**: https://ieeexplore.ieee.org/document/5895098
...
- **Status**: ❌
- **Reason**: code-aided 심볼 타이밍 복원 성능분석 - 동기화 응용, 떼어낼 디코더/코드설계 기법 없음
- **ID**: ieee:5733454
- **Type**: journal
- **Published**: July 2011
- **Authors**: Nan Wu, Hua Wang, Jingming Kuang +1
- **PDF**: https://ieeexplore.ieee.org/document/5733454
- **Abstract**: We analyze the performance of a code-aided (CA) decision-directed (DD) timing synchronizer, which can exploit the dependence structure across coded symbols to improve the timing recovery accuracy. Due to the inherent coupling between timing recovery and decoding, most existing studies rely on extensive simulation rather than on analytical methods to evaluate performance of timing recovery for coded systems. We propose analytical methods in this paper towards this end. A first key step is to approximate timing-offset-induced inter-symbol interference (ISI) as an additive Gaussian noise, since in the low signal-to-noise ratio (SNR) regime the background noise is large enough to mask the ISI. Then, we derive semi-analytical expressions for the mean and variance of extrinsic information as functions of timing offset, building on which we characterize both open-loop and closed-loop performance of decision-directed timing synchronizers. Monte Carlo simulation results corroborate that the proposed method accurately characterizes the performance of CA DD timing recovery, for systems with a wide range of channel bandwidth and different channel codes.

## A syndrome-based LDPC decoder with very low error floor

- **Status**: ✅
- **Reason**: 신드롬 기반 offset Min-Sum 디코더로 error floor 개선; 이식 가능 디코더 알고리즘(C)
- **ID**: ieee:6004950
- **Type**: conference
- **Published**: 6-8 July 2
- **Authors**: I. Tsatsaragkos, N. Kanistras, V. Paliouras
- **PDF**: https://ieeexplore.ieee.org/document/6004950
- **Abstract**: This work improves the performance of LDPC decoders that implement iterative algorithms dominated by oscillatory behavior - such as offset Min-Sum algorithm - in cases of unsuccessful decoding of received words. The proposed LDPC decoder is applied on the decoding procedure of an LDPC algorithm by selecting the one of the N different estimated codewords (each produced at an iteration of the decoder) that corresponds to the syndrome with the minimum number of unsatisfied check constraints, as the output of the decoder. This method provides a decoder that achieves a notable performance improvement in the error floor region of operation for the offset Min-Sum decoding algorithm, with an insignificant increase of the hardware complexity.

## Multiple LDPC decoder of very low bit-error rate

- **Status**: ✅
- **Reason**: 다중 알고리즘 디코더로 error floor의 BER 저감, 반복 감소; 이식 가능 디코더 알고리즘(C)
- **ID**: ieee:6004951
- **Type**: conference
- **Published**: 6-8 July 2
- **Authors**: I. Tsatsaragkos, N. Kanistras, V. Paliouras
- **PDF**: https://ieeexplore.ieee.org/document/6004951
- **Abstract**: The error correcting capability of LDPC based systems at low noise levels is often dominated by the so-called error floor, a region in the BER vs. noise level plot, where BER reduction slows down as the noise level decreases. The error floor behavior is commonly attributed to the sub-optimality of iterative decoding algorithms on graphs with cycles, which become trapped to local minimum solutions. Trapping of the decoder depends on several factors, including the decoding algorithm. A particular received word that is not decoded by a certain algorithm may be decoded successfully by a different algorithm. The proposed Multiple Decoder exploits this diverse behavior, by decoding a particular received word with N different algorithms, composing an LDPC decoder that achieves very low BER in the error floor region of operation, less iterations and higher throughput than the equivalent single decoder system.

## Novel intentional puncturing schemes for finite-length irregular LDPC codes

- **Status**: ✅
- **Reason**: 유한길이 불규칙 LDPC rate-compatible 펑처링 스킴(짧은사이클/EMD 기반); 바이너리 코드설계 기법(E)
- **ID**: ieee:6004903
- **Type**: conference
- **Published**: 6-8 July 2
- **Authors**: Jingjing Liu, Rodrigo C. de Lamare
- **PDF**: https://ieeexplore.ieee.org/document/6004903
- **Abstract**: In this paper, we investigate rate-compatible (RC) short/ moderate length irregular low-density parity-check (LDPC) codes and propose three different novel puncturing schemes to determine the preferred puncturing positions so as to achieve less performance degradation. The first one is based on a short cycle counting algorithm as well as grouping variable nodes which have the equal length of short cycles in the Tanner Graph; the second scheme can be seen as an improved version of the first one by means of introducing another metric called Extrinsic Message Degree (EMD); and the last scheme relies on a simulation-based exhaustive search in order to find the best puncturing pattern among several random puncturing patterns. The simulation results show that all three proposed schemes can achieve the rate flexibility and minimize the performance loss over the additive white Gaussian noise (AWGN) channel.

## Multi-rate QC-LDPC scheme for narrowband powerline communication system

- **Status**: ❌
- **Reason**: PLC용 표준 QC-LDPC 다중률 스킴 적용, 새 디코더/구성 기여 없음.
- **ID**: ieee:5982561
- **Type**: conference
- **Published**: 4-8 July 2
- **Authors**: Qi Zhang, Fang Yang, Jian Song
- **PDF**: https://ieeexplore.ieee.org/document/5982561
- **Abstract**: In this paper, a new channel coding scheme for the narrowband powerline communication (PLC) system employing quasi-cyclic (QC) low-density parity-check (LDPC) codes of multiple code rate and multiple code length is proposed. Combined with proper constellation mappings, 4 transmission modes can be supported, which provides different error correction capabilities. A comparison with the coding scheme in the G3 specification is performed, confirming that the proposed scheme is superior to G3 in terms of both flexibility and reliability while the efficiency is higher than that of G3. A study on the implementation complexity on FPGA is also conducted to prove the feasibility of using the proposed scheme in future practical narrowband PLC system.

## A study on heterogeneous sensing and networking platform for Intelligent Transportation System

- **Status**: ❌
- **Reason**: ITS 센싱/네트워킹 플랫폼, LDPC 부수 적용으로 떼어낼 기법 없음.
- **ID**: ieee:5982835
- **Type**: conference
- **Published**: 4-8 July 2
- **Authors**: Jingfang Huang, Honggang Wang, Hong Liu
- **PDF**: https://ieeexplore.ieee.org/document/5982835
- **Abstract**: Intelligent Transportation System (ITS) is to utilize the sensing, networking and information technologies for transport applications such as road and vehicles. In ITS, various sensors and networks play a vital role in the capturing and transmitting of transportation information. Especially, wireless networks' spectrum resource and reliability are important factors in this system. In this paper, we firstly give a brief description of the techniques adopted by the Intelligent Transportation System, and we then study an heterogeneous sensing and networking platform for the ITS traffic monitoring applications. A case study on video camera based traffic monitoring is given and the adoption of Low-Density Parity-Check (LDPC) coding is proved to support reliable communication.

## Implementation of soft-decision forward error correction for 100G digital coherent system

- **Status**: ✅
- **Reason**: A/C 인접: LDPC(4608,4080) soft-decision FEC HW 에뮬레이터 구현, error floor/NCG 정량-디코더 HW 이식 여지로 살림.
- **ID**: ieee:6015196
- **Type**: conference
- **Published**: 4-8 July 2
- **Authors**: Kiyoshi Onohara, Yoshikuni Miyata, Kenya Sugihara +5
- **PDF**: https://ieeexplore.ieee.org/document/6015196
- **Abstract**: We discuss implementation and performance evaluation of LDPC(4608,4080) for 100Gb/s throughput by hardware emulator. We expect that an NCG of the LDPC code concatenated with enhanced FEC is 10.8 dB at a BER of 10-15.

## Adaptive use of two decode-and-forward algorithms in relay-enhanced wireless transmissions

- **Status**: ❌
- **Reason**: 릴레이 협력 채널코딩 적응 운용, 링크적응 분석으로 떼어낼 LDPC 디코더 기법 없음.
- **ID**: ieee:5982638
- **Type**: conference
- **Published**: 4-8 July 2
- **Authors**: Corina Bota, Anghel Botos, Mihaly Varga +1
- **PDF**: https://ieeexplore.ieee.org/document/5982638
- **Abstract**: The use of cooperative channel coding is shown to increase the performance of relay-assisted wireless transmissions. This paper analyzes the spectral efficiencies provided by two cooperative channel coding algorithms and by the non-cooperative transmission, while ensuring a coded-block error rate (BLER) smaller than a target value BLERt. The results obtained in the study are used to propose an adaptive employment of these transmission configurations, in order to provide the greatest spectral efficiency, while observing the BLERt constraint, in terms of the SNR values of the involved channels. The cooperation algorithm can be seen as a new dimension of link adaptation, besides the modulations and channel coding type and rate. The paper also analyzes the effects of the configuration's parameters, i.e. different modulations and coding rates used on the component links, upon the spectral efficiencies provided by the studied algorithms.

## A one-tap frequency domain modified RLS equalizer combined with LDPC decoder using channel state information in OFDM systems

- **Status**: ❌
- **Reason**: OFDM RLS 등화기+CSI 결합 응용, min-sum은 기존 그대로 사용하고 신규 디코더 기여 없음
- **ID**: ieee:6037283
- **Type**: conference
- **Published**: 26-30 July
- **Authors**: Yung-An Kao, Ming-Yuan Hsiao
- **PDF**: https://ieeexplore.ieee.org/document/6037283
- **Abstract**: In this paper, the low-density parity-check (LDPC) code decoder with aided channel state information (CSI) for an OFDM based system is presented. The CSI on different subcarriers are obtained from the one-tap frequency domain equalizer based on the modified RLS algorithm. Compare to channel estimation only by training symbols with the conventional decoding method of min-sum algorithm, the proposed method with the same decoding algorithm has better performance because it adopt the adaptive equalizer and decode with CSI which represent different reliabilities on subcarriers.

## Research of LDPC decoding based on LLR BP algorithm

- **Status**: ✅
- **Reason**: LLR BP 디코더 개선(Hybrid Check-Variable offset)으로 short LDPC 루프 대응, 바이너리 BP 이식 가능(C)
- **ID**: ieee:6037098
- **Type**: conference
- **Published**: 26-30 July
- **Authors**: Sun Zengyou, Zhang Jin, Du Juan
- **PDF**: https://ieeexplore.ieee.org/document/6037098
- **Abstract**: In iterations processing of LLR BP algorithm for LDPC decoding, the computational complexity of check nodes renewal is high; The decoding performance will be reduced, while there are lots of loops in short LDPC codes. According to these problems, a new LLR BP decoding algorithm based on Hybrid Check-Variable offset processing is proposed in this paper. Decrease complexity by adjusting amplitudes of check nodes processing and relativity of variable nodes by adjusting their information. Simulation results show that this improved decoding algorithm, compared with LLR BP algorithm, is consideration to decoding performance of LDPC and computational complexity.

## Adaptive decoding of LDPC codes based on cross-entropy

- **Status**: ✅
- **Reason**: cross-entropy 기반 적응 가중/정지기준으로 메시지패싱 수렴 가속, 바이너리 LDPC 디코더 이식 가능(C)
- **ID**: ieee:6037266
- **Type**: conference
- **Published**: 26-30 July
- **Authors**: Tso-Cho Chen
- **PDF**: https://ieeexplore.ieee.org/document/6037266
...
- **Status**: ❌
- **Reason**: 메모리용 Reed-Solomon 부호+곱셈알고리즘, 비-LDPC이고 LDPC BP 이식성 없음
- **ID**: ieee:6037272
- **Type**: conference
- **Published**: 26-30 July
- **Authors**: Liyi Xiao, Zheng Sun, Ming Zhu
- **PDF**: https://ieeexplore.ieee.org/document/6037272
- **Abstract**: With the increasing density of transistor and the probability of MBUs (Multiple Bits Upsets), Hamming code is not enough to provide required reliability in memories. Therefore multiple error correction codes are necessity. In this paper, Reed-Solomon (RS) codes are proposed to protect memories against MBUs which can achieve higher error correction capability (e.g. 8 bits). Besides a novel and low overhead multiplication algorithm is proposed and used in RS codes encoding and decoding process. At last, the encoder and decoder are implemented using Verilog HDL and validated through a number of simulations. The experiment results show that compared with other ECCs, RS code has higher error correction capability and lower area overhead.

## A blind watermarking algorithm using low density parity check coding for Mpeg-4 videos

- **Status**: ❌
- **Reason**: MPEG-4 비디오 워터마킹에 LDPC 부수 사용, 떼어낼 ECC 기법 없음
- **ID**: ieee:6001883
- **Type**: conference
- **Published**: 26-28 July
- **Authors**: Teng Hao, Yibiao Yu
- **PDF**: https://ieeexplore.ieee.org/document/6001883
- **Abstract**: A new digital watermarking algorithm for MPEG-4 video based on LDPC coding is proposed. Which embeds into and retrieves the digital watermark information out of the MPEG-4 video using LDPC coding technology and human visual characteristics. The watermark is encoded by LDPC at first and then embedded into the intermediate frequency coefficient of the luminance of the video's I-VOP. Furthermore, it is retrieved from the watermarked video by the LDPC iterative methods. Making full use of LDPC codes' error correction features can reduce the bit error rate in transmission process of the watermarking, and improve the anti-attack capability of the watermarking. Experimental results show that this algorithm greatly decrease the bit error rate of the retrieved watermark and is robust to the video attacking such as frame deleting ,frame switching ,frame average, recompression,et al. The watermark can be extracted blindly. As compared without LDPC watermarking, the performance of the algorithm is improved.

## Joint Network-Channel Coding in Distributed Antenna System

- **Status**: ❌
- **Reason**: joint network-channel coding 무선 릴레이 응용, LDPC는 베이스라인 채널코드일 뿐 떼어낼 ECC 기법 없음
- **ID**: ieee:5978911
- **Type**: conference
- **Published**: 25-27 July
- **Authors**: Jiafeng Liu, Tuanfa Qin, Kan Chang
- **PDF**: https://ieeexplore.ieee.org/document/5978911
- **Abstract**: In order to reduce high bit error rate (BER) over wireless multi-access channel with low signal to noise ratio (SNR), a joint network-channel coding algorithm for wireless relay communications is proposed. In our scheme, low density parity check (LDPC) is applied as channel code to work together with network coding in the relay node. At the destination nodes, joint network-channel decoding is applied, in which the channel message with higher SNR is chosen as the received information. Experimental results show that the network model proposed is able to support network coding well with low System BER. Compared with the traditional Distributed Antenna System (DAS) and the system without relay antenna, the new scheme decreases system BER by 47% and 34% on average.

## Some Results on Update Complexity of a Linear Code Ensemble

- **Status**: ❌
- **Reason**: linear code update complexity 순수 이론 bound, 디코더/HW/구성으로 안 이어짐
- **ID**: ieee:5979095
- **Type**: conference
- **Published**: 25-27 July
- **Authors**: Alan Jule, Iryna Andriyanova
- **PDF**: https://ieeexplore.ieee.org/document/5979095
...
## Ultra-narrow-spacing Tb/s long haul transmission using soft LDPC decoding with Gaussian processes

- **Status**: ❌
- **Reason**: Gaussian process 기반 등화+LDPC로 광통신 비선형 보상, LDPC는 베이스라인이고 떼어낼 ECC 기법 없음
- **ID**: ieee:6000076
- **Type**: conference
- **Published**: 18-20 July
- **Authors**: M. N. Sakib, O. Liboiron-Ladouceur
- **PDF**: https://ieeexplore.ieee.org/document/6000076
- **Abstract**: We investigate the performance of LDPC coding and Gaussian process based equalization for simultaneous compensation of linear and nonlinear effects. The proposed scheme can help to increase spectral efficiency and improve nonlinearity tolerance.

## An Improved Construction Method of QC-LDPC Codes Based on the PEG Algorithm

- **Status**: ✅
- **Reason**: PEG 기반 QC-LDPC 구성으로 girth +2, short cycle 제거 — 바이너리 QC-LDPC 코드설계 기법(E)
- **ID**: ieee:5990282
- **Type**: conference
- **Published**: 17-18 July
- **Authors**: Liqun Huang, Yuliang Wang, Ping Gong
- **PDF**: https://ieeexplore.ieee.org/document/5990282
- **Abstract**: Due to quasi-cyclic structure of parity-check matrices, Quasi-Cyclic Low-Density Parity-Check (QC-LDPC) codes reduce hardware requirements, and thus they have a higher practical value than random LDPC codes. This paper proposes an improved construction method of QC-LDPC based on PEG Algorithm, and the girth of the QC-LDPC codes has an increase of 2. Firstly, a base matrix is generated by the PEG algorithm, and then the base matrix is expanded using the unit circle permutation matrix, at the same time, the short loop of the base matrix is eliminated. This approach has the advantages of the PEG algorithm and the QC-LDPC codes, and simulation results show that bit error rate (BER) performance of the QC-LDPC codes presented in this paper is the same as or exceeds the one of randomly constructed LDPC codes.

## A low-complexity IRA-LDPC coded noncoherent unitary space-time modulation system on Rayleigh flat fading channel

- **Status**: ❌
- **Reason**: IRA-LDPC+공간시간변조 MIMO 무선 응용, 표준 BP만 사용, 떼어낼 신규 디코더/구성 없음
- **ID**: ieee:5988690
- **Type**: conference
- **Published**: 15-17 July
- **Authors**: Li Peng, Lingling Yang, Qiuping Peng
- **PDF**: https://ieeexplore.ieee.org/document/5988690
- **Abstract**: This letter constructs a practical low- complexity coded modulation MIMO system which concatenates the IRA-LDPC codes based on the permutation Q-matrix with the noncoherent unitary space-time modulation (USTM) based on sine-cosine function for Rayleigh flat fading channel. In the receiver, we adopt dual detecting scheme: first relative coarse low complexity ML detect and then relative precise MAP detect with shrinking search range of constellation points. Combining the dual detecting scheme with the Belief-Propagation (BP) iterative decoding algorithm comes into being an iterative feedback demodulate-decoding system. The IRA-LDPC coded modulation system can obtain about 15 dB coding gain at 10-5 BER over the uncoded USTM system. Some results are based on simulation testing.

## Simulation research of wireless communication system of LDPC code based on the adaptive modulation and precoding joint optimization technology

- **Status**: ❌
- **Reason**: 무선 adaptive modulation+precoding 응용, LDPC는 부수 베이스라인이고 떼어낼 ECC 기법 없음
- **ID**: ieee:5987977
- **Type**: conference
- **Published**: 15-17 July
- **Authors**: Zhi-hua Yang, Shao-qing Zhang, Kai Zhang
- **PDF**: https://ieeexplore.ieee.org/document/5987977
- **Abstract**: In order to improve the wireless communication system reliability and throughput, the adaptive modulation and precoding joint optimization wireless communication system model is designed. Simulated experiments of this joint optimization wireless communication system model are done in VC++ environment. The experimental results show that the designed wireless communication system of LDPC code based on the adaptive modulation and precoding joint optimization technology can considerably increase wireless transmission system throughput, and has Lowerber BER than Higher-order modulation precoding wireless communication system.

## A multi-level Hierarchical Quasi-Cyclic matrix for implementation of flexible partially-parallel LDPC decoders

- **Status**: ✅
- **Reason**: 새 다층 Hierarchical QC 행렬 구성+partially-parallel FPGA 디코더 아키텍처, 바이너리 QC-LDPC 코드설계+HW 이식 가능(D/E)
- **ID**: ieee:6011832
- **Type**: conference
- **Published**: 11-15 July
- **Authors**: Vikram Arkalgud Chandrasetty, Syed Mahfuzul Aziz
- **PDF**: https://ieeexplore.ieee.org/document/6011832
...
## Low-density parity-check codes used for polarization mode dispersion mitigation in 40-Gb/s optical fiber system

- **Status**: ❌
- **Reason**: 광섬유 PMD 완화에 표준 LDPC(2040,1903) 적용·RS와 성능비교, 새 디코더·구성·HW 기여 없는 응용
- **ID**: ieee:6206888
- **Type**: conference
- **Published**: 11-14 July
- **Authors**: Lixia Xi, Haoran Chen, Minliang Li +2
- **PDF**: https://ieeexplore.ieee.org/document/6206888
- **Abstract**: Instead of Reed-Solomon (RS) code, Low-density parity-check (LDPC) codes are used with distributed fast polarization scramblers (FPSs) for polarization mode dispersion (PMD) mitigation in 40-Gb/s optical fiber system. The scrambling speed of FPS for LDPC (2040, 1903) codes system is discussed, the optimal speed 10MHz is obtained. The correction performance of LDPC codes are compared with RS codes. Results show the LDPC code can achieve a better performance than the RS (255, 239) code at a similar redundancy with soft decoding.

## Soft In Soft Out Reed Solomon decoding

- **Status**: ❌
- **Reason**: Reed-Solomon SISO 디코더, 비-LDPC 부호이고 RS 전용 SISO 기법이라 바이너리 LDPC BP 이식성 없음
- **ID**: ieee:6020920
- **Type**: conference
- **Published**: 11-13 July
- **Authors**: Anum Ali
- **PDF**: https://ieeexplore.ieee.org/document/6020920
- **Abstract**: The aim of this project was to investigate and implement Soft In Soft Out decoder based on Reed Solomon Codes. This type of error correction is widely used in data communication applications, such as Digital Video Broadcast (DVB), optical carriers and Space Data Systems (CCSDS) primary choice is RS Codes by Consultative committee for deep space communications signal error correction.
