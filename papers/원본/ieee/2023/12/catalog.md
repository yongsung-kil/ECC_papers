...
- **Status**: ✅
- **Reason**: 고열중치 코드용 최적화 scaled Min-sum + 2단 스케일링/HW 아키텍처(C/D) — McEliece 응용이나 디코더·HW 기법 NAND 이식 가능
- **ID**: ieee:10274562
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Jiaxuan Cai, Xinmiao Zhang
- **PDF**: https://ieeexplore.ieee.org/document/10274562
...
- **Status**: ✅
- **Reason**: 확장 min-sum LDPC 동적 체크노드 스케줄링(C) — 이식 가능 디코더 알고리즘 신규 기여, 바이너리
- **ID**: ieee:10196364
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Chi Zhang, Xiaoxi Yi, Guicai Yu
- **PDF**: https://ieeexplore.ieee.org/document/10196364
- **Abstract**: With the construction process of smart cities, cyber-physical systems (CPS) are going to become a link to connect all things with intelligence. The low-density parity check (LDPC) code is an import information encryption format in computer networks. In this article, from the perspective of LDPC code decoding algorithm scheduling strategy, we design a dynamic check node scheduling (DCNS) strategy for extended min-sum LDPC decoding in cyber-physical systems. In this strategy, the reliability of the node is checked to determine whether to update. For the un-updated variable node, it is optimized according to the results of real-time scheduling. Compared with the flooding mechanism and LBP scheduling of the usual algorithms, this proposed scheduling strategy improves the error correction performance by selecting some nodes for decoding and performing message optimization, while not significantly increasing the decoding computation. Especially, compared with the LBP, the bit error rate and frame error rate of the proposed algorithm are reduced by 48.3% and 53.2% on average in the finite domain 64F range. Compared with BP, the bit error rate and frame error rate of the proposed algorithm are reduced by 11.3% and 13.2% on average in the finite domain 64F range.

## Iterative Receiver for Uplink MIMO-PDMA System and Its Optimization

- **Status**: ❌
- **Reason**: 비이진(non-binary GF(q)) LDPC 사용 — 즉시 제외
- **ID**: ieee:10183354
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Qingqing Liu, Jin Xu, Hanqing Ding +3
- **PDF**: https://ieeexplore.ieee.org/document/10183354
- **Abstract**: In this article, the baseband signal model of multiple-input multiple-output pattern division multiple access (MIMO-PDMA) system with pattern matrix constructed by the Kronecker product is established first. Secondly, for high-dimensional MIMO-PDMA with high-order modulation systems, a factor graph-based real-valued expectation-propagation (FGREP) algorithm is proposed as the low-complexity front-end detector. Then, non-binary low-density parity-check (LDPC) instead of its binary counterpart is employed as error correcting code to enhance the receiver performance. As a result, it is combined with the FGREP-detector to develop an iterative detection and decoding receiver, named FGREP-IDD. In addition, in order to further improve the iterative detection performance of this kind of three-stage receiver, a symbol-wise extrinsic information transfer (SEXIT)-based optimization algorithm is employed to search for a near-optimal degree distribution for the concatenated non-binary LDPC. To calculate the SEXIT of the component decoder for optimization, it is critical to calculate the average mutual information of the FGREP-detector which is time consuming by means of the traditional method. In this work, a modified interpolation method that employs vector-valued increment instead of scalar-valued one is proposed to accelerate the calculation. The SEXIT obtained by the proposed method is almost coincide with the one obtained by numerical simulation. At last, SEXIT chart based analysis and simulation results verified the above idea.

## Robust Semantic Communications With Masked VQ-VAE Enabled Codebook

- **Status**: ❌
- **Reason**: 시맨틱 통신 VQ-VAE 코드북, 채널코딩 ECC/LDPC 기법 없음 — 제외
- **ID**: ieee:10101778
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Qiyu Hu, Guangyi Zhang, Zhijin Qin +3
- **PDF**: https://ieeexplore.ieee.org/document/10101778
...
- **Status**: ❌
- **Reason**: STSK-OTFS 변조·다중접속용 ML 다중사용자 검출기 — 무선 변조/검출 특이적, 떼어낼 LDPC ECC 기법 없음
- **ID**: ieee:10250854
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Zeping Sui, Hongming Zhang, Sumei Sun +2
- **PDF**: https://ieeexplore.ieee.org/document/10250854
...
- **Status**: ❌
- **Reason**: Frameless ALOHA 매체접속 프로토콜의 throughput·AoI 분석 — LDPC ECC와 무관
- **ID**: ieee:10225599
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Andrea Munari, Francisco Lázaro, Giuseppe Durisi +1
- **PDF**: https://ieeexplore.ieee.org/document/10225599
...
- **Status**: ❌
- **Reason**: frameless ALOHA AoI 최적화 랜덤접속 프로토콜, ECC/LDPC 기법 없음 — 제외
- **ID**: ieee:10123405
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Yuhao Huang, Jian Jiao, Ye Wang +4
- **PDF**: https://ieeexplore.ieee.org/document/10123405
- **Abstract**: In this paper, we focus on the optimal problem of average age of information (AAoI) in grant-free massive access, and propose an age-critical frameless ALOHA (ACFA) random access protocol, where the AAoI is implicitly reduced by banning the transmission of activated user equipments (UEs) recovered successfully in the last frame. In particular, we analyze the dense and sparse access models according to the activation probability, and present these scenarios with time-stamped sampling either at the beginning of the frame or in the first slot transmitting the packet. In order to qualify the AAoI of proposed protocol, we define two virtual rates and establish an iterative framework to analyze the access successful probability (ASP) of the protocol in asymptotic regime, and derive the closed-form expressions of AAoI as a function of ASP and virtual rate in all cases. Further, we formulate the optimal problems of normalized AAoI in all cases, and obtain the selection of access parameters by asymptotic analysis and simulations, respectively. Finally, we compare our protocol with state-of-the-art schemes, and the simulation results show that the ACFA random access protocol outperforms these benchmark schemes, and has great potential of access-banned policy in minimizing AAoI for frame-based protocols.

## Belief-Selective Propagation Detection for MIMO Systems

- **Status**: ❌
- **Reason**: MIMO 검출용 BP detector(BsP) 개선 — 채널검출기이지 LDPC ECC 디코더가 아니고 NAND로 떼어낼 부호/디코더 기법 없음
- **ID**: ieee:10223704
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Wenyue Zhou, Yifei Shen, Liping Li +3
- **PDF**: https://ieeexplore.ieee.org/document/10223704
...
- **Status**: ❌
- **Reason**: 시맨틱 통신 rate-distortion 이론·신경망 추정 — LDPC ECC 무관
- **ID**: ieee:10387242
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Dongxu Li, Jianhao Huang, Chuan Huang +3
- **PDF**: https://ieeexplore.ieee.org/document/10387242
...
## Design and Performance Evaluation of Polar Coding for BICM Systems With Phase Noise

- **Status**: ❌
- **Reason**: 위상잡음 BICM용 Polar 부호 설계 — Polar 코드/SC·SCL, LDPC가 아니라 제외
- **ID**: ieee:10292651
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Qian Wang, Wenqiang Ma, Ling Liu +3
- **PDF**: https://ieeexplore.ieee.org/document/10292651
- **Abstract**: This letter addresses the issue of polar coding design for bit-interleaved coded modulation (BICM) systems over the phase noise channel. Specifically, the existence of phase reference error (PRE) due to the imperfect phase estimation is considered. The generalized mutual information (GMI) for BICM systems with the PRE considered is first analyzed, using the exact statistical information of the received amplitude and phase, which is a foundation for the coding and decoding. Then based on the calculated average GMI per bit layer for a given PRE variance and signal-to-noise ratio (SNR), we can iteratively calculate the reliabilities of the polarized subchannels to construct polar codes. Finally, the superior performance of the phase noise-matched polar design is verified by comparing it with that of the channel-mismatched design, with 8-ary constellations used for simplicity. In addition, the comparison of the decoding performance between successive cancellation (SC) and successive cancellation list (SCL) algorithms is provided. Detailed numerical results on both frame error rate and bit error rate are given with respect to different PRE variances and SNR values. Note that this proposed scheme can be generalized to arbitrary two-dimensional high-order constellations.

## A Sliding-Window LMMSE Turbo Equalization Scheme for OTFS

- **Status**: ❌
- **Reason**: OTFS용 LMMSE 터보 등화기 sliding-window 복잡도 절감 — 등화기 기법, LDPC 디코더/구성 기여 없음
- **ID**: ieee:10288504
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Franz Lampel, Alex Alvarado, Frans M. J. Willems
- **PDF**: https://ieeexplore.ieee.org/document/10288504
- **Abstract**: This letter discusses a recently proposed linear minimum mean square error (LMMSE) turbo equalization scheme for OTFS. The complexity of the LMMSE-based equalizer, measured by the number of complex multiply-and-addition operations, scales with the cube of the number of transmitted symbols in an OTFS frame. To lower the complexity, we propose to limit the number of symbols in the equalization process by introducing a sliding window and exploiting the periodicity properties of the delay-Doppler domain. We demonstrate that for a practical channel model, which introduces fractional delay and Doppler shifts, even a narrow sliding window provides a bit error rate (BER) performance within  $ {\mathrm {0.17~ \text {dB}}}$  compared to the unconstrained LMMSE-based turbo equalizer. At the same time, the complexity of the sliding window LMMSE turbo equalizer is reduced by a factor of 10. The BER gap becomes even smaller with increasing window size, allowing for a complexity/performance trade-off.

## Index and Maximum Distance Separable Modulation

- **Status**: ❌
- **Reason**: Index+MDS 변조 개념과 OFDM 검출기 — 변조/MDS 부호 기반, LDPC ECC 기여 없음
- **ID**: ieee:10283875
- **Type**: journal
- **Published**: Dec. 2023
- **Authors**: Yongjia Zhu, Yuyao He, Rugui Yao +2
- **PDF**: https://ieeexplore.ieee.org/document/10283875
- **Abstract**: This letter proposes a novel index and maximum distance separable (IMDS) modulation concept, in which both the index of multiple disjoint constellations and the classical amplitude and phase modulation (APM) symbols are used to convey information. In this regard, the signal constellation is firstly determined based on the index bits, and the other information bits are encoded to an MDS codeword, which is further mapped to symbols from the indexed constellation. A practical scheme using the IMDS concept and orthogonal frequency division multiplexing (OFDM) is also proposed. In addition, an efficient low-complexity detector is designed for OFDM-IMDS. Furthermore, the bit error rate (BER), diversity order and spectral efficiency (SE) performance are analyzed. The analysis and numerical results show that OFDM-IMDS outperforms than OFDM-MDS, OFDM-IM and other existing OFDM schemes on SE and BER.

## Machine Learning Structure for Box-plus Operation with Soft Information Processing

- **Status**: ✅
- **Reason**: C: CND box-plus 연산의 ML 구조 제안 — 디코더 체크노드 연산 이식 가능
- **ID**: ieee:10382042
- **Type**: conference
- **Published**: 8-9 Dec. 2
- **Authors**: Okzata Recy, Khoirul Anwar, Gelar Budiman
- **PDF**: https://ieeexplore.ieee.org/document/10382042
- **Abstract**: This paper studies machine learning (ML) structure for box-plus operation as an element of future error correction based on artificial intelligence (AI) using soft information processing. Box-plus operations are important components for check-node (CND) of error correction codes based on bipartite graphs. We propose an ML structure for box-plus operation and observe the behavior of the structure by investigating all possible structures constructed from 6 neurons in the hidden layer. We utilize activation function: (i) hyperbolic tangent (tanh) function for the hidden layer since it maps any real-valued input to the 1 to 1 range for soft information, and (ii) linear function only at the output layer. We evaluate the performances of the proposed structure using a series of computer simulations. We found that a single hidden layer with many neurons is better than multiple hidden layers with less number of neurons, because the box-plus operation is not too complex. We expect that these results open new ground for the development of future error correction codes based on ML.

## Multi-Hop Teleportation Protocols for Quantum Networks Privacy in Quantum Key Distribution

- **Status**: ❌
- **Reason**: 양자 텔레포테이션 멀티홉 라우팅·QKD, 고전 바이너리 LDPC 기법 없음
- **ID**: ieee:10456103
- **Type**: conference
- **Published**: 8-9 Dec. 2
- **Authors**: Darshan Nyati, Rakshit Kothari, Mayank Patel +1
- **PDF**: https://ieeexplore.ieee.org/document/10456103
- **Abstract**: This study examines the development of wireless quantum communication network and advance quantum com- munication. In this paper we present the idea of system scheme to transfer quantum states in the wireless quantum communication network and advance. Quantum communication is a rapidly evolving field that will significantly impact the twenty-first century. Wireless quantum communication networks transport quantum states using a multi-hop teleportation quantum routing approach. This technique does not require compatibility with classical paths, as quantum states are transported directly along the designated path. The chance of successfully transferring a quantum state is determined using statistics. Researchers analyze statistics to optimize network performance and develop strategies for quantum state transfer success. Advancements in error correction techniques enhance the reliability of quantum communication networks. A quantum state is moved from the source to the destination using direct multi-hop teleportation on the chosen path. Statistics are used to determine the likelihood of successfully transmitting the quantum state.

## Reversible Data Hiding in Encrypted Images Using Four-Color Approach

- **Status**: ❌
- **Reason**: 암호화 이미지 가역 데이터 은닉(RDHEI), ECC 아님
- **ID**: ieee:10367151
- **Type**: conference
- **Published**: 8-14 Dec. 
- **Authors**: Zi-Long Liu, Chi-Man Pun
- **PDF**: https://ieeexplore.ieee.org/document/10367151
...
## A Multi-parameter Compensated Min-Sum Based LDPC Decoding Algorithm

- **Status**: ✅
- **Reason**: C: min-sum 변형(다중파라미터 보상 MCMS) 새 디코더 알고리즘 제안 — NAND 이식 가능
- **ID**: ieee:10507301
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Shuxun He, Cheng Wang, Songsong Cai +1
- **PDF**: https://ieeexplore.ieee.org/document/10507301
- **Abstract**: The Minimum Sum (MS) algorithm is a standard Low-Density Parity-Check Codes (LDPC) decoding algorithm. Compared with traditional algorithms, it uses approximate calculations to reduce the computational complexity of the algorithm when calculating parity nodes, resulting in performance loss issues. To improve the LDPC decoding performance, the LDPC decoding algorithm of Multi-parameter Compensated Min-Sum (MCMS) is proposed. This algorithm introduces relevant information, such as channel and code length, into calculating update verification nodes. Introducing multiplication and correction factors to reduce errors in validation node calculations through multi-parameter compensation. Simulation results show that the MCMS algorithm has superior error rate performance compared with the MS algorithm.

## Probability Shaping for High-Order Systems with DSC-LDPC Codes

- **Status**: ❌
- **Reason**: DSC-LDPC 기반 JSCC + probability shaping; LDPC는 베이스라인, 떼어낼 ECC 기법 없음
- **ID**: ieee:10507715
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Jichen He, Qiufang Lian, Chen Chen +2
- **PDF**: https://ieeexplore.ieee.org/document/10507715
- **Abstract**: The research on joint source-channel coding (JSCC) systems primarily employs low-order BPSK modulation. However, such low spectral efficiency modulation schemes cannot meet the demands of future high-speed communication. In real-world scenarios, source probabilities are often unequal. Therefore, introducing high-order modulation with a probability shaping model, combined with dual spatially coupled low-density parity-check (DSC-LDPC) codes, can better exploit system performance and further increase throughput.Utilizing the remaining source redundancy to achieve amplitude shaping gains, a novel approach called joint source-channel coded modulation (JSCCM) is proposed, serving as a solution based on probabilistic amplitude shaping (PAS). This paper suggests the combination of quadrature amplitude modulation (QAM) modulation with a DSC-LDPC-JSCC system and examines whether probability shaping techniques can further enhance the system.

## Low Complexity Deep Learning Method for Base Graph of Protograph LDPC Decoding

- **Status**: ✅
- **Reason**: protograph LDPC base graph 기반 신경망 디코더로 파라미터 절감, 이식 가능 디코더(C)
- **ID**: ieee:10507610
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Shuo Liang, Xingcheng Liu, Lin Zhang
- **PDF**: https://ieeexplore.ieee.org/document/10507610
- **Abstract**: Deep learning-based decoding algorithms have emerged as a focal point currently in contemporary research. Neural Offset Min-Sum Belief Propagation decoding algorithm is one of the traditional deep learning methods rooted in model-driven approaches. However, with the gradual increase in codeword length, there is an inevitable surge in the training cost and time complexity of such kind of deep learning algorithms. Addressing this issue, a novel deep learning algorithm based on the structural characteristics of the base graph in protograph LDPC codes is proposed. By strategically training the base graph and introducing a unique bit decoding allocation strategy, we endeavor to harmonize the Deep Learning algorithm with the intrinsic structure of the base graph itself. This approach contrasts favorably with several existing methods that utilize conventional segmental deep learning decoding algorithms for protograph LDPC codes. Simulation results demonstrate that, compared to other model-driven based deep learning algorithms, the proposed algorithm significantly reduces the number of parameters required for storage and training, while the loss in decoding performance remains within acceptable bounds.
...
- **Status**: ✅
- **Reason**: RF/FSO 응용이나 DL 기반 LDPC 디코더 제안; 애매하여 in으로 살림(Phase 3 재검토)
- **ID**: ieee:10507665
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Ibrahima Gueye, Idy Diop, Ibra Dioum +1
- **PDF**: https://ieeexplore.ieee.org/document/10507665
...
## An Improved Syndrome Bit Flipping Decoder with Narrowed Flipping Set for LDPC Codes

- **Status**: ✅
- **Reason**: 새 Syndrome Bit Flipping 디코더(narrowed flipping set)로 이식 가능한 디코더 알고리즘(C)
- **ID**: ieee:10507492
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Wenlin Zhao, Jie Zhang, Li Li +1
- **PDF**: https://ieeexplore.ieee.org/document/10507492
- **Abstract**: In this paper, we propose an improved Syndrome Bit Flipping (SBF) decoding algorithm that utilizes narrowed flipping set. The narrowed flipping set is prone to contain all error bit positions with high probability. To construct the narrowed flipping set, part of its elements are selected according to channel information, and the rest are constructed by random selections. The narrowed flipping set mitigates possible error propagation by avoiding some non-error bits being unnessary flipped, thereby further improving the decoding performance of the original SBF algorithm. The simulation results show that the proposed algorithm can achieve about 1.2 dB bit error rate gain compared with the original SBF algorithm, with a modest increase in computational complexity.

## Novel Construction and Decoding of Polar Product Codes with SPC Codes

- **Status**: ❌
- **Reason**: Polar product codes + SPC; LDPC 아님
- **ID**: ieee:10507716
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Yue Xu, Ming Jiang, Lijie Hu +1
- **PDF**: https://ieeexplore.ieee.org/document/10507716
- **Abstract**: This paper proposes a novel product coding and decoding method for polar codes by choosing the single parity-check (SPC) codes as the second component codes. The study explores both the systematic and non-systematic forms of product codes, based on different coding methods of SPC codes. Moreover, a bit-flipped iterative decoding method of the product codes is proposed. The simulation results show that there is a gain of 0.3∼0.8 dB compared with transmitting polar codes directly. Additionally, the proposed scheme exhibits more robust performance gains with a reduction in computational complexity of over 60% when compared with the classic polar product codes.

## An Efficient Puncturing Scheme for Polar Codes Based on Fixed Information Set

- **Status**: ❌
- **Reason**: Polar codes puncturing scheme; not LDPC and no transferable LDPC technique
- **ID**: ieee:10507424
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Zijian Qin, Yangcan Zhou, Jingjing Zhu +1
- **PDF**: https://ieeexplore.ieee.org/document/10507424
- **Abstract**: For traditional polar codes, the code length is limited to a power of two, due to the use of the 2×2 kernel matrix. To obtain flexible code lengths, puncturing is widely considered for polar codes. In this paper, we propose an efficient puncturing scheme based on fixed information set (FIS) of the mother code. First, we estimate the qualities of bit channels using the Gaussian approximation after puncturing. In addition, the search scope of punctured bits in the codeword is significantly reduced to the number of frozen bits. Furthermore, a computationally efficient method for selecting the most promising puncturing pattern is employed. Moreover, a low-complexity recursive method is leveraged to construct the puncturing pattern. Simulation results demonstrate that the proposed puncturing scheme outperforms the state-of-the-art FIS-based puncturing schemes remarkably in terms of error-correction performance.

## Reliability-Based HARQ: Performance Analysis and Segmented Optimization

- **Status**: ❌
- **Reason**: Reliability-Based HARQ 재전송 프로토콜; LDPC 디코더/코드설계 기여 아님
- **ID**: ieee:10507701
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Lingjie Zhou, Li Chen, Huarui Yin
- **PDF**: https://ieeexplore.ieee.org/document/10507701
- **Abstract**: Reliability-Based hybrid automatic repeat request (RB-HARQ) scheme can reduce the amount of retransmission bits by retransmitting the most unreliable bits. In this work, we provide an approximate method for calculating the BER of retransmission frame of RB-HARQ scheme. Further, in order to lower the feedback overhead, we propose Segmented Reliability-Based HARQ (SRB-HARQ), choosing the most unreliable bit in every segment to retransmit, and the performance analysis is also given for SRB-HARQ. Both the theoretical analysis and simulation results can prove that the SRB-HARQ can reduce the feedback overhead and provide similar frame error rate (FER) of RB-HARQ at the same time.

## Multi-Way Compression for Channel Neural Decoding with Quantization

- **Status**: ✅
- **Reason**: C/D: 신경망 채널 디코더 양자화+압축(TR분해·가중치공유), LDPC 포함, HW 경량화 이식 가능
- **ID**: ieee:10507411
- **Type**: conference
- **Published**: 8-11 Dec. 
- **Authors**: Yuanhui Liang, Chan-Tong Lam, Qingle Wu +2
- **PDF**: https://ieeexplore.ieee.org/document/10507411
...
- **Status**: ✅
- **Reason**: Stochastic computing 기반 LDPC BP 디코더 컴파일 — SC 디코더 HW 구현 기법 이식 가능성 (C/D), 애매하여 보존
- **ID**: ieee:10386131
- **Type**: conference
- **Published**: 5-6 Dec. 2
- **Authors**: Alex Ilov, Alexandru Paler
- **PDF**: https://ieeexplore.ieee.org/document/10386131
- **Abstract**: Stochastic computing (SC) is executed on bitstreams which encode probabilities into the ratio between the number of one bits and the length of the stream. SC is successfully applied, for example, to image processing and beliefpropagation (BP) of low-density parity-check (LDPC) codes. With increased interest into SC, there is a gap with respect to the available scalable design automation tools needed for the fast prototyping of SC circuits. We bridge this gap by starting from the BitSAD.jl library, and implement data structures and methods to achieve orders of magnitude speed-ups for the compilation of SC circuits. We demonstrate the speedups by compiling BP decoders for LDPC codes. Our improvements enable the compilation of circuits which were previously out of reach for the tools. Our methods pave the way towards scalable design automation of SC.

## A High Flexibility Decoding Structure with High Hardware Utilization Efficiency for QC-LDPC

- **Status**: ✅
- **Reason**: QC-LDPC 디코더 HW: 레이어 내 노드 재배치 동적계획법으로 메모리 충돌 해소+높은 HW 활용률, NAND LDPC 디코더에 이식 가능(D)
- **ID**: ieee:10465058
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Zhenliang Yet, Xiongfei Zhai, Jinghong Mot +1
- **PDF**: https://ieeexplore.ieee.org/document/10465058
- **Abstract**: Due to the excellent performance of the quasi-cyclic low-density parity-check (QC-LDPC) code, it has attracted a lot of attention in communications and storage recently. To achieve high flexibility, some flexible 5G LDPC decoders are reported. However, in the existing work, the resources are underutilized for short codes, which yields the poor processing rate and resource utilization. In this paper, we propose a highly flexible decoding structure with high hardware utilization efficiency (HUE) that can be applied to multiple various code lengths simultaneously. Besides, a dynamic planning method for node reordering within layers is developed, which can perfectly solve the memory access conflict problem. The experimental results show that the proposed structure can achieve significant improvement in hardware utilization compared with the existing works.

## Convergence Optimized Joint Detection and Decoding for LDPC Coded SC-MIMO Systems

- **Status**: ✅
- **Reason**: LDPC 결합 검출·복호의 dynamic message damping·layered message updating 수렴 최적화; BP 메시지 갱신 기법은 NAND LDPC 디코더로 이식 가능(C)
- **ID**: ieee:10437755
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Xuanbo Zhang, Kaining Han, Jianhao Hu
- **PDF**: https://ieeexplore.ieee.org/document/10437755
- **Abstract**: With the increasing requirement for high-speed transmission, multiple input multiple out (MIMO) technologies have been proposed. Beneficial from low peak-to-average power ratio (PAPR), single carrier (SC) MIMO systems are suitable for long-distance transmission. However, one of the significant challenges of SC-MIMO systems is the vast complexity of channel equalization and signal detection, especially for the inter-symbol interference (ISI) caused by the frequency-selective fading channels. Due to acceptable complexity and near maximum likelihood (ML) detection performance, the Gaussian approximate interference (GAI) based belief propagation (BP) algorithm has attracted lots of attention. With the help of extrinsic information feedback from low-density parity check (LDPC) codes, iterative detection and decoding (IDD) scheme has been proposed, which has a near to approximate maximum a posterior (MAP) detection performance. Nevertheless, the IDD scheme still suffers from relatively slow convergence resulting from complex connections on the factor graph. Combined with the joint detection and decoding (JDD) scheme and two optimization methods, namely dynamic message damping and layered message updating, a convergence optimized joint detection and decoding (CO-JDD) algorithm is proposed in this paper. Evaluation results show that the proposed CO-JDD algorithm has 2dB bit error rate (BER) performance gains compared with IDD scheme. Besides, the proposed CO-JDD algorithm also has a 70% iteration reduction compared with the IDD scheme.

## Quaternary-Binary Message-Passing Decoder for Quantum LDPC Codes

- **Status**: ❌
- **Reason**: QLDPC quaternary 스태빌라이저 그래프 디코더 — 양자 전용 스태빌라이저/CSS 개념에 의존하는 quaternary BP, 바이너리 이식 부적합
- **ID**: ieee:10436874
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Dimitris Chytas, Nithin Raveendran, Asit Kumar Pradhan +1
- **PDF**: https://ieeexplore.ieee.org/document/10436874
...
- **Status**: ❌
- **Reason**: wiretap 채널 보안성능 bound; LDPC는 5G 표준코드 사용 언급뿐, ECC 디코더·구성 기여 없음
- **ID**: ieee:10437478
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Johannes Voichtleitner, Moritz Wiese, Anna Frank +1
- **PDF**: https://ieeexplore.ieee.org/document/10437478
- **Abstract**: This paper compares different algorithms to check semantic security on AWGN wiretap channels. Each algorithm provides upper and lower bounds on the performance of an attack strategy that is close to the best attack strategy. The advantage of these algorithms is that they have lower computational complexity compared to the best attack strategy. We also show that the proposed algorithms can be further improved by including cyclic redundancy check bits and parity check bits generated in the algorithms, for example, when polar codes or LDPC codes according to the 5G standard are used in the coding layer. Finally, we show the compatibility of the algorithms for both polar codes and LDPC codes.

## Adaptive Symbol Mapping for QC-LDPC Coded BICM HARQ Systems

- **Status**: ❌
- **Reason**: QC-LDPC BICM-HARQ의 심볼매핑·RCA-DE 분석; 매핑·변조 최적화로 NAND LDPC 부호설계·디코더에 떼어낼 기여 없음
- **ID**: ieee:10464817
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Hongsil Jeong, Min Jang, Seho Myung +3
- **PDF**: https://ieeexplore.ieee.org/document/10464817
...
- **Status**: ❌
- **Reason**: 확산모델 기반 채널 디노이징 + JSCC 시맨틱 통신 — LDPC는 베이스라인 비교용, 이식 기법 없음
- **ID**: ieee:10436728
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Tong Wu, Zhiyong Chen, Dazhi He +4
- **PDF**: https://ieeexplore.ieee.org/document/10436728
...
- **Status**: ❌
- **Reason**: 비동기 unsourced random access의 그래프 기반 충돌 해소 기법; LDPC는 부수 언급, NAND ECC로 떼어낼 디코더·코드설계 기여 없음
- **ID**: ieee:10437166
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Tianya Li, Yongpeng Wu, Wenjun Zhang +2
- **PDF**: https://ieeexplore.ieee.org/document/10437166
...
- **Status**: ❌
- **Reason**: 비이진(non-binary/finite-field) LDPC 기반 UMA 코딩 — 비이진 LDPC는 즉시 제외
- **ID**: ieee:10437578
- **Type**: conference
- **Published**: 4-8 Dec. 2
- **Authors**: Xinyi Sui, Zhongwei Si, Jincheng Dai
- **PDF**: https://ieeexplore.ieee.org/document/10437578
- **Abstract**: Unsourced multiple access (UMA) is a promising technique for the Internet-of-Things systems with a large number of users. In this paper, we explore the good algebraic properties of finite fields and propose a non-binary coding scheme for the UMA. The scheme is based on the slotted structure and T-fold ALOHA, and each active user randomly selects one slot to transmit its message. Finite-field operations are performed for both the channel coding and the multiple access. Compared to binary-field operations, the finite-field sparse spreading and multiuser superposition lead to improved diversity and less interference on each channel use. In addition, the non-binary LDPC code boosts the performance under short/moderate code length. We derive the joint belief-propagation decoding algorithm for the concatenated finite-field factor graph, which is then verified by the extrinsic information transfer (EXIT) analysis. The numerical results in terms of energy-efficiency performance are provided, which clearly demonstrate the superiority to the existing schemes in the region with large user numbers. The proposed scheme has the potential to carry out larger-scale user connections, and the energy-efficiency performance can be further improved by optimizing the coding parameters.

## Practical Wiretap Code Design by Concatenated Autoencoder and LDPC Codes

- **Status**: ❌
- **Reason**: Wiretap 보안 코드(autoencoder+LDPC 연접); LDPC는 표준 내부코드로 사용, 떼어낼 새 디코더/구성 기법 없고 보안 응용 특이적
- **ID**: ieee:10375067
- **Type**: conference
- **Published**: 4-7 Dec. 2
- **Authors**: Shangxuan Lyu, Ramprasad Raghunath, Luca Kunz +3
- **PDF**: https://ieeexplore.ieee.org/document/10375067
- **Abstract**: Practical wiretap code design for reliable and confidential communication is a difficult multi-objective task. In literature, neural networks (NN) have been designed as channel encoders and decoders. Our previous work shows that an autoencoder by NN can construct a wiretap encoder and decoder used at the legitimate transmitter and receiver, respectively. In particular, the choice of the reward function provides us with high flexibility to attain the reliability-secrecy tradeoff. However, due to the high training complexity, designing a sufficiently long wiretap code to reach an acceptable error probability performance is challenging. In this work, we construct a concatenated wiretap code, where the inner code consists of a low-density parity check code, and the outer code is learnt by an autoencoder. Numerical results demonstrate that for both (204,4) and (572,4) concatenated codes, the block error rate can be significantly reduced to around 10-4, rendering the code practically useful while maintaining a low level of information leakage at around 2.66 bits out of 572 channel-use.

## New Polar code for 5G New Radio Network

- **Status**: ❌
- **Reason**: 5G NR polar code 설계; LDPC 아님, NAND 이식 가능 기법 없음
- **ID**: ieee:10479335
- **Type**: conference
- **Published**: 4-7 Dec. 2
- **Authors**: Ibtissem Brahmi, Emna Hajlaoui, Souhir Elleuch +2
- **PDF**: https://ieeexplore.ieee.org/document/10479335
- **Abstract**: Over the past decade, polar codes have gained momentum in academia and industry. They are selected for the control channel in the 3rd Generation Partnership Project (3GPP) for the 5th generation (5G) New Radio (NR) standard for mobile communications. To this end, the present work proposes a new polar code for ultra-reliable and low latency (URLLC) use case of 5G NR. Since polar codes are known by their performance in terms of error correction, two scenarios are considered. The first is interested in the evolution of Block Error Rate (BLER) as a function of downlink Signal Noise Ratio (SNR) while using Quadrature phase shift keying (QPSK) modulation on an Additive White Gaussian Noise (AWGN) channel. The second scenario is interested in the evolution of BLER as a function of uplink SNR while using QPSK modulation on an AWGN channel. Simulation results prove the performance of the new polar code type for 5G NR networks, especially in terms of throughput and BLER.

## Intelligent Hybrid Channel Coding Schemes in 5G Device Centric Mobile Communication System

- **Status**: ✅
- **Reason**: ML 기반 LDPC/polar 디코딩 알고리즘 리뷰 및 신경망 디코더 제안; 이식 가능 디코더(C) 관점 애매, Phase 3 재검토
- **ID**: ieee:10435853
- **Type**: conference
- **Published**: 4-5 Dec. 2
- **Authors**: Madhavsingh Indoonundon, Tulsi Pawan Fowdur, Dragorad A. Milovanovic
- **PDF**: https://ieeexplore.ieee.org/document/10435853
- **Abstract**: The performance of 5G NR relies heavily on the channel codes used. Since Release 15 of 5G NR, Low-Density Parity Check (LDPC) codes and polar codes have been incorporated in the physical layer for error correction in the data channel and control channel respectively. The integration of machine learning (ML) with channel codes shows promise in improving error correction capabilities and overall system performance in 5G NR communications. This paper gives a review of state-of-the-art ML-based channel decoding algorithms which aim at improving the error performance of LDPC and polar codes. The Eb/N0 gains of these ML-based techniques are compared against each other to identify the most effective ones. Gaps in these research were also identified and used to propose two hybrid intelligent channel coding schemes as future works.

## New Radio Polar Coding Performance Enhancement Study Using QAM and APSK Modulation Schemes for 6G

- **Status**: ❌
- **Reason**: Polar 코드 + 고차변조(16APSK/QAM) 성능 개선 — LDPC 아님, 떼어낼 LDPC 기법 없음
- **ID**: ieee:10441959
- **Type**: conference
- **Published**: 30-31 Dec.
- **Authors**: Edom Wondimu Tadesse, Pushparaghavan Annamalai, Fikreselam Gared Mengistu
- **PDF**: https://ieeexplore.ieee.org/document/10441959
- **Abstract**: In various regions of the world, the 5G network has been operational since 2020. In 5G better spectrum and power resources are included as extra features. A better network is still required, nevertheless, to support the Internet of Everything and the needs of the holographic society. Consequently, it is anticipated that the 6G network will debut in 2030 to meet future demand. The new 6G technologies will improve the air interface and transmission technologies. Enhanced channel coding is one of these new technologies. For 5G, the 3GPP chose the channel coding techniques to be LDPC and Polar codes. By enhancing the 5G polar codes performance it can be applied as channel coding techniques in 6G. Using higher-order modulation techniques with these polar codes is one approach. We have used polar codes with 16APSK and QAM with low code rates to improve the performance of 5G polar codes. The result showed better BER performance for polar codes with 16APSK and QAM at a low coding rate than BPSK modulation with a high code rate.

## Joint Radio Frequency and Optical Spectral Optimization for High-Speed Wide Area Fiber Optic Communications

- **Status**: ❌
- **Reason**: RF/광 파이버 스펙트럼 최적화 — LDPC 무관, 떼어낼 ECC 기법 없음
- **ID**: ieee:10442045
- **Type**: conference
- **Published**: 29-31 Dec.
- **Authors**: Vivek V, Aditya Kumar, S. Yuvaraj +3
- **PDF**: https://ieeexplore.ieee.org/document/10442045
- **Abstract**: This paper affords a progressive approach to high-pace wide area fiber optic (WAFO) communications by way of combining radio frequency (RF) and optical spectral optimization strategies. This method presents a new approach to the longstanding task of reaching high-pace, long-distance transmission over current networks. The optimization consists of maximizing the spectral performance of the RF and optical structures with the aid of the use of techniques which includes frequency-agile modulation codec's, linear dispersion compensation, and spectral shaping strategies. Moreover, dynamic optical and RF network routing algorithms are proposed to enhance potential and reduce latency. The proposed optimization is confirmed thru enormous simulation and numerical analysis. Results indicate big overall performance gains in phrases of spectral performance, records price, postpone, and reliability. This joint RF-optical optimization approach is an efficient and value-effective answer for WAFO networks and ought to notably benefit large-scale and future WAFO deployments.

## Information-Theoretic Security in BB84 QKD

- **Status**: ❌
- **Reason**: BB84 QKD 후처리, 표준 LDPC/Hamming/Polar 코드 선택 비교만 — 양자 보안 도메인, 새 디코더/구성 기여 없음
- **ID**: ieee:10368530
- **Type**: conference
- **Published**: 29 Nov.-1 
- **Authors**: Shawn A. Prestridge, James G. Dunham, Dinesh Rajan
- **PDF**: https://ieeexplore.ieee.org/document/10368530
- **Abstract**: This paper demonstrates ways to improve the post-processing phase of BB84-based Quantum Key Distribution (QKD) while maintaining Information-Theoretic security. Maximum entropy in the distilled bits can be achieved provided that we follow certain rules on which bits are sacrificed based on the properties of the generator matrix. We use standard decoder techniques to distill bits and establish Information-Theoretic security and provide insights into how those techniques can be adapted to provide greater bit throughput. Finally, we compare popular coding schemes in BB84 post-processing as a basis for code selection and discover at low initial Bit Error Rates (BER), a Polar 4/5 or Hamming* give the best performance and for higher initial BER, the LDPC 3/4 and Hamming* (a modified Hamming code) give the best performance [1] [2] [9] [10].

## 5G NR: Enhanced QC-LDPC Code Development

- **Status**: ❌
- **Reason**: 5G NR QC-LDPC를 pruning으로 rate matching 최적화 — 표준 QC-LDPC 응용 특이적, 떼어낼 신규 기법 빈약
- **ID**: ieee:10451756
- **Type**: conference
- **Published**: 19-23 Dec.
- **Authors**: Asha. KS, Akhilendra Pratap Singh, Annapurna H. S.
- **PDF**: https://ieeexplore.ieee.org/document/10451756
- **Abstract**: The fifth generation (5G) new radio (NR) standard relies heavily on QC-LDPC codes to effectively rectify errors in data lines. To achieve the exacting standards of high capacity and low latency, the QC-LDPC codes' encryption and decoding in the 5G NR system requires a number of optimizations. The rate matching procedure, which chooses particular code bits for transfer and eliminates the need to encrypt and decrypt unused bits, is a crucial component. This study presents an effective 5G NR-specific application for QC-LDPC codes. It focuses on full-base matrix optimization by pruning, resulting in increased efficiency for QC-LDPC algorithms in 5G NR.

## Impact of LDPC Encoded Data in Uplink of 5G PD-NOMA System

- **Status**: ❌
- **Reason**: 5G PD-NOMA에서 LDPC 인코딩 데이터의 변조·채널모델별 BER 분석, LDPC 부수 사용·떼어낼 기법 없음 — 제외
- **ID**: ieee:10435278
- **Type**: conference
- **Published**: 18-19 Dec.
- **Authors**: Parameswaran Ramesh, Bollineni Krishna Sai, Ranjith V +1
- **PDF**: https://ieeexplore.ieee.org/document/10435278
- **Abstract**: In order to increase data transmission reliability across noisy channels, 3GPP-recommended 5G Non-Orthogonal Multiple Access (NOMA) communication systems use an error-correcting code called Low Density Parity Check (LDPC). The objective of this research is to analyze the LDPC-encoded data with various modulation schemes over different channel models. The investigation is restricted to QAM and QPSK modulation schemes over free space models, two-ray models, and log normal distribution channel models. The simulation is carried out in MATLAB version R2022b. The BER obtained in each model with respect to distance is examined. From the analysis, it is inferred that QAM exhibits a higher BER when compared to QPSK under the all-channel model. However, QAM-modulated data can be transferred over longer distances than QPSK when connectivity is ensured. Also, a higher data rate can be achieved when compared to QPSK. In 16 QAM scenario the experienced BER would be 8.9*10-8 in the Log Normal Channel model.

## Implementation and Performance Analysis of NOMA on Software Defined Radio

- **Status**: ❌
- **Reason**: NOMA SDR 구현에서 LDPC를 룩업테이블 인코더로 단순 사용; 신규 기법 없음
- **ID**: ieee:10468708
- **Type**: conference
- **Published**: 17-20 Dec.
- **Authors**: Vimal Bhatia, Shubham Bisen
- **PDF**: https://ieeexplore.ieee.org/document/10468708
- **Abstract**: Utilizing non-orthogonal multiple access (NOMA) in the downlink enhances the communication network’s capacity and improves its spectral efficiency. It can theoretically serve a large number of users in each time/frequency slot as compared to orthogonal multiple access techniques that only serve one user per orthogonal slot. This technique is of importance for ultra-dense networks in beyond 5G and 6G networks for mobile communication, as there is a need for massive connectivity, high spectral efficiency and high system throughput. However, there is dearth of research on practical implementation and studying the impact of different channel encoding. Hence the objective of this paper is to implement NOMA on software defined radio platform considering different channel coding schemes. We also observe the execution and memory usage for three popular channel encoders/decoders used for implementing NOMA, namely Hamming code, Convolutional code and low-density parity check (LDPC) code. A look-up table is utilized for designing the LDPC encoder, and its viability is analyzed. Further, comparison of bit error rate for the considered model is performed.

## On Performance Evaluation of Channel Codes over Satellite Channels

- **Status**: ❌
- **Reason**: 위성채널 채널코드 BER 성능평가; 신규 디코더/구성/HW 없는 순수 성능비교
- **ID**: ieee:10468705
- **Type**: conference
- **Published**: 17-20 Dec.
- **Authors**: Shubham Srivastava, Adrish Banerjee, Abhishek Kumar Gupta
- **PDF**: https://ieeexplore.ieee.org/document/10468705
- **Abstract**: This paper presents a comprehensive performance evaluation of error control coding techniques for reliable satellite communication links. Various modulation formats including 64-QAM (Quadrature Amplitude Modulation), QPSK (Quadrature Phase Shift Keying), 8-PSK, and 16-QAM are considered over satellite channel models. To accurately model real-world impairments, a flexible Lutz model is considered, with parameters optimized to match measurements across frequency bands like L and UHF. Extensive Monte Carlo simulations quantify bit error rate (BER) performance under different code/modulation configurations and channel conditions.

## An Upper Bound of Block Error Rate for Finite-Length Coding in Massive MISO Systems

- **Status**: ❌
- **Reason**: MISO 유한길이 부호 BLER 상한 + Hadamard 코딩; 순수 이론 bound, 떼어낼 LDPC 디코더/구성 없음
- **ID**: ieee:10543953
- **Type**: conference
- **Published**: 17-20 Dec.
- **Authors**: Xinwei Xu, Dengsheng Lin, Yue Xiao
- **PDF**: https://ieeexplore.ieee.org/document/10543953
- **Abstract**: In this contribution, we provide an upper bound of block error rate (BLER) for massive multiple-input single-output (MISO) antenna systems when the number of transmission antennas approaches infinity. In our model, we investigate a coding method suitable for a finite-length code in MISO systems. Specifically, an antenna transmission power allocation algorithm is derived for the MISO system first, which ensures that the encoded symbols on every transmission antenna are over the equivalent channels with the same signal-to-noise ratio (SNR). Furthermore, we introduce the orthogonal Hadamard coding and develop the upper bound of the BLER for the finite-length code. Finally, the proposed upper bound for finite-length code is validated through simulations, showing its approximation to both the dispersion bound and the Shannon bound when the code length is short with low code rate.

## Blind Estimation of Polar Encoder Parameters

- **Status**: ❌
- **Reason**: 폴라 부호 파라미터 블라인드 추정; LDPC 무관, 디코더 이식 기법 없음
- **ID**: ieee:10468860
- **Type**: conference
- **Published**: 17-20 Dec.
- **Authors**: Aman Gupta, Swaminathan R.
- **PDF**: https://ieeexplore.ieee.org/document/10468860
- **Abstract**: Forward error correction (FEC) codes are widely used to correct the errors in the transmitted data and it improves the reliability of the digital transmission. In a practical communication system, accurate information about the channel coding technique is required at the receiver side. However, in certain applications and future generation communication systems, it is mandatory to propose automatic code recognition techniques at the receiver in order to successfully decode the received information signals. In this paper, we propose two algorithms for blind estimation of codeword length and code dimension of polar codes, which are included as codes for the control channels in the fifth generation (5G) wireless standard. The two algorithms work in a sequence, where the first algorithm will estimate the value of codeword length and the second algorithm will estimate the value of code dimension. The proposed algorithms are compared with the existing algorithms in the literature in terms of accuracy of estimation of code parameters. The results of the comparative analysis and the test case results based on the proposed algorithms are reported.

## Improved Reliability of Enhanced Sparse Vector Coding Using Maximal Ratio Transmission for Short Packet Transmission

- **Status**: ❌
- **Reason**: sparse vector coding 단패킷 전송+MRT; 채널 ECC 아님, LDPC 무관
- **ID**: ieee:10469145
- **Type**: conference
- **Published**: 17-20 Dec.
- **Authors**: Dolly Kumari, Mahima Chaudhary, Risham Doley +1
- **PDF**: https://ieeexplore.ieee.org/document/10469145
...
- **Status**: ❌
- **Reason**: 폴라 부호 SCL 리스트 디코더; 부호 의존적 path-sorting 기법으로 바이너리 LDPC BP 이식 불가
- **ID**: ieee:10543790
- **Type**: conference
- **Published**: 17-20 Dec.
- **Authors**: Shan Chen, Shan Cao, Limin Jiang +1
- **PDF**: https://ieeexplore.ieee.org/document/10543790
- **Abstract**: The utilization of 5G and beyond wireless communication systems in autonomous driving and remote healthcare has necessitated the development of channel codes with high reliability and low latency. The multi-bit successive cancellation list (MSCL) parallel decoding of polar codes has shown great potential in the decrease of decoding latency while maintaining a high level of reliability, in comparison to the conventional successive cancellation list (SCL) decoding. However, the complexity of decoding path sorting increases exponentially with the increase of parallelism, which poses significant limitations to the improvement of parallelism. To address this issue, this paper proposes a critical set aided multi-bit successive cancellation list (CS-MSCL) decoding algorithm. Critical sets are introduced to the MSCL decoding, which prunes less reliable candidate paths to guarantee high decoding performance. Meanwhile, dynamic configuration is allowed according to the application scenario to make a good balance in decoding latency, error correction performance, and algorithmic complexity. Simulation results demonstrate that the algorithmic complexity decreased by 1099.04% compared to MSCL decoding for (1024, 512) polar codes when the degree of parallelism is 8 and the list size is 16.

## Doppler-Frequency Domain Transmission Based on OFDM

- **Status**: ❌
- **Reason**: Doppler-frequency OFDM 전송/등화 — LDPC 무관, ECC 기법 없음
- **ID**: ieee:10407186
- **Type**: conference
- **Published**: 15-17 Dec.
- **Authors**: Ziheng Wei, Zhisong Bie, Jinhong Jiang
- **PDF**: https://ieeexplore.ieee.org/document/10407186
- **Abstract**: In this paper, a new D-F domain transmission based on OFDM system is proposed. The signal is transmitted in the Doppler frequency domain, and the signal is transformed in a two-dimensional way, which retains the frequency diversity gain and can resist the Doppler effect to a certain extent. For this modulation method, a two-level equalization scheme is proposed, which can guarantee the performance of the above two advantages to a certain extent, and the D-F, OFDM and OTFS systems are compared under different channels from theory and simulation. The simulation results show that the D-F domain transmission achieves good performance in some channels.

## Image transmission in underwater acoustic communication channel using LDPC codes

- **Status**: ❌
- **Reason**: 수중음향통신 응용에 표준 LDPC를 그대로 적용. 떼어낼 신규 디코더/HW/코드설계 기여 없음
- **ID**: ieee:10466028
- **Type**: conference
- **Published**: 14-16 Dec.
- **Authors**: Avik Kumar Das, Saikat Chandra Bakshi, Ankita Pramanik +1
- **PDF**: https://ieeexplore.ieee.org/document/10466028
- **Abstract**: Traditional methods in underwater acoustic communication (UWAC) often struggle to overcome signal degradation and noise interference. This paper explores the application of low-density parity-check (LDPC) codes for improved image transmission in UWAC. The proposed work demonstrates the efficacy of a rate six by seven and a rate five by six LDPC codes in the UWAC channel to compensate the variable nature of noises in UWAC.

## Construction of New Finite Geometry Non-Binary LDPC Codes With Reduced Alphabets

- **Status**: ❌
- **Reason**: 유한기하 비이진(non-binary) LDPC 코드 구성 — 비이진 LDPC 명시적 제외
- **ID**: ieee:10365279
- **Type**: conference
- **Published**: 11-13 Dec.
- **Authors**: Zakaria M'Rabet, Fouad Ayoub, Mostafa Belkasmi +1
- **PDF**: https://ieeexplore.ieee.org/document/10365279
- **Abstract**: The finite Euclidean and Projective geometry codes are the most known among One-Step Majority-Logic Decodable codes that are Low-Density Parity-Check codes as well. Unfortunately, these codes are rare, and we do not have a great diversity when designing communication systems with a fixed code-length and code rate. Regarding the non-binary Euclidian and projective geometric Codes, each code is restricted to a unique alphabet. To offer a bigger range of possible alphabets for each code, we propose new construction by redefining the incidence vector of the lines of Euclidean and projective geometries over finite fields. This approach leads to an enrichment of the class of EG and PG codes. An error performance study of some new codes has been carried out and shows good results.

## A New Hybrid Structure for Concatenated Codes

- **Status**: ❌
- **Reason**: polar+Hamming concatenated erasure 부호(QUIC-FEC), 비-LDPC이며 떼어낼 BP 디코더 기법 없음.
- **ID**: ieee:10426327
- **Type**: conference
- **Published**: 1-3 Dec. 2
- **Authors**: Ruonan Cheng, Lijun Zhang
- **PDF**: https://ieeexplore.ieee.org/document/10426327
- **Abstract**: Erasure codes play an important role in improving network performance. Adding erasure codes to QUIC can effectively reduce data retransmission times, leading to better network performance. However, the complexity of the RS codes used in QUIC-FEC is high. Using short polar codes can reduce complexity. However, short polar codes have poor erasure correction performance. To solve this problem, we first proposed a method of combining (7, 4) Hamming codes with short polar codes in parallel. After providing protection for the incompletely polarized channels, the performance was significantly improved while maintaining low complexity and delay. Subsequently, based on Tanner diagrams of polar codes and Monte Carlo simulations, we determined the key bits that had the significant impact on the performance of the parallel concatenated codes. Therefore, we proposed a new parallel-serial hybrid structure, using (7, 4) Hamming codes to provide additional protection for these key bits. The results showed that the performance of the hybrid concatenated codes was better than that of the parallel concatenated codes, even better than the polar codes with the same bit rate and nearly twice the length of hybrid concatenated codes. The hybrid scenario also outperformed the RS codes used in QUIC protocol to restore lost packets. This new hybrid concatenated scheme can also be directly extended to a wider range of component codes.
