...
- **Status**: ❌
- **Reason**: GMAC 다중접속용 raptor-like protograph 마스킹 — 무선 다중접속 응용 특이적, NAND ECC로 떼어낼 기법 없음
- **ID**: ieee:9454378
- **Type**: journal
- **Published**: Aug. 2021
- **Authors**: Na Gao, Yin Xu, Yihang Huang +4
- **PDF**: https://ieeexplore.ieee.org/document/9454378
...
- **Status**: ✅
- **Reason**: LDPC BP 디코딩 동적 스케줄(VN-centric, conditional innovation metric); 새 디코더 스케줄링 기법으로 NAND 이식 가능(C)
- **ID**: ieee:9427123
- **Type**: journal
- **Published**: Aug. 2021
- **Authors**: Tofar C.-Y. Chang, Pin-Han Wang, Jian-Jia Weng +2
- **PDF**: https://ieeexplore.ieee.org/document/9427123
...
- **Status**: ✅
- **Reason**: 바이너리 QC-LDPC 디코더 알고리즘(LLRC segregation)+layered scheduling+FPGA HW 아키텍처; NAND LDPC에 이식 가능(C/D)
- **ID**: ieee:9399161
- **Type**: journal
- **Published**: Aug. 2021
- **Authors**: Anuj Verma, Rahul Shrestha
- **PDF**: https://ieeexplore.ieee.org/document/9399161
...
- **Status**: ❌
- **Reason**: 4D Hurwitz 신호 성상도·set partitioning·multilevel coding; 변조/격자 설계, LDPC ECC 기법 없음
- **ID**: ieee:9439958
- **Type**: journal
- **Published**: Aug. 2021
- **Authors**: Daniel Rohweder, Sebastian Stern, Robert F. H. Fischer +2
- **PDF**: https://ieeexplore.ieee.org/document/9439958
...
- **Status**: ❌
- **Reason**: Massive MIMO 채널 사운딩 reference signal 설계; ECC/LDPC와 무관
- **ID**: ieee:9424581
- **Type**: journal
- **Published**: Aug. 2021
- **Authors**: Branislav M. Popović, Peng Wang, Fredrik Berggren
- **PDF**: https://ieeexplore.ieee.org/document/9424581
...
- **Status**: ❌
- **Reason**: 무선 CD-NOMA(SCMA/DCMA) 코드북 설계·sphere decoder; LDPC ECC와 무관, 떼어낼 디코더/HW 기법 없음
- **ID**: ieee:9369968
- **Type**: journal
- **Published**: Aug. 2021
- **Authors**: Zilong Liu, Lie-Liang Yang
- **PDF**: https://ieeexplore.ieee.org/document/9369968
...
- **Status**: ✅
- **Reason**: Novel memory-effective FPGA LDPC decoder (CNSU based on layered normalized min-sum); portable decoder + HW (C/D).
- **ID**: ieee:9599253
- **Type**: conference
- **Published**: 6-8 Aug. 2
- **Authors**: Hongyuan Li, Zhenghong Yu, Wanjun Zheng +2
- **PDF**: https://ieeexplore.ieee.org/document/9599253
...
- **Status**: ❌
- **Reason**: 지문 cancellable 바이오크립토에 LDPC를 fuzzy extractor로 사용 — 보안 응용, 새 디코더/구성/HW 없음
- **ID**: ieee:9484391
- **Type**: conference
- **Published**: 4-7 Aug. 2
- **Authors**: Xingbo Dong, Zhe Jin, Leshan Zhao +1
- **PDF**: https://ieeexplore.ieee.org/document/9484391
...
- **Status**: ❌
- **Reason**: DRM+ 디지털라디오에 표준 LDPC+LDM 적용 평가; 새 디코더/코드설계 없이 시스템 성능 측정 수준
- **ID**: ieee:9547138
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Felipe Costa Pais, Ricardo Seriacopi Rabaça, Vinicius Piro Barragam +2
- **PDF**: https://ieeexplore.ieee.org/document/9547138
...
- **Status**: ✅
- **Reason**: MET-DE 기반 저율 QC-LDPC 코드 설계+세분화된 cycle classification으로 error floor 완화 — 바이너리 코드설계(E)
- **ID**: ieee:9547020
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Jian Song, Zhitong He, Kewu Peng +1
- **PDF**: https://ieeexplore.ieee.org/document/9547020
- **Abstract**: DTMB-A adopts LDPC codes with three code rates 1/2, 2/3 and 5/6, and low-rate LDPC codes are needed by DTMB-A to support scenarios which require low spectral efficiency and carrier-to-inference ratio. In this paper, a potential design of low-rate DTMB-A LDPC codes is presented. The newly-designed LDPC codes employ a lower-triangular Raptor-like base matrix, which is carefully optimized by multi-edge-type density evolution (MET-DE). A more detailed cycle classification method is proposed to mitigate the error floor of the newly designed codes, while progressive lifting is employed to keep the feature of length-scalability. BER and BLER simulation results are presented, which demonstrates that the newly-designed low-rate DTMB-A LDPC codes have better performance than ATSC 3.0 Type-A LDPC codes with similar code lengths, within both waterfall region and error floor region, and over both BI-AWGN and Rayleigh fading channels.

## Piecewise Linear Interpolation based LOG-BP algorithm for 5G LDPC codes

- **Status**: ✅
- **Reason**: BP를 log-domain 구간선형보간(PLI-LOG-BP)으로 근사하고 MMSE 기반 분할점 선택 제시 — 이식 가능한 새 디코더 알고리즘(C)
- **ID**: ieee:9547172
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Bin Xu, Yin Xu, Dazhi He +3
- **PDF**: https://ieeexplore.ieee.org/document/9547172
- **Abstract**: BP algorithm is one of the most prominent decoding algorithms based on message passing. However, it is not commonly adopted in hardware implementation for the high computational complexity. The normalized min-sum (NMS) algorithm was then proposed to solve this problem but with performance flaws. To satisfy the comprehensive requirements of complexity and accuracy in 5G systems, we introduce an approximated algorithm based on BP in log-domain and name it piecewise linear interpolation based LOG-BP (PLI-LOG-BP) algorithm. In our novel algorithm, the non-linear mapping function in the horizontal step of BP is replaced by several linear functions for simplification. Meanwhile, we also propose an MMSE-based selection algorithm of segment points that improves the approximation accuracy. To verify the validation of the proposed algorithm, we apply 5G LDPC codes in our simulation and obtain BLER performance results. According to these results, PLI-LOG-BP algorithm decoding performance is significantly better than the NMS algorithm and even almost as good as the BP algorithm.

## DE-aided ANMSA with edge classification and its application for 5G-NR LDPC codes

- **Status**: ✅
- **Reason**: MET-DE+KL divergence로 edge-classification 적응형 정규화 min-sum(ANMSA) 제안 — 새 min-sum 변형 디코더(C)
- **ID**: ieee:9547073
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Ziqi Zhou, Kewu Peng, Jian Song +1
- **PDF**: https://ieeexplore.ieee.org/document/9547073
- **Abstract**: Belief propagation (BP) provides the basis for the design of 5G new radio low density parity check (5G-NR LDPC) codes, which in turn require a BP-based decoding algorithm. The min-sum algorithm (MSA), the simplification of sum-product algorithm (SPA), should hardly be regarded as a BP-based decoding algorithm because of its minimization. To remedy this, this paper adopts the multi-edge-type density evolution (MET-DE) as the main analysis tool, and introduces the concept of Kullback-Leibler divergence. Then the least Kullback-Leibler divergence criterion is proposed to identify the optimal normalization factors. Finally, Density-Evolution-aided Adaptive Normalization Min-Sum Algorithm (DE-aided ANMSA) with edge classification is further proposed for the decoding implementation of 5G-NR LDPC codes. Simulation results demonstrate that the proposed decoding algorithm can achieve excellent performance, while the greatest advantage is that its factors could be obtained more flexibly and accurately than traditional training methods. In addition, the optimal factors for base graph 2 in 5G-NR LDPC codes are provided as well.

## Efficient Multicast Schemes in Vehicle Network Based on Luby Transform Codes

- **Status**: ❌
- **Reason**: LT(fountain) 코드 기반 멀티캐스트 재전송 기법; LDPC는 패킷 전송 수단으로 부수 언급, 떼어낼 ECC 디코더/구성 기여 없음
- **ID**: ieee:9547079
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Bin Xu, Yin Xu, Dazhi He +3
- **PDF**: https://ieeexplore.ieee.org/document/9547079
- **Abstract**: Multicast technology in vehicle network is widely concerned with the development of the fifth-generation (5G) systems. The base station is required to transmit the public information to the users in coverage more reliably and precisely. In this paper, two effective schemes named serial LT codes (SLT) scheme and parallel LT codes (PLT) scheme are proposed to enhance the communication reliability. Compared to traditional retransmission, these schemes require fewer retransmitted blocks for correction, occupying shorter bandwidth and achieving lower latency. In our schemes, the base station generates retransmitted blocks by LT codes and transmits them as LDPC packets. Users receive the information and decode the needed parts for error correction. We introduce the detailed processes and derivation of our schemes. The simulation shows their low retransmission block consumption which benefits efficient communication in a multicast system.

## Improved Repetition Transmission for NR-MBS

- **Status**: ❌
- **Reason**: NR-MBS 반복전송(slot aggregation) 신뢰도 개선 — 무선 응용, 떼어낼 LDPC ECC 기법 없음
- **ID**: ieee:9547022
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Seok-Ki Ahn, Kyung-Joong Kim, Sung-Ik Park
- **PDF**: https://ieeexplore.ieee.org/document/9547022
- **Abstract**: This paper proposes modified repetition transmission schemes to improve the reliability of 5G new radio (NR) multicast broadcast service (MBS), which is based on the slot aggregation in 3GPP release (Rel-) 15. By transmitting multiple PDSCHs conveying the same transport block (TB) within a repetition period, NR-MBS can guarantee the decoding performance for a large number of users. We modify the physical layer processing of PDSCH generation compared with the Rel-15 slot aggregation for enhancing decoding performance without any increase of computational and operational complexity. Based on the performance analysis of the proposed scheme and computer simulations over mobile environments, it is verified that the proposed schemes would improve the reliability of NR-MBS comparable with the slot aggregation scheme in Rel-15.

## Non-Equiprobable Non-Uniform APSK Constellations Design for BICM Systems

- **Status**: ❌
- **Reason**: APSK 비균일 성상도 설계(변조)로 LDPC ECC 기법 아님; 떼어낼 디코더/코드설계 기여 없음
- **ID**: ieee:9547196
- **Type**: conference
- **Published**: 4-6 Aug. 2
- **Authors**: Xiaohan Duan, Hanjiang Hong, Yin Xu +3
- **PDF**: https://ieeexplore.ieee.org/document/9547196
...
- **Status**: ✅
- **Reason**: 가상 채널 출력으로 BP 수렴 가속하는 새 디코딩 기법(LDPC-CC) - 이식 가능 디코더 알고리즘(C)
- **ID**: ieee:9548754
- **Type**: conference
- **Published**: 30-31 Aug.
- **Authors**: Chung-Hsuan Wang, Jo-Han Lu
- **PDF**: https://ieeexplore.ieee.org/document/9548754
...
- **Status**: ❌
- **Reason**: polar code의 design-SNR과 BP 디코더 BER - LDPC 아닌 polar, NAND LDPC 이식 기법 없음
- **ID**: ieee:9548765
- **Type**: conference
- **Published**: 30-31 Aug.
- **Authors**: Kohei Ueda, Satoshi Suyama, Takahiro Asai +1
- **PDF**: https://ieeexplore.ieee.org/document/9548765
- **Abstract**: In order to reduce the decoding latency, the belief propagation (BP) decoder, which is generally used for decoding of the low-density parity-check (LDPC) codes, is effective. In general, the error rate performance of BP decoder is degraded. Therefore, this paper investigates the influence of design signal-to-noise ratio (SNR) on the bit error rate (BER) performance under BP decoder. We first generate the multiple polar codes by setting the multiple design-SNR. The BER performance is used to optimize the polar code for the BP decoder. The simulation results employing N = 1024 and the coding rate of 1/2 show that by setting slightly larger design-SNR compared to the SC decoder, the BER performance is slightly improved.

## Transmission Performance of OFDM-based 1024-QAM under Different Types of Multipath Fading Channels

- **Status**: ❌
- **Reason**: OFDM 1024-QAM 무선 전송 성능 - LDPC 무관, ECC 기법 없음
- **ID**: ieee:9548774
- **Type**: conference
- **Published**: 30-31 Aug.
- **Authors**: Daisuke Kosuge, Hiroyuki Otsuka
- **PDF**: https://ieeexplore.ieee.org/document/9548774
- **Abstract**: Fifth-generation (5G) new radio (NR) supports multiple subcarrier spacings of orthogonal frequency division multiplexing (OFDM) signals. 5G NR also supports 1024-quadrature amplitude modulation (QAM) for 5G allowing to achieve higher peak data rates and improve spectral efficiency. In this paper, we present the transmission performance of OFDM-based 1024-QAM for multiple subcarrier spacings under different types of multipath fading channels, i.e., Rayleigh fading channel in frequency range 1 (FR1), Rayleigh fading channel in FR2, and Rician fading in FR2. Using link-level computer simulations, we clarify the BER performance of OFDM-based 1024-QAM as parameters of coding rates for the subcarrier spacings of 15 and 240 kHz under three types of multipath fading channels based on tapped delay line models.

## Performance analysis of NOMA using different types Receivers

- **Status**: ❌
- **Reason**: NOMA 수신기 성능 분석에 LDPC 부수 언급 - 떼어낼 디코더/구성 없음
- **ID**: ieee:9563918
- **Type**: conference
- **Published**: 30-31 Aug.
- **Authors**: Roaa Ali Hussein, Saad Ahmed Ayoob
- **PDF**: https://ieeexplore.ieee.org/document/9563918
- **Abstract**: The 5G supports several technologies aimed at improving the performance of the communication networks in terms of the sender, channel, and receiver. One of these techniques, Non-Orthogonal Multiple Access (NOMA) that improves transmission performance and channel access. NOMA completes performance in the receiver. This paper analyzes NOMA performance evaluation in two methods. The first method compares the efficiency of different types of NOMA receiver schemes such as Zero Forcing (ZF), Maximum Likelihood (ML), Sphere, and Minimum Mean Square-Error (MMSE). The second method changes the pattern of user sites for NOMA cell and suggests an optimal NOMA receiver scheme to serve users at the Cell Edge (CE) and obtain a greater user throughput rate. Simulation performed with the Vienna 5G Link Level Simulator (LLS), and modulation type used in encoding is Orthogonal Frequency Division Multiplexing (OFDM). Also, the low-density parity test (LDPC) coding is used. The results show the influence of the receivers, changing the receivers (ZF, MMSE) does not affect the throughput of the subscribers (UE3, UE4) when reaching transmitted power (17.14 dBm). Also, (MMSE and ZF) receivers achieve better user service performance in (CE) than Sphere type receiver by one-third of the percentage. While ML algorithm achieved similar performance as ZF, MMSE but at a transmitted power greater than (17.14 dBm).

## Analysis of anti-interference performance of navigation signal based on FH-BOC modulation system

- **Status**: ❌
- **Reason**: FH-BOC 항법 신호 변조 시스템, LDPC와 무관. ECC 기법 없음
- **ID**: ieee:9574148
- **Type**: conference
- **Published**: 27-28 Aug.
- **Authors**: Minshu Zhang, Lixin Zhang, Qibing Xu +1
- **PDF**: https://ieeexplore.ieee.org/document/9574148
- **Abstract**: Based on the existing BOC modulation system, we propose a new navigation signal system based on Frequency Hopping-BOC (FH-BOC) is proposed. This paper establishes the mathematical model of FH-BOC, and analyzes some representative FH-BOC signal characteristics. The results show that the proposed FH-BOC signal has excellent code tracking performance, anti-interference and anti-multipath performance. The proposed modulation scheme can provide a new idea for the design of the next generation GNSS navigation signal system.

## The optimization of error floor in M-QAM multilevel coded modulation scheme based on LDPC code

- **Status**: ✅
- **Reason**: LDPC 기반 MLCM error floor 최적화(성분 부호율 조정) — error floor 코드설계 기법(E) 가능성, 애매하여 보존
- **ID**: ieee:9563725
- **Type**: conference
- **Published**: 23-27 Aug.
- **Authors**: Mao Ge, Liqian Wang, Zhihui Wang +1
- **PDF**: https://ieeexplore.ieee.org/document/9563725
- **Abstract**: In M-QAM multilevel coded modulation scheme with LDPC, the system error floor can be effectively reduced and the coding gain can be improved by adjusting the component code rate based on the capacity rule.

## VAMP with Individual Variances and Sequential Processing for Compressed Sensing

- **Status**: ❌
- **Reason**: 압축센싱 VAMP 순차 스케줄링, 채널코딩 LDPC 디코더 아님
- **ID**: ieee:9616006
- **Type**: conference
- **Published**: 23-27 Aug.
- **Authors**: Carmen Sippel, Robert F.H. Fischer
- **PDF**: https://ieeexplore.ieee.org/document/9616006
- **Abstract**: The compressed sensing (CS) problem is considered, where a sparse signal with known structure has to be recovered from an under-determined system of linear equations. Iterative algorithms in CS alternate between two subproblems, which are given by the structure of the signal and the system of equations, respectively. Hereby, the components of the signal vector are usually updated in parallel. Alternatively, a sequential processing of the components of the signal vector is possible, where the order of processing can be varied and individual variances are utilized. A suitable scheduling can speed up the convergence and thereby reduce the complexity, which is necessary to achieve a certain performance level. In this paper, we introduce a sequential algorithm for CS, discuss a variety of schedules and show by numerical simulations that the number of signal components to be processed in order to attain a specific performance level can be significantly reduced with suitable schedules.
...
- **Status**: ❌
- **Reason**: WSS 대역협소화 하 확률성형 64-QAM 정보율 평가, 코딩 기법 없음
- **ID**: ieee:9563647
- **Type**: conference
- **Published**: 23-27 Aug.
- **Authors**: Linan Li, Junjie Zeng, Minglong Pu +3
- **PDF**: https://ieeexplore.ieee.org/document/9563647
- **Abstract**: An efficient procedure is presented for evaluating the performance implications of bandwidth narrowing due to cascaded WSSs on probabilistically shaped and uniform 64- QAM using the BW AIR. The constellation distribution that yields the optimum shaping gain in BW AIR for a given SNR depends on the extent of the narrowing.

## The research of probabilistic shaping signal transmission scheme based on neural network LLR calculation

- **Status**: ✅
- **Reason**: 신경망 기반 LLR 계산 기법 — NAND LDPC LLR 양자화/연산에 이식 가능성(C), 보존
- **ID**: ieee:9563893
- **Type**: conference
- **Published**: 23-27 Aug.
- **Authors**: Pandi Pang, Huan Chang, Qi Zhang +6
- **PDF**: https://ieeexplore.ieee.org/document/9563893
