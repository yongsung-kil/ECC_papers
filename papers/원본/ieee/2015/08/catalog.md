...
- **Status**: ❌
- **Reason**: 바이너리 LDPC 코셋리더 그래프 기반 rate-distance bound 개선, 순수 이론 bound로 디코더/HW/구성으로 안 이어짐
- **ID**: ieee:7114293
- **Type**: journal
- **Published**: Aug. 2015
- **Authors**: Eran Iceland, Alex Samorodnitsky
- **PDF**: https://ieeexplore.ieee.org/document/7114293
...
- **Status**: ❌
- **Reason**: 서베이(SC-LDPC/비이진 LDPC/polar 개관) — 구체적 신규 디코더·구성 기여 없음
- **ID**: ieee:7265214
- **Type**: journal
- **Published**: Aug. 2015
- **Authors**: Erdal Ankan, Najeeb ul Hassan, Michael Lentmaier +2
- **PDF**: https://ieeexplore.ieee.org/document/7265214
...
- **Status**: ❌
- **Reason**: 무선 멀티캐스트 채널상태피드백 스킴, LDPC 무관 통신 기법
- **ID**: ieee:7113812
- **Type**: journal
- **Published**: Aug. 2015
- **Authors**: In-Ho Lee, Seok-Chul Kwon
- **PDF**: https://ieeexplore.ieee.org/document/7113812
- **Abstract**: Multi-group multicasting has recently attracted substantial attention. In this letter, we propose quality-based channel state information feedback (QCF) in multicast systems with greedy scheduling of multiple multicast groups, and present exact expressions for outage probability and average capacity of the multicast systems using QCF over independent Rayleigh fading channels. Further, we provide a method to determine the appropriate QCF threshold, and describe the relative feedback load of QCF with respect to full channel state information feedback. Our QCF scheme can significantly reduce the feedback load, simultaneously accomplishing negligible system performance loss.

## Efficient Soft-Input Soft-Output MIMO Chase Detectors for Arbitrary Number of Streams

- **Status**: ❌
- **Reason**: MIMO Chase 검출기(IDD), LDPC ECC와 무관한 검출 기법으로 이식 대상 없음
- **ID**: ieee:6997989
- **Type**: journal
- **Published**: Aug. 2015
- **Authors**: Ahmad Gomaa, Louay M. A. Jalloul
- **PDF**: https://ieeexplore.ieee.org/document/6997989
- **Abstract**: We present novel soft-input soft-output (SISO) multiple-input multiple-output (MIMO) detectors based on the Chase detection principle in the context of iterative detection and decoding (IDD). The proposed detector complexity is linear in the signal modulation constellation size and the number of spatial streams. Two variants of the SISO detector are developed, referred to as SISO block decision-feedback Chase detector (B-Chase) and SISO linear Chase detector (L-Chase). An efficient method is presented that uses the decoder output to modulate the signal constellation decision boundaries inside the detector leading to the SISO detector architecture. The performance of these detectors significantly improves with just a few number of IDD iterations. The effect of transmit and receive antenna correlation is simulated. For the high-correlation case, the superiority of SISO B-Chase over the SISO L-Chase is demonstrated.

## Performance simulation of using non-binary LDPC codes to against rain fade in Ka-band

- **Status**: ❌
- **Reason**: 비이진 LDPC(Ka대역 rain fade) — non-binary는 제외 대상
- **ID**: ieee:7279788
- **Type**: conference
- **Published**: 8-10 Aug. 
- **Authors**: Fu-xing Zhang, Sheng-wang Xu
- **PDF**: https://ieeexplore.ieee.org/document/7279788
- **Abstract**: Rain fade can lead to the deterioration of Ka-band Satellite communication performance, one of the efficient ways against rain fade is using channel coding with better performance. Non-binary LDPC codes, as an advanced error correcting code, having strong ability to correct errors, can be used to against rain fade. The paper presents an analysis on influence of rainfalls in Ka-band Satellite communication and establishes the channel model of rain fade. Simulation on system performance based on non-binary codes and rain fade is also conducted, which is brought into comparison to binary LDPC codes and uncoded situations. Result shows, LDPC codes can obtain over 20dB gains over uncoded situation, and non-binary LDPC codes have 1dB performance advantage over binary LDPC codes.
...
- **Status**: ❌
- **Reason**: Rayleigh 페이딩용 polar code 구성; 비-LDPC, dynamic frozen 기법 LDPC 이식성 없음
- **ID**: ieee:7454357
- **Type**: conference
- **Published**: 25-28 Aug.
- **Authors**: Peter Trifonov
- **PDF**: https://ieeexplore.ieee.org/document/7454357
- **Abstract**: A simple method for construction of polar codes for Rayleigh fading channel is presented. The subchannels induced by the polarizing transformation are modeled as multipath fading channels, and their diversity order and noise variance are tracked. It is shown that the performance of polar codes in fading channels can be substantially improved by employing the construction with dynamic frozen symbols. The obtained codes are shown to provide significant gain with respect to LDPC codes.

## Phase noise compensation for dually-polarized systems with independent transmission streams

- **Status**: ❌
- **Reason**: 이중편파 무선 위상잡음 보상 동기화 수신기; LDPC는 부수 언급, 떼어낼 ECC 디코더/코드 기법 없음
- **ID**: ieee:7454339
- **Type**: conference
- **Published**: 25-28 Aug.
- **Authors**: M. Martalò, G. Ferrari, M. Asim +5
- **PDF**: https://ieeexplore.ieee.org/document/7454339
...
- **Status**: ❌
- **Reason**: polar code 순차복호(Viterbi list) 개선; 부호 의존적이라 바이너리 LDPC BP로 이식 불가, 비-LDPC 제외
- **ID**: ieee:7454356
- **Type**: conference
- **Published**: 25-28 Aug.
- **Authors**: Grigorii Trofimiuk, Peter Trifonov
- **PDF**: https://ieeexplore.ieee.org/document/7454356
...
- **Status**: ❌
- **Reason**: 5G NOMA MU-BICM-ID용 Raptor-like LDPC 최적화 — 무선 MAC 채널 특이적, NAND 이식성 없음
- **ID**: ieee:7289290
- **Type**: conference
- **Published**: 24-28 Aug.
- **Authors**: Shuang Chen, Kewu Peng, Yushu Zhang +1
- **PDF**: https://ieeexplore.ieee.org/document/7289290
...
- **Status**: ❌
- **Reason**: DTMB 지상파 방송 시스템 성능분석; 무선 응용 특이적, LDPC 떼어낼 기법 없음
- **ID**: ieee:7289085
- **Type**: conference
- **Published**: 24-28 Aug.
- **Authors**: Yue Liu, Kewu Peng, Jian Song +1
- **PDF**: https://ieeexplore.ieee.org/document/7289085
- **Abstract**: Digital television terrestrial multimedia broadcasting (DTMB) is the digital television terrestrial broadcasting (DTTB) standard apllied in the People's Republic of China. The performance degradation of each transmission step in DTMB system is analyzed thoroughly in this paper. The overall degradation includes two main parts: the time-frequency bandwidth loss and imperfect coded modulation scheme. This analysis provides the guideline for the potential improvement in each step. Aiming to retrieve degradations in each process, we propose some alternative and effective proposals which could be adopted in future DTTB system.

## Comparative simulation for physical layer key generation methods

- **Status**: ❌
- **Reason**: 물리계층 키 생성/조정/프라이버시증폭 비교; 보안·reconciliation 영역, 떼어낼 LDPC 디코더/HW 기법 없음
- **ID**: ieee:7289068
- **Type**: conference
- **Published**: 24-28 Aug.
- **Authors**: Amal Saad, Amr Mohamed, Tarek M. Elfouly +2
- **PDF**: https://ieeexplore.ieee.org/document/7289068
...
- **Status**: ❌
- **Reason**: GPS Gold code LFSR upset 보호 — LDPC 무관, 비-LDPC 오류정정/redundancy
- **ID**: ieee:7282164
- **Type**: conference
- **Published**: 2-5 Aug. 2
- **Authors**: Mohamed Mourad Hafidhi, Emmanuel Boutillon, Chris Winstead
- **PDF**: https://ieeexplore.ieee.org/document/7282164
- **Abstract**: This paper1 examines five strategies for upset protection in Gold sequence generators used to maintain signal acquisition in GPS receivers. This work is motivated by the increased prevalence of single-event upsets in ultra low-power GPS receivers. If any upset occurs in the Linear Feedback Shift Register (LFSR) modules, then the corresponding satellite signal must be re-acquired, resulting in high energy expenditure and time delay. We evaluate the performance and complexity of methods based on error correction and modular redundancy.
